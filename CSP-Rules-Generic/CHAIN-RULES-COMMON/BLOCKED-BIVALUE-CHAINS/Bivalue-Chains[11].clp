...
(defrule activate-bivalue-chain[11]
   (declare (salience ?*activate-bivalue-chain[11]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont bivalue-chain))
=>
   (if ?*print-levels* then (printout t Entering_level_BC11))
   (assert (technique ?cont bivalue-chain[11]))
   (bind ?*technique* BC[11])
)



(defrule track-bivalue-chain[11]
   (declare (salience ?*activate-bivalue-chain[11]-salience*))
   ?level <- (technique ?cont bivalue-chain[11])
=>
...
(defrule partial-bivalue-chain[11]
   (declare (salience ?*partial-bivalue-chain[11]-salience*))
   (logical
      (chain
         (type bivalue-chain)
         (context ?cont)
      ...
      (technique ?cont bivalue-chain[11])
      ;;; ?new-llc
...
(defrule apply1-bivalue-chain[11]
   (declare (salience ?*bivalue-chain[11]-salience*))
   (chain
      (type bivalue-chain)
      (context ?cont)
      (length 11)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L11* ?*print-bivalue-chain* ?*print-bivalue-chain-11*) then
      (print-bivalue-chain-without-crlf 11 ?zzz $?llcs $?rlcs $?csp-vars)
   )
   (assert (apply-rule-as-a-pseudo-block ?cont))
   (assert (pseudo-blocked ?cont bivalue-chain[11] 11 ?zzz $?llcs $?rlcs $?csp-vars))
)



(defrule apply2-bivalue-chain[11]
   ...
   (pseudo-blocked ?cont bivalue-chain[11] 11 ?zzz $?llcs $?rlcs $?csp-vars)
