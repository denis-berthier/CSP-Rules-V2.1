...
(defrule apply1-bivalue-chain[15]
...
=>
   ...
   (if (or ?*print-actions* ?*print-L15* ?*print-bivalue-chain* ?*print-bivalue-chain-15*) then
      (print-bivalue-chain-without-crlf 15 ?zzz $?llcs $?rlcs $?csp-vars)
   )
   (assert (apply-rule-as-a-pseudo-block ?cont))
   (assert (pseudo-blocked ?cont bivalue-chain[15] 15 ?zzz $?llcs $?rlcs $?csp-vars))
)



(defrule apply2-bivalue-chain[15]
   ...
   (pseudo-blocked ?cont bivalue-chain[15] 15 ?zzz $?llcs $?rlcs $?csp-vars)
