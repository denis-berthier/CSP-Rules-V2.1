...
(defrule apply1-bivalue-chain[17]
...
=>
   ...
   (if (or ?*print-actions* ?*print-L17* ?*print-bivalue-chain* ?*print-bivalue-chain-17*) then
      (print-bivalue-chain-without-crlf 17 ?zzz $?llcs $?rlcs $?csp-vars)
   )
   (assert (apply-rule-as-a-pseudo-block ?cont))
   (assert (pseudo-blocked ?cont bivalue-chain[17] 17 ?zzz $?llcs $?rlcs $?csp-vars))
)



(defrule apply2-bivalue-chain[17]
   ...
   (pseudo-blocked ?cont bivalue-chain[17] 17 ?zzz $?llcs $?rlcs $?csp-vars)
