...
(defrule apply1-bivalue-chain[20]
...
=>
   ...
   (if (or ?*print-actions* ?*print-L20* ?*print-bivalue-chain* ?*print-bivalue-chain-20*) then
      (print-bivalue-chain-without-crlf 20 ?zzz $?llcs $?rlcs $?csp-vars)
   )
   (assert (apply-rule-as-a-pseudo-block ?cont))
   (assert (pseudo-blocked ?cont bivalue-chain[20] 20 ?zzz $?llcs $?rlcs $?csp-vars))
)



(defrule apply2-bivalue-chain[20]
   ...
   (pseudo-blocked ?cont bivalue-chain[20] 20 ?zzz $?llcs $?rlcs $?csp-vars)
