...
(defrule apply1-bivalue-chain[4]
...
=>
   ...
   (if (or ?*print-actions* ?*print-L4* ?*print-bivalue-chain* ?*print-bivalue-chain-4*) then
      (print-bivalue-chain-without-crlf 4 ?zzz $?llcs $?rlcs $?csp-vars)
   )
   (assert (apply-rule-as-a-pseudo-block ?cont))
   (assert (pseudo-blocked ?cont bivalue-chain[4] 4 ?zzz $?llcs $?rlcs $?csp-vars))
)



(defrule apply2-bivalue-chain[4]
   ...
   (pseudo-blocked ?cont bivalue-chain[4] 4 ?zzz $?llcs $?rlcs $?csp-vars)
