...
(defrule activate-bivalue-chain[9]
   (declare (salience ?*activate-bivalue-chain[9]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont bivalue-chain))
=>
   (if ?*print-levels* then (printout t Entering_level_BC9))
   (assert (technique ?cont bivalue-chain[9]))
   (bind ?*technique* BC[9])
)



(defrule track-bivalue-chain[9]
   (declare (salience ?*activate-bivalue-chain[9]-salience*))
   ?level <- (technique ?cont bivalue-chain[9])
=>
...
(defrule partial-bivalue-chain[9]
   (declare (salience ?*partial-bivalue-chain[9]-salience*))
   (logical
      (chain
         (type bivalue-chain)
         (context ?cont)
      ...
      (technique ?cont bivalue-chain[9])
      ;;; ?new-llc
...
(defrule apply1-bivalue-chain[9]
   (declare (salience ?*bivalue-chain[9]-salience*))
   (chain
      (type bivalue-chain)
      (context ?cont)
      (length 9)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L9* ?*print-bivalue-chain* ?*print-bivalue-chain-9*) then
      (print-bivalue-chain-without-crlf 9 ?zzz $?llcs $?rlcs $?csp-vars)
   )
   (assert (apply-rule-as-a-pseudo-block ?cont))
   (assert (pseudo-blocked ?cont bivalue-chain[9] 9 ?zzz $?llcs $?rlcs $?csp-vars))
)



(defrule apply2-bivalue-chain[9]
   ...
   (pseudo-blocked ?cont bivalue-chain[9] 9 ?zzz $?llcs $?rlcs $?csp-vars)
