...
(defrule apply1-typed-bivalue-chain[6]
...
=>
   ...
   (if (or ?*print-actions* ?*print-L6* ?*print-typed-bivalue-chain* ?*print-typed-bivalue-chain-6*) then
      (print-typed-bivalue-chain-without-crlf ?csp-type 6 ?zzz $?llcs $?rlcs $?csp-vars)
   )
   (assert (apply-rule-as-a-pseudo-block ?cont))
   (assert (pseudo-blocked ?cont typed-bivalue-chain[6] ?csp-type 6 ?zzz $?llcs $?rlcs $?csp-vars))
)



(defrule apply2-typed-bivalue-chain[6]
   ...
   (pseudo-blocked ?cont typed-bivalue-chain[6] ?csp-type 6 ?zzz $?llcs $?rlcs $?csp-vars)
