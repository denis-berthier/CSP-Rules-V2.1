...
(defrule apply1-typed-bivalue-chain[9]
...
=>
   ...
   (if (or ?*print-actions* ?*print-L9* ?*print-typed-bivalue-chain* ?*print-typed-bivalue-chain-9*) then
      (print-typed-bivalue-chain-without-crlf ?csp-type 9 ?zzz $?llcs $?rlcs $?csp-vars)
   )
   (assert (apply-rule-as-a-pseudo-block ?cont))
   (assert (pseudo-blocked ?cont typed-bivalue-chain[9] ?csp-type 9 ?zzz $?llcs $?rlcs $?csp-vars))
)



(defrule apply2-typed-bivalue-chain[9]
   ...
   (pseudo-blocked ?cont typed-bivalue-chain[9] ?csp-type 9 ?zzz $?llcs $?rlcs $?csp-vars)
