...
(defrule activate-forcing-braid[8]
   (declare (salience ?*activate-forcing-braid[8]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   ...
   (assert (technique ?cont forcing-braid[8]))
   (bind ?*technique* FB[8])
)



(defrule track-forcing-braid[8]
   (declare (salience ?*activate-forcing-braid[8]-salience*))
   ?level <- (technique ?cont forcing-braid[8])
=>
...
(defrule forcing-braid[8]-value
   (declare (salience ?*forcing-braid[8]-value-salience*))
   (bivalue ?cont ?zzz1 ?zzz2 ?ll)
   (technique ?cont forcing-braid[8])
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L8* ?*print-forcing-braid* ?*print-forcing-braid-8*) then
...
(defrule forcing-braid[8]-candidate
   (declare (salience ?*forcing-braid[8]-candidate-salience*))
   (bivalue ?cont ?zzz1 ?zzz2 ?ll)
   (technique ?cont forcing-braid[8])
   (chain
      (type ?type1&:(or (eq ?type1 partial-whip) (eq ?type1 partial-braid)))
