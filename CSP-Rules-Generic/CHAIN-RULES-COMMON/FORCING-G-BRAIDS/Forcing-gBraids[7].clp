...
(defrule activate-forcing-gbraid[7]
   (declare (salience ?*activate-forcing-gbraid[7]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_FB7))
   (assert (technique ?cont forcing-gbraid[7]))
   (bind ?*technique* FgB[7])
)



(defrule track-forcing-gbraid[7]
   (declare (salience ?*activate-forcing-gbraid[7]-salience*))
   ?level <- (technique ?cont forcing-gbraid[7])
=>
...
(defrule forcing-gbraid[7]-value
   (declare (salience ?*forcing-gbraid[7]-value-salience*))
   (bivalue ?cont ?zzz1 ?zzz2 ?ll)
   (technique ?cont forcing-gbraid[7])
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L7* ?*print-forcing-gbraid* ?*print-forcing-gbraid-7*) then
...
(defrule forcing-gbraid[7]-candidate
   (declare (salience ?*forcing-gbraid[7]-candidate-salience*))
   (bivalue ?cont ?zzz1 ?zzz2 ?ll)
   (technique ?cont forcing-gbraid[7])
   (chain
      (type ?type1&:(or (eq ?type1 partial-whip) (eq ?type1 partial-gwhip) (eq ?type1 partial-braid) (eq ?type1 partial-gbraid)))
