...
=>
   ...
   (if (or ?*print-actions* ?*print-L11* ?*print-oddagon* ?*print-oddagon-11*) then
      (print-oddagon-without-crlf 11 ?zzz (create$ $?rlcs ?cand1) (create$ $?csp-vars ?new-csp))
   )
   ...
   (if (not ?*blocked-oddagons*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont oddagon[11] ?zzz $?z-cands))
   )
)
...
(defrule apply-oddagon[11]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont oddagon[11] ?zzz $?z-cands)
...
(defrule oddagon[11]-print-z-candidates
