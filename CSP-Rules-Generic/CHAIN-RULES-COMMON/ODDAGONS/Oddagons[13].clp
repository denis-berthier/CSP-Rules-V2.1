...
(defrule activate-oddagon[13]
   (declare (salience ?*activate-oddagon[13]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_O13))
   (assert (technique ?cont partial-oddagon[12]))
   (assert (technique ?cont oddagon[13]))
   (bind ?*technique* O[13])
)



(defrule track-oddagon[13]
   (declare (salience ?*activate-oddagon[13]-salience*))
   ?level <- (technique ?cont oddagon[13])
=>
...
(defrule partial-oddagon[12]
   (declare (salience ?*partial-oddagon[12]-salience*))
   (logical
      (csp-chain
      ...
      (technique ?cont partial-oddagon[12])
...
(defrule oddagon[13]
   (declare (salience ?*oddagon[13]-salience*))
   (csp-chain
      (type partial-oddagon)
      (context ?cont)
      (length 12)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L13* ?*print-oddagon* ?*print-oddagon-13*) then
      (print-oddagon-without-crlf 13 ?zzz (create$ $?rlcs ?cand1) (create$ $?csp-vars ?new-csp))
   )
   ...
   (if (not ?*blocked-oddagons*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont oddagon[13] ?zzz $?z-cands))
   )
)
...
(defrule apply-oddagon[13]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont oddagon[13] ?zzz $?z-cands)
...
(defrule oddagon[13]-print-z-candidates
