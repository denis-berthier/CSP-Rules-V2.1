...
=>
   ...
   (if (or ?*print-actions* ?*print-L15* ?*print-oddagon* ?*print-oddagon-15*) then
      (print-oddagon-without-crlf 15 ?zzz (create$ $?rlcs ?cand1) (create$ $?csp-vars ?new-csp))
   )
   ...
   (if (not ?*blocked-oddagons*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont oddagon[15] ?zzz $?z-cands))
   )
)
...
(defrule apply-oddagon[15]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont oddagon[15] ?zzz $?z-cands)
...
(defrule oddagon[15]-print-z-candidates
