...
(defrule activate-oddagon[17]
   (declare (salience ?*activate-oddagon[17]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_O17))
   (assert (technique ?cont partial-oddagon[16]))
   (assert (technique ?cont oddagon[17]))
   (bind ?*technique* O[17])
)



(defrule track-oddagon[17]
   (declare (salience ?*activate-oddagon[17]-salience*))
   ?level <- (technique ?cont oddagon[17])
=>
...
(defrule partial-oddagon[16]
   (declare (salience ?*partial-oddagon[16]-salience*))
   (logical
      (csp-chain
      ...
      (technique ?cont partial-oddagon[16])
...
(defrule oddagon[17]
   (declare (salience ?*oddagon[17]-salience*))
   (csp-chain
      (type partial-oddagon)
      (context ?cont)
      (length 16)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L17* ?*print-oddagon* ?*print-oddagon-17*) then
      (print-oddagon-without-crlf 17 ?zzz (create$ $?rlcs ?cand1) (create$ $?csp-vars ?new-csp))
   )
   ...
   (if (not ?*blocked-oddagons*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont oddagon[17] ?zzz $?z-cands))
   )
)
...
(defrule apply-oddagon[17]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont oddagon[17] ?zzz $?z-cands)
...
(defrule oddagon[17]-print-z-candidates
