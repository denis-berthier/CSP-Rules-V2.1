...
=>
   ...
   (if (or ?*print-actions* ?*print-L21* ?*print-oddagon* ?*print-oddagon-21*) then
      (print-oddagon-without-crlf 21 ?zzz (create$ $?rlcs ?cand1) (create$ $?csp-vars ?new-csp))
   )
   ...
   (if (not ?*blocked-oddagons*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont oddagon[21] ?zzz $?z-cands))
   )
)
...
(defrule apply-oddagon[21]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont oddagon[21] ?zzz $?z-cands)
...
(defrule oddagon[21]-print-z-candidates
