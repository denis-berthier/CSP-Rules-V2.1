...
(defrule activate-oddagon[23]
   (declare (salience ?*activate-oddagon[23]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_O23))
   (assert (technique ?cont partial-oddagon[22]))
   (assert (technique ?cont oddagon[23]))
   (bind ?*technique* O[23])
)



(defrule track-oddagon[23]
   (declare (salience ?*activate-oddagon[23]-salience*))
   ?level <- (technique ?cont oddagon[23])
=>
...
(defrule partial-oddagon[22]
   (declare (salience ?*partial-oddagon[22]-salience*))
   (logical
      (csp-chain
      ...
      (technique ?cont partial-oddagon[22])
...
(defrule oddagon[23]
   (declare (salience ?*oddagon[23]-salience*))
   (csp-chain
      (type partial-oddagon)
      (context ?cont)
      (length 22)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L23* ?*print-oddagon* ?*print-oddagon-23*) then
      (print-oddagon-without-crlf 23 ?zzz (create$ $?rlcs ?cand1) (create$ $?csp-vars ?new-csp))
   )
   ...
   (if (not ?*blocked-oddagons*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont oddagon[23] ?zzz $?z-cands))
   )
)
...
(defrule apply-oddagon[23]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont oddagon[23] ?zzz $?z-cands)
...
(defrule oddagon[23]-print-z-candidates
