...
(defrule activate-oddagon[27]
   (declare (salience ?*activate-oddagon[27]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_O27))
   (assert (technique ?cont partial-oddagon[26]))
   (assert (technique ?cont oddagon[27]))
   (bind ?*technique* O[27])
)



(defrule track-oddagon[27]
   (declare (salience ?*activate-oddagon[27]-salience*))
   ?level <- (technique ?cont oddagon[27])
=>
...
(defrule partial-oddagon[26]
   (declare (salience ?*partial-oddagon[26]-salience*))
   (logical
      (csp-chain
      ...
      (technique ?cont partial-oddagon[26])
...
(defrule oddagon[27]
   (declare (salience ?*oddagon[27]-salience*))
   (csp-chain
      (type partial-oddagon)
      (context ?cont)
      (length 26)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L27* ?*print-oddagon* ?*print-oddagon-27*) then
      (print-oddagon-without-crlf 27 ?zzz (create$ $?rlcs ?cand1) (create$ $?csp-vars ?new-csp))
   )
   ...
   (if (not ?*blocked-oddagons*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont oddagon[27] ?zzz $?z-cands))
   )
)
...
(defrule apply-oddagon[27]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont oddagon[27] ?zzz $?z-cands)
...
(defrule oddagon[27]-print-z-candidates
