...
=>
   ...
   (if (or ?*print-actions* ?*print-L29* ?*print-oddagon* ?*print-oddagon-29*) then
      (print-oddagon-without-crlf 29 ?zzz (create$ $?rlcs ?cand1) (create$ $?csp-vars ?new-csp))
   )
   ...
   (if (not ?*blocked-oddagons*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont oddagon[29] ?zzz $?z-cands))
   )
)
...
(defrule apply-oddagon[29]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont oddagon[29] ?zzz $?z-cands)
...
(defrule oddagon[29]-print-z-candidates
