...
=>
   ...
   (if (or ?*print-actions* ?*print-L35* ?*print-oddagon* ?*print-oddagon-35*) then
      (print-oddagon-without-crlf 35 ?zzz (create$ $?rlcs ?cand1) (create$ $?csp-vars ?new-csp))
   )
   ...
   (if (not ?*blocked-oddagons*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont oddagon[35] ?zzz $?z-cands))
   )
)
...
(defrule apply-oddagon[35]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont oddagon[35] ?zzz $?z-cands)
...
(defrule oddagon[35]-print-z-candidates
