...
=>
   ...
   (if (or ?*print-actions* ?*print-L5* ?*print-oddagon* ?*print-oddagon-5*) then
      (print-oddagon-without-crlf 5 ?zzz (create$ $?rlcs ?cand1) (create$ $?csp-vars ?new-csp))
   )
   ...
   (if (not ?*blocked-oddagons*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont oddagon[5] ?zzz $?z-cands))
   )
)
...
(defrule apply-oddagon[5]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont oddagon[5] ?zzz $?z-cands)
...
(defrule oddagon[5]-print-z-candidates
