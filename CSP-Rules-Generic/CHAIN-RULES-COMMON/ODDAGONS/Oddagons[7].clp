...
(defrule activate-oddagon[7]
   (declare (salience ?*activate-oddagon[7]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_O7))
   (assert (technique ?cont partial-oddagon[6]))
   (assert (technique ?cont oddagon[7]))
   (bind ?*technique* O[7])
)



(defrule track-oddagon[7]
   (declare (salience ?*activate-oddagon[7]-salience*))
   ?level <- (technique ?cont oddagon[7])
=>
...
(defrule partial-oddagon[6]
   (declare (salience ?*partial-oddagon[6]-salience*))
   (logical
      (csp-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L7* ?*print-oddagon* ?*print-oddagon-7*) then
      (print-oddagon-without-crlf 7 ?zzz (create$ $?rlcs ?cand1) (create$ $?csp-vars ?new-csp))
   )
   ...
   (if (not ?*blocked-oddagons*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont oddagon[7] ?zzz $?z-cands))
   )
)
...
(defrule apply-oddagon[7]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont oddagon[7] ?zzz $?z-cands)
...
(defrule oddagon[7]-print-z-candidates
