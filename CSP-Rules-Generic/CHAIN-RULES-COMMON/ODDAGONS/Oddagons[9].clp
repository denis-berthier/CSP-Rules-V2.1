...
=>
   ...
   (if (or ?*print-actions* ?*print-L9* ?*print-oddagon* ?*print-oddagon-9*) then
      (print-oddagon-without-crlf 9 ?zzz (create$ $?rlcs ?cand1) (create$ $?csp-vars ?new-csp))
   )
   ...
   (if (not ?*blocked-oddagons*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont oddagon[9] ?zzz $?z-cands))
   )
)
...
(defrule apply-oddagon[9]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont oddagon[9] ?zzz $?z-cands)
...
(defrule oddagon[9]-print-z-candidates
