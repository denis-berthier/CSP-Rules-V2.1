...
(defrule activate-OR4-forcing-whip[14]
   (declare (salience ?*activate-OR4-forcing-whip[14]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4FW14))
   (assert (technique ?cont OR4-forcing-whip[14]))
   (assert (technique ?cont partial-whip[13]))
   (bind ?*technique* OR4FW[14])
)



(defrule track-OR4-forcing-whip[14]
   (declare (salience ?*activate-OR4-forcing-whip[14]-salience*))
   ?level <- (technique ?cont OR4-forcing-whip[14])
=>
...
(defrule OR4-forcing-whip[14]-candidate-000p
   (declare (salience ?*OR4-forcing-whip[14]-candidate-salience*))
   (technique ?cont OR4-forcing-whip[14])
...
=>
   (retract ?ret)
   (if (or ?*print-actions* ?*print-L14* ?*print-OR4-forcing-whip* ?*print-OR4-forcing-whip-14*) then
...
(defrule OR4-forcing-whip[14]-candidate-00pq
   (declare (salience ?*OR4-forcing-whip[14]-candidate-salience*))
   (technique ?cont OR4-forcing-whip[14])
...
(defrule OR4-forcing-whip[14]-candidate-0pqr
   (declare (salience ?*OR4-forcing-whip[14]-candidate-salience*))
   (technique ?cont OR4-forcing-whip[14])
...
(defrule OR4-forcing-whip[14]-candidate-pqrs
   (declare (salience ?*OR4-forcing-whip[14]-candidate-salience*))
   (technique ?cont OR4-forcing-whip[14])
   (OR-k-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 4) (OR-candidates ?zzz1 ?zzz2 ?zzz3 ?zzz4))
   (chain
   ...
   (chain
      (type partial-whip)
      (context ?cont)
      (length ?p2&:(or (< ?p1 ?p2) (and (= ?p1 ?p2) (< ?zzz1 ?zzz2)))&:(< ?p2 12))
      (target ?zzz2)
      (llcs $?llcs2)
      (rlcs $?rlcs2)
      (csp-vars $?csps2)
...
(defrule OR4-forcing-whip[14]-value
   (declare (salience ?*OR4-forcing-whip[14]-value-salience*))
   (technique ?cont OR4-forcing-whip[14])
   (OR-k-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 4) (OR-candidates ?zzz1 ?zzz2 ?zzz3 ?zzz4))
   (chain
      (type partial-whip)
