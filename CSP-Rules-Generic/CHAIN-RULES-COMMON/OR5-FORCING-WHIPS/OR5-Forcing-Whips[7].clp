...
(defrule activate-OR5-forcing-whip[7]
   (declare (salience ?*activate-OR5-forcing-whip[7]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5FW7))
   (assert (technique ?cont OR5-forcing-whip[7]))
   (assert (technique ?cont partial-whip[6]))
   (bind ?*technique* OR5FW[7])
)



(defrule track-OR5-forcing-whip[7]
   (declare (salience ?*activate-OR5-forcing-whip[7]-salience*))
   ?level <- (technique ?cont OR5-forcing-whip[7])
=>
...
(defrule OR5-forcing-whip[7]-candidate-0000p
   (declare (salience ?*OR5-forcing-whip[7]-candidate-salience*))
   (technique ?cont OR5-forcing-whip[7])
...
=>
   (retract ?ret)
   (if (or ?*print-actions* ?*print-L7* ?*print-OR5-forcing-whip* ?*print-OR5-forcing-whip-7*) then
...
(defrule OR5-forcing-whip[7]-candidate-000pq
   (declare (salience ?*OR5-forcing-whip[7]-candidate-salience*))
   (technique ?cont OR5-forcing-whip[7])
...
(defrule OR5-forcing-whip[7]-candidate-00pqr
   (declare (salience ?*OR5-forcing-whip[7]-candidate-salience*))
   (technique ?cont OR5-forcing-whip[7])
...
(defrule OR5-forcing-whip[7]-candidate-0pqrs
   (declare (salience ?*OR5-forcing-whip[7]-candidate-salience*))
   (technique ?cont OR5-forcing-whip[7])
...
(defrule OR5-forcing-whip[7]-candidate-pqrst
   (declare (salience ?*OR5-forcing-whip[7]-candidate-salience*))
   (technique ?cont OR5-forcing-whip[7])
   (OR-k-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 5) (OR-candidates ?zzz1 ?zzz2 ?zzz3 ?zzz4 ?zzz5))
   (chain
...
(defrule OR5-forcing-whip[7]-value
   (declare (salience ?*OR5-forcing-whip[7]-value-salience*))
   (technique ?cont OR5-forcing-whip[7])
   (OR-k-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 5) (OR-candidates ?zzz1 ?zzz2 ?zzz3 ?zzz4 ?zzz5))
   (chain
      (type partial-whip)
   ...
   (chain
      (type partial-whip)
      (context ?cont)
      (length ?p2&:(or (< ?p1 ?p2) (and (= ?p1 ?p2) (< ?zzz1 ?zzz2)))&:(< ?p2 4))
      (target ?zzz2)
      (llcs $?llcs2)
      (rlcs $?rlcs2)
      (csp-vars $?csps2)
