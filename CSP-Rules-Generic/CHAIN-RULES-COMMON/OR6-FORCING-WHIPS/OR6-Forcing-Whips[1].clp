...
(defrule activate-OR6-forcing-whip[1]
   (declare (salience ?*activate-OR6-forcing-whip[1]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6FW1))
   (assert (technique ?cont OR6-forcing-whip[1]))
   (assert (technique ?cont partial-whip[0]))
   (bind ?*technique* OR6FW[1])
)



(defrule track-OR6-forcing-whip[1]
   (declare (salience ?*activate-OR6-forcing-whip[1]-salience*))
   ?level <- (technique ?cont OR6-forcing-whip[1])
=>
