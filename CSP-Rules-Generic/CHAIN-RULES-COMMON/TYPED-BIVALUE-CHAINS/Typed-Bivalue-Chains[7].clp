...
(defrule activate-typed-bivalue-chain[7]
   (declare (salience ?*activate-typed-bivalue-chain[7]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_TyBC7))
   (assert (technique ?cont typed-bivalue-chain[7]))
   (bind ?*technique* TyBC[7])
)



(defrule track-typed-bivalue-chain[7]
   (declare (salience ?*activate-typed-bivalue-chain[7]-salience*))
   ?level <- (technique ?cont typed-bivalue-chain[7])
=>
...
(defrule typed-partial-bivalue-chain[7]
   (declare (salience ?*typed-partial-bivalue-chain[7]-salience*))
   (logical
      (typed-chain
      ...
      (technique ?cont typed-bivalue-chain[7])
      ;;; ?new-llc
...
(defrule typed-bivalue-chain[7]
   (declare (salience ?*typed-bivalue-chain[7]-salience*))
   (typed-chain
      (type typed-bivalue-chain)
      (csp-type ?csp-type)
      (context ?cont)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L7* ?*print-typed-bivalue-chain* ?*print-typed-bivalue-chain-7*) then
      (print-typed-bivalue-chain-without-crlf ?csp-type 7 ?zzz $?llcs $?rlcs $?csp-vars)
   )
   (if (not ?*blocked-bivalue-chains*)
      then (printout t crlf)
      else ; prepare for finding more targets
         (assert (apply-rule-as-a-pseudo-block ?cont))
         (assert (pseudo-blocked ?cont typed-bivalue-chain[7] ?csp-type 7 ?zzz $?llcs $?rlcs $?csp-vars))
   )
)



(defrule apply-typed-bivalue-chain-to-more-targets[7]
   ...
   (pseudo-blocked ?cont typed-bivalue-chain[7] ?csp-type 7 ?zzz $?llcs $?rlcs $?csp-vars)
