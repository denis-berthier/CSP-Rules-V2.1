...
(defrule activate-typed-bivalue-chain[8]
   (declare (salience ?*activate-typed-bivalue-chain[8]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_TyBC8))
   (assert (technique ?cont typed-bivalue-chain[8]))
   (bind ?*technique* TyBC[8])
)



(defrule track-typed-bivalue-chain[8]
   (declare (salience ?*activate-typed-bivalue-chain[8]-salience*))
   ?level <- (technique ?cont typed-bivalue-chain[8])
=>
...
(defrule typed-partial-bivalue-chain[8]
   (declare (salience ?*typed-partial-bivalue-chain[8]-salience*))
   (logical
      (typed-chain
...
(defrule typed-bivalue-chain[8]
   (declare (salience ?*typed-bivalue-chain[8]-salience*))
   (typed-chain
      (type typed-bivalue-chain)
      (csp-type ?csp-type)
      (context ?cont)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L8* ?*print-typed-bivalue-chain* ?*print-typed-bivalue-chain-8*) then
      (print-typed-bivalue-chain-without-crlf ?csp-type 8 ?zzz $?llcs $?rlcs $?csp-vars)
   )
   (if (not ?*blocked-bivalue-chains*)
      then (printout t crlf)
      else ; prepare for finding more targets
         (assert (apply-rule-as-a-pseudo-block ?cont))
         (assert (pseudo-blocked ?cont typed-bivalue-chain[8] ?csp-type 8 ?zzz $?llcs $?rlcs $?csp-vars))
   )
)



(defrule apply-typed-bivalue-chain-to-more-targets[8]
   ...
   (pseudo-blocked ?cont typed-bivalue-chain[8] ?csp-type 8 ?zzz $?llcs $?rlcs $?csp-vars)
