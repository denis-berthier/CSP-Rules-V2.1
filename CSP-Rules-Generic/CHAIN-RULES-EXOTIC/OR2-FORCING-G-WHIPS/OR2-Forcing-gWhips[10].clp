...
(defrule activate-OR2-forcing-gwhip[10]
   (declare (salience ?*activate-OR2-forcing-gwhip[10]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2FW10))
   (assert (technique ?cont OR2-forcing-gwhip[10]))
   (assert (technique ?cont partial-whip[9]))
   (assert (technique ?cont partial-gwhip[9]))
   (bind ?*technique* OR2FgW[10])
)



(defrule track-OR2-forcing-gwhip[10]
   (declare (salience ?*activate-OR2-forcing-gwhip[10]-salience*))
   ?level <- (technique ?cont OR2-forcing-gwhip[10])
=>
...
(defrule OR2-forcing-gwhip[10]-candidate-0p
   (declare (salience ?*OR2-forcing-gwhip[10]-candidate-salience*))
   (technique ?cont OR2-forcing-gwhip[10])
...
=>
   ...
   (if (or ?*print-actions* ?*print-L10* ?*print-OR2-forcing-gwhip* ?*print-OR2-forcing-gwhip-10*) then
...
(defrule OR2-forcing-gwhip[10]-candidate-pq
   (declare (salience ?*OR2-forcing-gwhip[10]-candidate-salience*))
   (technique ?cont OR2-forcing-gwhip[10])
   (ORk-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 2) (OR-candidates ?zzz1 ?zzz2))
   (chain
...
(defrule OR2-forcing-gwhip[10]-value
   (declare (salience ?*OR2-forcing-gwhip[10]-value-salience*))
   (technique ?cont OR2-forcing-gwhip[10])
   (ORk-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 2) (OR-candidates ?zzz1 ?zzz2))
   (chain
      (type ?type1&:(or (eq ?type1 partial-whip) (eq ?type1 partial-gwhip)))
