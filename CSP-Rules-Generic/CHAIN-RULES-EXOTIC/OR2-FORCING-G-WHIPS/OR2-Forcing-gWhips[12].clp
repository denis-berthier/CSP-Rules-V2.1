...
(defrule activate-OR2-forcing-gwhip[12]
   (declare (salience ?*activate-OR2-forcing-gwhip[12]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2FW12))
   (assert (technique ?cont OR2-forcing-gwhip[12]))
   (assert (technique ?cont partial-whip[11]))
   (assert (technique ?cont partial-gwhip[11]))
   (bind ?*technique* OR2FgW[12])
)



(defrule track-OR2-forcing-gwhip[12]
   (declare (salience ?*activate-OR2-forcing-gwhip[12]-salience*))
   ?level <- (technique ?cont OR2-forcing-gwhip[12])
=>
...
(defrule OR2-forcing-gwhip[12]-candidate-0p
   (declare (salience ?*OR2-forcing-gwhip[12]-candidate-salience*))
   (technique ?cont OR2-forcing-gwhip[12])
...
=>
   ...
   (if (or ?*print-actions* ?*print-L12* ?*print-OR2-forcing-gwhip* ?*print-OR2-forcing-gwhip-12*) then
...
(defrule OR2-forcing-gwhip[12]-candidate-pq
   (declare (salience ?*OR2-forcing-gwhip[12]-candidate-salience*))
   (technique ?cont OR2-forcing-gwhip[12])
   (ORk-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 2) (OR-candidates ?zzz1 ?zzz2))
   (chain
...
(defrule OR2-forcing-gwhip[12]-value
   (declare (salience ?*OR2-forcing-gwhip[12]-value-salience*))
   (technique ?cont OR2-forcing-gwhip[12])
   (ORk-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 2) (OR-candidates ?zzz1 ?zzz2))
   (chain
      (type ?type1&:(or (eq ?type1 partial-whip) (eq ?type1 partial-gwhip)))
