...
(defrule activate-OR2-forcing-gwhip[16]
   (declare (salience ?*activate-OR2-forcing-gwhip[16]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2FW16))
   (assert (technique ?cont OR2-forcing-gwhip[16]))
   (assert (technique ?cont partial-whip[15]))
   (assert (technique ?cont partial-gwhip[15]))
   (bind ?*technique* OR2FgW[16])
)



(defrule track-OR2-forcing-gwhip[16]
   (declare (salience ?*activate-OR2-forcing-gwhip[16]-salience*))
   ?level <- (technique ?cont OR2-forcing-gwhip[16])
=>
...
(defrule OR2-forcing-gwhip[16]-candidate-0p
   (declare (salience ?*OR2-forcing-gwhip[16]-candidate-salience*))
   (technique ?cont OR2-forcing-gwhip[16])
...
=>
   ...
   (if (or ?*print-actions* ?*print-L16* ?*print-OR2-forcing-gwhip* ?*print-OR2-forcing-gwhip-16*) then
...
(defrule OR2-forcing-gwhip[16]-candidate-pq
   (declare (salience ?*OR2-forcing-gwhip[16]-candidate-salience*))
   (technique ?cont OR2-forcing-gwhip[16])
   (ORk-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 2) (OR-candidates ?zzz1 ?zzz2))
   (chain
...
(defrule OR2-forcing-gwhip[16]-value
   (declare (salience ?*OR2-forcing-gwhip[16]-value-salience*))
   (technique ?cont OR2-forcing-gwhip[16])
   (ORk-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 2) (OR-candidates ?zzz1 ?zzz2))
   (chain
      (type ?type1&:(or (eq ?type1 partial-whip) (eq ?type1 partial-gwhip)))
