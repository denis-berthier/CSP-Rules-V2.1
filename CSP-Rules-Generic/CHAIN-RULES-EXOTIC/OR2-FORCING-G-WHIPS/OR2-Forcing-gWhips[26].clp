...
(defrule activate-OR2-forcing-gwhip[26]
   (declare (salience ?*activate-OR2-forcing-gwhip[26]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2FW26))
   (assert (technique ?cont OR2-forcing-gwhip[26]))
   (assert (technique ?cont partial-whip[25]))
   (assert (technique ?cont partial-gwhip[25]))
   (bind ?*technique* OR2FgW[26])
)



(defrule track-OR2-forcing-gwhip[26]
   (declare (salience ?*activate-OR2-forcing-gwhip[26]-salience*))
   ?level <- (technique ?cont OR2-forcing-gwhip[26])
=>
...
(defrule OR2-forcing-gwhip[26]-candidate-0p
   (declare (salience ?*OR2-forcing-gwhip[26]-candidate-salience*))
   (technique ?cont OR2-forcing-gwhip[26])
...
=>
   ...
   (if (or ?*print-actions* ?*print-L26* ?*print-OR2-forcing-gwhip* ?*print-OR2-forcing-gwhip-26*) then
...
(defrule OR2-forcing-gwhip[26]-candidate-pq
   (declare (salience ?*OR2-forcing-gwhip[26]-candidate-salience*))
   (technique ?cont OR2-forcing-gwhip[26])
   (ORk-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 2) (OR-candidates ?zzz1 ?zzz2))
   (chain
...
(defrule OR2-forcing-gwhip[26]-value
   (declare (salience ?*OR2-forcing-gwhip[26]-value-salience*))
   (technique ?cont OR2-forcing-gwhip[26])
   (ORk-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 2) (OR-candidates ?zzz1 ?zzz2))
   (chain
      (type ?type1&:(or (eq ?type1 partial-whip) (eq ?type1 partial-gwhip)))
