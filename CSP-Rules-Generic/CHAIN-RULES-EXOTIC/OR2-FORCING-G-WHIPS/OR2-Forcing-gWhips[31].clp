...
(defrule activate-OR2-forcing-gwhip[31]
   (declare (salience ?*activate-OR2-forcing-gwhip[31]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2FW31))
   (assert (technique ?cont OR2-forcing-gwhip[31]))
   (assert (technique ?cont partial-whip[30]))
   (assert (technique ?cont partial-gwhip[30]))
   (bind ?*technique* OR2FgW[31])
)



(defrule track-OR2-forcing-gwhip[31]
   (declare (salience ?*activate-OR2-forcing-gwhip[31]-salience*))
   ?level <- (technique ?cont OR2-forcing-gwhip[31])
=>
...
(defrule OR2-forcing-gwhip[31]-candidate-0p
   (declare (salience ?*OR2-forcing-gwhip[31]-candidate-salience*))
   (technique ?cont OR2-forcing-gwhip[31])
...
=>
   (retract ?ret)
   (if (or ?*print-actions* ?*print-L31* ?*print-OR2-forcing-gwhip* ?*print-OR2-forcing-gwhip-31*) then
...
(defrule OR2-forcing-gwhip[31]-candidate-pq
   (declare (salience ?*OR2-forcing-gwhip[31]-candidate-salience*))
   (technique ?cont OR2-forcing-gwhip[31])
   (ORk-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 2) (OR-candidates ?zzz1 ?zzz2))
   (chain
...
(defrule OR2-forcing-gwhip[31]-value
   (declare (salience ?*OR2-forcing-gwhip[31]-value-salience*))
   (technique ?cont OR2-forcing-gwhip[31])
   (ORk-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 2) (OR-candidates ?zzz1 ?zzz2))
   (chain
      (type ?type1&:(or (eq ?type1 partial-whip) (eq ?type1 partial-gwhip)))
