...
(defrule activate-OR2-forcing-gwhip[32]
   (declare (salience ?*activate-OR2-forcing-gwhip[32]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2FW32))
   (assert (technique ?cont OR2-forcing-gwhip[32]))
   (assert (technique ?cont partial-whip[31]))
   (assert (technique ?cont partial-gwhip[31]))
   (bind ?*technique* OR2FgW[32])
)



(defrule track-OR2-forcing-gwhip[32]
   (declare (salience ?*activate-OR2-forcing-gwhip[32]-salience*))
   ?level <- (technique ?cont OR2-forcing-gwhip[32])
=>
...
(defrule OR2-forcing-gwhip[32]-candidate-0p
   (declare (salience ?*OR2-forcing-gwhip[32]-candidate-salience*))
   (technique ?cont OR2-forcing-gwhip[32])
...
=>
   (retract ?ret)
   (if (or ?*print-actions* ?*print-L32* ?*print-OR2-forcing-gwhip* ?*print-OR2-forcing-gwhip-32*) then
...
(defrule OR2-forcing-gwhip[32]-candidate-pq
   (declare (salience ?*OR2-forcing-gwhip[32]-candidate-salience*))
   (technique ?cont OR2-forcing-gwhip[32])
   (ORk-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 2) (OR-candidates ?zzz1 ?zzz2))
   (chain
...
(defrule OR2-forcing-gwhip[32]-value
   (declare (salience ?*OR2-forcing-gwhip[32]-value-salience*))
   (technique ?cont OR2-forcing-gwhip[32])
   (ORk-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 2) (OR-candidates ?zzz1 ?zzz2))
   (chain
      (type ?type1&:(or (eq ?type1 partial-whip) (eq ?type1 partial-gwhip)))
