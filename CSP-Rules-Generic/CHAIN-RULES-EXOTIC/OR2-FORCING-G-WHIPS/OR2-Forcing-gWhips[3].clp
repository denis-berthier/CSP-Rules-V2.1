...
(defrule activate-OR2-forcing-gwhip[3]
   (declare (salience ?*activate-OR2-forcing-gwhip[3]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2FW3))
   (assert (technique ?cont OR2-forcing-gwhip[3]))
   (assert (technique ?cont partial-whip[2]))
   (assert (technique ?cont partial-gwhip[2]))
   (bind ?*technique* OR2FgW[3])
)



(defrule track-OR2-forcing-gwhip[3]
   (declare (salience ?*activate-OR2-forcing-gwhip[3]-salience*))
   ?level <- (technique ?cont OR2-forcing-gwhip[3])
=>
...
(defrule OR2-forcing-gwhip[3]-candidate-0p
   (declare (salience ?*OR2-forcing-gwhip[3]-candidate-salience*))
   (technique ?cont OR2-forcing-gwhip[3])
...
=>
   (retract ?ret)
   (if (or ?*print-actions* ?*print-L3* ?*print-OR2-forcing-gwhip* ?*print-OR2-forcing-gwhip-3*) then
...
(defrule OR2-forcing-gwhip[3]-candidate-pq
   (declare (salience ?*OR2-forcing-gwhip[3]-candidate-salience*))
   (technique ?cont OR2-forcing-gwhip[3])
   (ORk-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 2) (OR-candidates ?zzz1 ?zzz2))
   (chain
   ...
   (chain
      ...
      (context ?cont)
      (length ?p2&:(or (< ?p1 ?p2) (and (= ?p1 ?p2) (< ?zzz1 ?zzz2)))&:(= (+ ?p1 ?p2) 2))
      (target ?zzz2)
      (llcs $?llcs2)
      (rlcs $?rlcs2)
      (csp-vars $?csps2)
...
(defrule OR2-forcing-gwhip[3]-value
   (declare (salience ?*OR2-forcing-gwhip[3]-value-salience*))
   (technique ?cont OR2-forcing-gwhip[3])
   (ORk-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 2) (OR-candidates ?zzz1 ?zzz2))
   (chain
      (type ?type1&:(or (eq ?type1 partial-whip) (eq ?type1 partial-gwhip)))
