...
(defrule activate-OR2-gwhip[10]
   (declare (salience ?*activate-OR2-gwhip[10]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW10))
   (assert (technique ?cont OR2-gwhip[10]))
   (assert (technique ?cont partial-whip[9]))
   (assert (technique ?cont partial-gwhip[9]))
   (assert (technique ?cont partial-OR2-gwhip[9]))
   (bind ?*technique* OR2gW[10])
)



(defrule track-OR2-gwhip[10]
   (declare (salience ?*activate-OR2-gwhip[10]-salience*))
   ?level <- (technique ?cont OR2-gwhip[10])
=>
...
(defrule OR2-gwhip[10]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[10]-salience-1*))
   (ORk-chain
...
(defrule OR2-gwhip[10]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[10]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 9)
