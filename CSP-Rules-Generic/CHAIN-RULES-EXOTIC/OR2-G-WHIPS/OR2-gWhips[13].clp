...
(defrule activate-OR2-gwhip[13]
   (declare (salience ?*activate-OR2-gwhip[13]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW13))
   (assert (technique ?cont OR2-gwhip[13]))
   (assert (technique ?cont partial-whip[12]))
   (assert (technique ?cont partial-gwhip[12]))
   (assert (technique ?cont partial-OR2-gwhip[12]))
   (bind ?*technique* OR2gW[13])
)



(defrule track-OR2-gwhip[13]
   (declare (salience ?*activate-OR2-gwhip[13]-salience*))
   ?level <- (technique ?cont OR2-gwhip[13])
=>
...
(defrule OR2-gwhip[13]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[13]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L13* ?*print-OR2-gwhip* ?*print-OR2-gwhip-13*) then
...
(defrule OR2-gwhip[13]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[13]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 12)
