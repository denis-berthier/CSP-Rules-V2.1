...
(defrule activate-OR2-gwhip[16]
   (declare (salience ?*activate-OR2-gwhip[16]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW16))
   (assert (technique ?cont OR2-gwhip[16]))
   (assert (technique ?cont partial-whip[15]))
   (assert (technique ?cont partial-gwhip[15]))
   (assert (technique ?cont partial-OR2-gwhip[15]))
   (bind ?*technique* OR2gW[16])
)



(defrule track-OR2-gwhip[16]
   (declare (salience ?*activate-OR2-gwhip[16]-salience*))
   ?level <- (technique ?cont OR2-gwhip[16])
=>
...
(defrule OR2-gwhip[16]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[16]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L16* ?*print-OR2-gwhip* ?*print-OR2-gwhip-16*) then
...
(defrule OR2-gwhip[16]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[16]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 15)
