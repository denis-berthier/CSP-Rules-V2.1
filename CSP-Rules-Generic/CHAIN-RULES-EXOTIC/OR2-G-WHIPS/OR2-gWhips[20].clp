...
(defrule activate-OR2-gwhip[20]
   (declare (salience ?*activate-OR2-gwhip[20]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW20))
   (assert (technique ?cont OR2-gwhip[20]))
   (assert (technique ?cont partial-whip[19]))
   (assert (technique ?cont partial-gwhip[19]))
   (assert (technique ?cont partial-OR2-gwhip[19]))
   (bind ?*technique* OR2gW[20])
)



(defrule track-OR2-gwhip[20]
   (declare (salience ?*activate-OR2-gwhip[20]-salience*))
   ?level <- (technique ?cont OR2-gwhip[20])
=>
...
(defrule OR2-gwhip[20]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[20]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L20* ?*print-OR2-gwhip* ?*print-OR2-gwhip-20*) then
...
(defrule OR2-gwhip[20]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[20]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 19)
