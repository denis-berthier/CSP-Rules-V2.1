...
(defrule activate-OR2-gwhip[21]
   (declare (salience ?*activate-OR2-gwhip[21]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW21))
   (assert (technique ?cont OR2-gwhip[21]))
   (assert (technique ?cont partial-whip[20]))
   (assert (technique ?cont partial-gwhip[20]))
   (assert (technique ?cont partial-OR2-gwhip[20]))
   (bind ?*technique* OR2gW[21])
)



(defrule track-OR2-gwhip[21]
   (declare (salience ?*activate-OR2-gwhip[21]-salience*))
   ?level <- (technique ?cont OR2-gwhip[21])
=>
...
(defrule OR2-gwhip[21]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[21]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L21* ?*print-OR2-gwhip* ?*print-OR2-gwhip-21*) then
...
(defrule OR2-gwhip[21]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[21]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 20)
