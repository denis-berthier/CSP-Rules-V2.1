...
(defrule activate-OR2-gwhip[22]
   (declare (salience ?*activate-OR2-gwhip[22]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW22))
   (assert (technique ?cont OR2-gwhip[22]))
   (assert (technique ?cont partial-whip[21]))
   (assert (technique ?cont partial-gwhip[21]))
   (assert (technique ?cont partial-OR2-gwhip[21]))
   (bind ?*technique* OR2gW[22])
)



(defrule track-OR2-gwhip[22]
   (declare (salience ?*activate-OR2-gwhip[22]-salience*))
   ?level <- (technique ?cont OR2-gwhip[22])
=>
...
(defrule OR2-gwhip[22]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[22]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L22* ?*print-OR2-gwhip* ?*print-OR2-gwhip-22*) then
...
(defrule OR2-gwhip[22]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[22]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 21)
