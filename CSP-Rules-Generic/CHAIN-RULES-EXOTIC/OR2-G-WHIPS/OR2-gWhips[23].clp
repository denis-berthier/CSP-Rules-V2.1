...
(defrule activate-OR2-gwhip[23]
   (declare (salience ?*activate-OR2-gwhip[23]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW23))
   (assert (technique ?cont OR2-gwhip[23]))
   (assert (technique ?cont partial-whip[22]))
   (assert (technique ?cont partial-gwhip[22]))
   (assert (technique ?cont partial-OR2-gwhip[22]))
   (bind ?*technique* OR2gW[23])
)



(defrule track-OR2-gwhip[23]
   (declare (salience ?*activate-OR2-gwhip[23]-salience*))
   ?level <- (technique ?cont OR2-gwhip[23])
=>
...
(defrule OR2-gwhip[23]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[23]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L23* ?*print-OR2-gwhip* ?*print-OR2-gwhip-23*) then
...
(defrule OR2-gwhip[23]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[23]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 22)
