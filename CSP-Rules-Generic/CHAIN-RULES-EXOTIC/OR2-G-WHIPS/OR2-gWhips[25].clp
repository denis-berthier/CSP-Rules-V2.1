...
(defrule activate-OR2-gwhip[25]
   (declare (salience ?*activate-OR2-gwhip[25]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW25))
   (assert (technique ?cont OR2-gwhip[25]))
   (assert (technique ?cont partial-whip[24]))
   (assert (technique ?cont partial-gwhip[24]))
   (assert (technique ?cont partial-OR2-gwhip[24]))
   (bind ?*technique* OR2gW[25])
)



(defrule track-OR2-gwhip[25]
   (declare (salience ?*activate-OR2-gwhip[25]-salience*))
   ?level <- (technique ?cont OR2-gwhip[25])
=>
...
(defrule OR2-gwhip[25]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[25]-salience-1*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 24)
      (target ?zzz)
      (length1 ?len1&:(= ?len1 23))
      (llcs1 $?llcs1)
      (rlcs1 $?rlcs1)
      (csp-vars1 $?csp-vars1)
      (last-rlc1 ?last-rlc1)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L25* ?*print-OR2-gwhip* ?*print-OR2-gwhip-25*) then
...
(defrule OR2-gwhip[25]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[25]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 24)
