...
(defrule activate-OR2-gwhip[26]
   (declare (salience ?*activate-OR2-gwhip[26]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW26))
   (assert (technique ?cont OR2-gwhip[26]))
   (assert (technique ?cont partial-whip[25]))
   (assert (technique ?cont partial-gwhip[25]))
   (assert (technique ?cont partial-OR2-gwhip[25]))
   (bind ?*technique* OR2gW[26])
)



(defrule track-OR2-gwhip[26]
   (declare (salience ?*activate-OR2-gwhip[26]-salience*))
   ?level <- (technique ?cont OR2-gwhip[26])
=>
...
(defrule OR2-gwhip[26]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[26]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L26* ?*print-OR2-gwhip* ?*print-OR2-gwhip-26*) then
...
(defrule OR2-gwhip[26]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[26]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 25)
