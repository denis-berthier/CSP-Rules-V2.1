...
(defrule activate-OR2-gwhip[29]
   (declare (salience ?*activate-OR2-gwhip[29]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW29))
   (assert (technique ?cont OR2-gwhip[29]))
   (assert (technique ?cont partial-whip[28]))
   (assert (technique ?cont partial-gwhip[28]))
   (assert (technique ?cont partial-OR2-gwhip[28]))
   (bind ?*technique* OR2gW[29])
)



(defrule track-OR2-gwhip[29]
   (declare (salience ?*activate-OR2-gwhip[29]-salience*))
   ?level <- (technique ?cont OR2-gwhip[29])
=>
...
(defrule OR2-gwhip[29]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[29]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L29* ?*print-OR2-gwhip* ?*print-OR2-gwhip-29*) then
...
(defrule OR2-gwhip[29]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[29]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 28)
