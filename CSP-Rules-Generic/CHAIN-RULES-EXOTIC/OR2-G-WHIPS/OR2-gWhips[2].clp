...
(defrule activate-OR2-gwhip[2]
   (declare (salience ?*activate-OR2-gwhip[2]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW2))
   (assert (technique ?cont OR2-gwhip[2]))
   (assert (technique ?cont partial-whip[1]))
   (assert (technique ?cont partial-gwhip[1]))
   (assert (technique ?cont partial-OR2-gwhip[1]))
   (bind ?*technique* OR2gW[2])
)



(defrule track-OR2-gwhip[2]
   (declare (salience ?*activate-OR2-gwhip[2]-salience*))
   ?level <- (technique ?cont OR2-gwhip[2])
=>
...
(defrule OR2-gwhip[2]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[2]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L2* ?*print-OR2-gwhip* ?*print-OR2-gwhip-2*) then
