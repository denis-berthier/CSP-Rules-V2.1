...
(defrule activate-OR2-gwhip[31]
   (declare (salience ?*activate-OR2-gwhip[31]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW31))
   (assert (technique ?cont OR2-gwhip[31]))
   (assert (technique ?cont partial-whip[30]))
   (assert (technique ?cont partial-gwhip[30]))
   (assert (technique ?cont partial-OR2-gwhip[30]))
   (bind ?*technique* OR2gW[31])
)



(defrule track-OR2-gwhip[31]
   (declare (salience ?*activate-OR2-gwhip[31]-salience*))
   ?level <- (technique ?cont OR2-gwhip[31])
=>
...
(defrule OR2-gwhip[31]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[31]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L31* ?*print-OR2-gwhip* ?*print-OR2-gwhip-31*) then
...
(defrule OR2-gwhip[31]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[31]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 30)
