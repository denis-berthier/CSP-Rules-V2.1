...
(defrule activate-OR2-gwhip[32]
   (declare (salience ?*activate-OR2-gwhip[32]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW32))
   (assert (technique ?cont OR2-gwhip[32]))
   (assert (technique ?cont partial-whip[31]))
   (assert (technique ?cont partial-gwhip[31]))
   (assert (technique ?cont partial-OR2-gwhip[31]))
   (bind ?*technique* OR2gW[32])
)



(defrule track-OR2-gwhip[32]
   (declare (salience ?*activate-OR2-gwhip[32]-salience*))
   ?level <- (technique ?cont OR2-gwhip[32])
=>
...
(defrule OR2-gwhip[32]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[32]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L32* ?*print-OR2-gwhip* ?*print-OR2-gwhip-32*) then
...
(defrule OR2-gwhip[32]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[32]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 31)
