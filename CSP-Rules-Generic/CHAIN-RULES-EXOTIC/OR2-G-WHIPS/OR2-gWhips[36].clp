...
(defrule activate-OR2-gwhip[36]
   (declare (salience ?*activate-OR2-gwhip[36]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW36))
   (assert (technique ?cont OR2-gwhip[36]))
   (assert (technique ?cont partial-whip[35]))
   (assert (technique ?cont partial-gwhip[35]))
   (assert (technique ?cont partial-OR2-gwhip[35]))
   (bind ?*technique* OR2gW[36])
)



(defrule track-OR2-gwhip[36]
   (declare (salience ?*activate-OR2-gwhip[36]-salience*))
   ?level <- (technique ?cont OR2-gwhip[36])
=>
...
(defrule OR2-gwhip[36]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[36]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L36* ?*print-OR2-gwhip* ?*print-OR2-gwhip-36*) then
...
(defrule OR2-gwhip[36]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[36]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 35)
