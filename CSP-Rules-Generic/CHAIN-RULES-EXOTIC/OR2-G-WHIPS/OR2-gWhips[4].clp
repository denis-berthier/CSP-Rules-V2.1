...
(defrule activate-OR2-gwhip[4]
   (declare (salience ?*activate-OR2-gwhip[4]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW4))
   (assert (technique ?cont OR2-gwhip[4]))
   (assert (technique ?cont partial-whip[3]))
   (assert (technique ?cont partial-gwhip[3]))
   (assert (technique ?cont partial-OR2-gwhip[3]))
   (bind ?*technique* OR2gW[4])
)



(defrule track-OR2-gwhip[4]
   (declare (salience ?*activate-OR2-gwhip[4]-salience*))
   ?level <- (technique ?cont OR2-gwhip[4])
=>
...
(defrule OR2-gwhip[4]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[4]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L4* ?*print-OR2-gwhip* ?*print-OR2-gwhip-4*) then
...
(defrule OR2-gwhip[4]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[4]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 3)
