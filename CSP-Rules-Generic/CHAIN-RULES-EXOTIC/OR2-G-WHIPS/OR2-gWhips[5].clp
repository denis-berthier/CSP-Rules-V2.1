...
(defrule activate-OR2-gwhip[5]
   (declare (salience ?*activate-OR2-gwhip[5]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW5))
   (assert (technique ?cont OR2-gwhip[5]))
   (assert (technique ?cont partial-whip[4]))
   (assert (technique ?cont partial-gwhip[4]))
   (assert (technique ?cont partial-OR2-gwhip[4]))
   (bind ?*technique* OR2gW[5])
)



(defrule track-OR2-gwhip[5]
   (declare (salience ?*activate-OR2-gwhip[5]-salience*))
   ?level <- (technique ?cont OR2-gwhip[5])
=>
...
(defrule OR2-gwhip[5]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[5]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L5* ?*print-OR2-gwhip* ?*print-OR2-gwhip-5*) then
...
(defrule OR2-gwhip[5]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[5]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 4)
