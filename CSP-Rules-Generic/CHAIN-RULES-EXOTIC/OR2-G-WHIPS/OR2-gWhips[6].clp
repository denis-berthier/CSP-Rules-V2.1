...
(defrule activate-OR2-gwhip[6]
   (declare (salience ?*activate-OR2-gwhip[6]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW6))
   (assert (technique ?cont OR2-gwhip[6]))
   (assert (technique ?cont partial-whip[5]))
   (assert (technique ?cont partial-gwhip[5]))
   (assert (technique ?cont partial-OR2-gwhip[5]))
   (bind ?*technique* OR2gW[6])
)



(defrule track-OR2-gwhip[6]
   (declare (salience ?*activate-OR2-gwhip[6]-salience*))
   ?level <- (technique ?cont OR2-gwhip[6])
=>
...
(defrule OR2-gwhip[6]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[6]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L6* ?*print-OR2-gwhip* ?*print-OR2-gwhip-6*) then
...
(defrule OR2-gwhip[6]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[6]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 5)
