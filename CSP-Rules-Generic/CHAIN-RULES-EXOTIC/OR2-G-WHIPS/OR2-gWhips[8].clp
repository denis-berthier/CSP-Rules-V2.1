...
(defrule activate-OR2-gwhip[8]
   (declare (salience ?*activate-OR2-gwhip[8]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW8))
   (assert (technique ?cont OR2-gwhip[8]))
   (assert (technique ?cont partial-whip[7]))
   (assert (technique ?cont partial-gwhip[7]))
   (assert (technique ?cont partial-OR2-gwhip[7]))
   (bind ?*technique* OR2gW[8])
)



(defrule track-OR2-gwhip[8]
   (declare (salience ?*activate-OR2-gwhip[8]-salience*))
   ?level <- (technique ?cont OR2-gwhip[8])
=>
...
(defrule OR2-gwhip[8]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[8]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L8* ?*print-OR2-gwhip* ?*print-OR2-gwhip-8*) then
...
(defrule OR2-gwhip[8]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[8]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 7)
