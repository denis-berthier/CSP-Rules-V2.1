...
(defrule activate-OR2-gwhip[9]
   (declare (salience ?*activate-OR2-gwhip[9]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR2gW9))
   (assert (technique ?cont OR2-gwhip[9]))
   (assert (technique ?cont partial-whip[8]))
   (assert (technique ?cont partial-gwhip[8]))
   (assert (technique ?cont partial-OR2-gwhip[8]))
   (bind ?*technique* OR2gW[9])
)



(defrule track-OR2-gwhip[9]
   (declare (salience ?*activate-OR2-gwhip[9]-salience*))
   ?level <- (technique ?cont OR2-gwhip[9])
=>
...
(defrule OR2-gwhip[9]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR2-gwhip[9]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L9* ?*print-OR2-gwhip* ?*print-OR2-gwhip-9*) then
...
(defrule OR2-gwhip[9]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR2-gwhip[9]-salience-2*))
   (ORk-chain
      (type partial-OR2-gwhip)
      (context ?cont)
      (length 8)
