...
(defrule activate-OR3-forcing-gwhip[4]
   (declare (salience ?*activate-OR3-forcing-gwhip[4]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3FW4))
   (assert (technique ?cont OR3-forcing-gwhip[4]))
   (assert (technique ?cont partial-whip[3]))
   (assert (technique ?cont partial-gwhip[3]))
   (bind ?*technique* OR3FgW[4])
)



(defrule track-OR3-forcing-gwhip[4]
   (declare (salience ?*activate-OR3-forcing-gwhip[4]-salience*))
   ?level <- (technique ?cont OR3-forcing-gwhip[4])
=>
...
(defrule OR3-forcing-gwhip[4]-candidate-00p
   (declare (salience ?*OR3-forcing-gwhip[4]-candidate-salience*))
   (technique ?cont OR3-forcing-gwhip[4])
...
=>
   (retract ?ret)
   (if (or ?*print-actions* ?*print-L4* ?*print-OR3-forcing-gwhip* ?*print-OR3-forcing-gwhip-4*) then
...
(defrule OR3-forcing-gwhip[4]-candidate-0pq
   (declare (salience ?*OR3-forcing-gwhip[4]-candidate-salience*))
   (technique ?cont OR3-forcing-gwhip[4])
...
(defrule OR3-forcing-gwhip[4]-candidate-pqr
   (declare (salience ?*OR3-forcing-gwhip[4]-candidate-salience*))
   (technique ?cont OR3-forcing-gwhip[4])
   (ORk-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 3) (OR-candidates ?zzz1 ?zzz2 ?zzz3))
   (chain
   ...
   (chain
      ...
      (context ?cont)
      (length ?p3&:(or (< ?p2 ?p3) (and (= ?p2 ?p3) (< ?zzz2 ?zzz3)))&:(= (+ ?p1 ?p2 ?p3) 3))
      (target ?zzz3)
      (llcs $?llcs3)
      (rlcs $?rlcs3)
      (csp-vars $?csps3)
...
(defrule OR3-forcing-gwhip[4]-value
   (declare (salience ?*OR3-forcing-gwhip[4]-value-salience*))
   (technique ?cont OR3-forcing-gwhip[4])
   (ORk-relation (OR-name ?or-name) (OR-complexity ?or-compl) (context ?cont) (OR-size 3) (OR-candidates ?zzz1 ?zzz2 ?zzz3))
   (chain
      (type ?type1&:(or (eq ?type1 partial-whip) (eq ?type1 partial-gwhip)))
