...
(defrule activate-OR3-gwhip[12]
   (declare (salience ?*activate-OR3-gwhip[12]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW12))
   (assert (technique ?cont OR3-gwhip[12]))
   (assert (technique ?cont partial-whip[11]))
   (assert (technique ?cont partial-gwhip[11]))
   (assert (technique ?cont partial-OR3-gwhip[11]))
   (bind ?*technique* OR3gW[12])
)



(defrule track-OR3-gwhip[12]
   (declare (salience ?*activate-OR3-gwhip[12]-salience*))
   ?level <- (technique ?cont OR3-gwhip[12])
=>
...
(defrule OR3-gwhip[12]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[12]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L12* ?*print-OR3-gwhip* ?*print-OR3-gwhip-12*) then
...
(defrule OR3-gwhip[12]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[12]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 11)
