...
(defrule activate-OR3-gwhip[13]
   (declare (salience ?*activate-OR3-gwhip[13]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW13))
   (assert (technique ?cont OR3-gwhip[13]))
   (assert (technique ?cont partial-whip[12]))
   (assert (technique ?cont partial-gwhip[12]))
   (assert (technique ?cont partial-OR3-gwhip[12]))
   (bind ?*technique* OR3gW[13])
)



(defrule track-OR3-gwhip[13]
   (declare (salience ?*activate-OR3-gwhip[13]-salience*))
   ?level <- (technique ?cont OR3-gwhip[13])
=>
...
(defrule OR3-gwhip[13]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[13]-salience-1*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 12)
      (target ?zzz)
      (length1 ?len1&:(= ?len1 11))
      (llcs1 $?llcs1)
      (rlcs1 $?rlcs1)
      (csp-vars1 $?csp-vars1)
      (last-rlc1 ?last-rlc1)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L13* ?*print-OR3-gwhip* ?*print-OR3-gwhip-13*) then
...
(defrule OR3-gwhip[13]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[13]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 12)
      (target ?zzz)
      (length1 ?len1&:(<= ?len1 10))
      (llcs1 $?llcs1)
      (rlcs1 $?rlcs1)
      (csp-vars1 $?csp-vars1)
      (last-rlc1 ?last-rlc1)
