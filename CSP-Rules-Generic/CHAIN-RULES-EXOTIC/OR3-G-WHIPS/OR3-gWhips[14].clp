...
(defrule activate-OR3-gwhip[14]
   (declare (salience ?*activate-OR3-gwhip[14]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW14))
   (assert (technique ?cont OR3-gwhip[14]))
   (assert (technique ?cont partial-whip[13]))
   (assert (technique ?cont partial-gwhip[13]))
   (assert (technique ?cont partial-OR3-gwhip[13]))
   (bind ?*technique* OR3gW[14])
)



(defrule track-OR3-gwhip[14]
   (declare (salience ?*activate-OR3-gwhip[14]-salience*))
   ?level <- (technique ?cont OR3-gwhip[14])
=>
...
(defrule OR3-gwhip[14]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[14]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L14* ?*print-OR3-gwhip* ?*print-OR3-gwhip-14*) then
...
(defrule OR3-gwhip[14]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[14]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 13)
