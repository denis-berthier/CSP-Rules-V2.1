...
(defrule activate-OR3-gwhip[15]
   (declare (salience ?*activate-OR3-gwhip[15]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW15))
   (assert (technique ?cont OR3-gwhip[15]))
   (assert (technique ?cont partial-whip[14]))
   (assert (technique ?cont partial-gwhip[14]))
   (assert (technique ?cont partial-OR3-gwhip[14]))
   (bind ?*technique* OR3gW[15])
)



(defrule track-OR3-gwhip[15]
   (declare (salience ?*activate-OR3-gwhip[15]-salience*))
   ?level <- (technique ?cont OR3-gwhip[15])
=>
...
(defrule OR3-gwhip[15]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[15]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L15* ?*print-OR3-gwhip* ?*print-OR3-gwhip-15*) then
...
(defrule OR3-gwhip[15]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[15]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 14)
