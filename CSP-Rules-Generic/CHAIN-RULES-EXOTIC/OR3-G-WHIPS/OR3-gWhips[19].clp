...
(defrule activate-OR3-gwhip[19]
   (declare (salience ?*activate-OR3-gwhip[19]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW19))
   (assert (technique ?cont OR3-gwhip[19]))
   (assert (technique ?cont partial-whip[18]))
   (assert (technique ?cont partial-gwhip[18]))
   (assert (technique ?cont partial-OR3-gwhip[18]))
   (bind ?*technique* OR3gW[19])
)



(defrule track-OR3-gwhip[19]
   (declare (salience ?*activate-OR3-gwhip[19]-salience*))
   ?level <- (technique ?cont OR3-gwhip[19])
=>
...
(defrule OR3-gwhip[19]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[19]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L19* ?*print-OR3-gwhip* ?*print-OR3-gwhip-19*) then
...
(defrule OR3-gwhip[19]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[19]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 18)
