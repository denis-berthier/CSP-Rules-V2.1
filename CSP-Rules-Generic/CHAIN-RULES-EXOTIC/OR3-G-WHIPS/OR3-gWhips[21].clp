...
(defrule activate-OR3-gwhip[21]
   (declare (salience ?*activate-OR3-gwhip[21]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW21))
   (assert (technique ?cont OR3-gwhip[21]))
   (assert (technique ?cont partial-whip[20]))
   (assert (technique ?cont partial-gwhip[20]))
   (assert (technique ?cont partial-OR3-gwhip[20]))
   (bind ?*technique* OR3gW[21])
)



(defrule track-OR3-gwhip[21]
   (declare (salience ?*activate-OR3-gwhip[21]-salience*))
   ?level <- (technique ?cont OR3-gwhip[21])
=>
...
(defrule OR3-gwhip[21]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[21]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L21* ?*print-OR3-gwhip* ?*print-OR3-gwhip-21*) then
...
(defrule OR3-gwhip[21]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[21]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 20)
