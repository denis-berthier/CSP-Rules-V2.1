...
(defrule activate-OR3-gwhip[22]
   (declare (salience ?*activate-OR3-gwhip[22]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW22))
   (assert (technique ?cont OR3-gwhip[22]))
   (assert (technique ?cont partial-whip[21]))
   (assert (technique ?cont partial-gwhip[21]))
   (assert (technique ?cont partial-OR3-gwhip[21]))
   (bind ?*technique* OR3gW[22])
)



(defrule track-OR3-gwhip[22]
   (declare (salience ?*activate-OR3-gwhip[22]-salience*))
   ?level <- (technique ?cont OR3-gwhip[22])
=>
...
(defrule OR3-gwhip[22]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[22]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L22* ?*print-OR3-gwhip* ?*print-OR3-gwhip-22*) then
...
(defrule OR3-gwhip[22]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[22]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 21)
