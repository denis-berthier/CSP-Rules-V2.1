...
(defrule activate-OR3-gwhip[23]
   (declare (salience ?*activate-OR3-gwhip[23]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW23))
   (assert (technique ?cont OR3-gwhip[23]))
   (assert (technique ?cont partial-whip[22]))
   (assert (technique ?cont partial-gwhip[22]))
   (assert (technique ?cont partial-OR3-gwhip[22]))
   (bind ?*technique* OR3gW[23])
)



(defrule track-OR3-gwhip[23]
   (declare (salience ?*activate-OR3-gwhip[23]-salience*))
   ?level <- (technique ?cont OR3-gwhip[23])
=>
...
(defrule OR3-gwhip[23]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[23]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L23* ?*print-OR3-gwhip* ?*print-OR3-gwhip-23*) then
...
(defrule OR3-gwhip[23]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[23]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 22)
