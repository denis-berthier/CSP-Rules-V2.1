...
(defrule activate-OR3-gwhip[24]
   (declare (salience ?*activate-OR3-gwhip[24]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW24))
   (assert (technique ?cont OR3-gwhip[24]))
   (assert (technique ?cont partial-whip[23]))
   (assert (technique ?cont partial-gwhip[23]))
   (assert (technique ?cont partial-OR3-gwhip[23]))
   (bind ?*technique* OR3gW[24])
)



(defrule track-OR3-gwhip[24]
   (declare (salience ?*activate-OR3-gwhip[24]-salience*))
   ?level <- (technique ?cont OR3-gwhip[24])
=>
...
(defrule OR3-gwhip[24]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[24]-salience-1*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 23)
      (target ?zzz)
      (length1 ?len1&:(= ?len1 22))
      (llcs1 $?llcs1)
      (rlcs1 $?rlcs1)
      (csp-vars1 $?csp-vars1)
      (last-rlc1 ?last-rlc1)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L24* ?*print-OR3-gwhip* ?*print-OR3-gwhip-24*) then
...
(defrule OR3-gwhip[24]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[24]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 23)
      (target ?zzz)
      (length1 ?len1&:(<= ?len1 21))
      (llcs1 $?llcs1)
      (rlcs1 $?rlcs1)
      (csp-vars1 $?csp-vars1)
      (last-rlc1 ?last-rlc1)
