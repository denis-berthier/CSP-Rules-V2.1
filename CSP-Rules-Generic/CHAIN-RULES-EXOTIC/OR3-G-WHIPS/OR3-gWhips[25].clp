...
(defrule activate-OR3-gwhip[25]
   (declare (salience ?*activate-OR3-gwhip[25]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW25))
   (assert (technique ?cont OR3-gwhip[25]))
   (assert (technique ?cont partial-whip[24]))
   (assert (technique ?cont partial-gwhip[24]))
   (assert (technique ?cont partial-OR3-gwhip[24]))
   (bind ?*technique* OR3gW[25])
)



(defrule track-OR3-gwhip[25]
   (declare (salience ?*activate-OR3-gwhip[25]-salience*))
   ?level <- (technique ?cont OR3-gwhip[25])
=>
...
(defrule OR3-gwhip[25]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[25]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L25* ?*print-OR3-gwhip* ?*print-OR3-gwhip-25*) then
...
(defrule OR3-gwhip[25]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[25]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 24)
