...
(defrule activate-OR3-gwhip[26]
   (declare (salience ?*activate-OR3-gwhip[26]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW26))
   (assert (technique ?cont OR3-gwhip[26]))
   (assert (technique ?cont partial-whip[25]))
   (assert (technique ?cont partial-gwhip[25]))
   (assert (technique ?cont partial-OR3-gwhip[25]))
   (bind ?*technique* OR3gW[26])
)



(defrule track-OR3-gwhip[26]
   (declare (salience ?*activate-OR3-gwhip[26]-salience*))
   ?level <- (technique ?cont OR3-gwhip[26])
=>
...
(defrule OR3-gwhip[26]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[26]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L26* ?*print-OR3-gwhip* ?*print-OR3-gwhip-26*) then
...
(defrule OR3-gwhip[26]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[26]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 25)
