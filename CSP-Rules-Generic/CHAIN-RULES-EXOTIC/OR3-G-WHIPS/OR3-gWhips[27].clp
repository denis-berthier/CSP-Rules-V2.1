...
(defrule activate-OR3-gwhip[27]
   (declare (salience ?*activate-OR3-gwhip[27]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW27))
   (assert (technique ?cont OR3-gwhip[27]))
   (assert (technique ?cont partial-whip[26]))
   (assert (technique ?cont partial-gwhip[26]))
   (assert (technique ?cont partial-OR3-gwhip[26]))
   (bind ?*technique* OR3gW[27])
)



(defrule track-OR3-gwhip[27]
   (declare (salience ?*activate-OR3-gwhip[27]-salience*))
   ?level <- (technique ?cont OR3-gwhip[27])
=>
...
(defrule OR3-gwhip[27]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[27]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L27* ?*print-OR3-gwhip* ?*print-OR3-gwhip-27*) then
...
(defrule OR3-gwhip[27]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[27]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 26)
