...
(defrule activate-OR3-gwhip[28]
   (declare (salience ?*activate-OR3-gwhip[28]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW28))
   (assert (technique ?cont OR3-gwhip[28]))
   (assert (technique ?cont partial-whip[27]))
   (assert (technique ?cont partial-gwhip[27]))
   (assert (technique ?cont partial-OR3-gwhip[27]))
   (bind ?*technique* OR3gW[28])
)



(defrule track-OR3-gwhip[28]
   (declare (salience ?*activate-OR3-gwhip[28]-salience*))
   ?level <- (technique ?cont OR3-gwhip[28])
=>
...
(defrule OR3-gwhip[28]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[28]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L28* ?*print-OR3-gwhip* ?*print-OR3-gwhip-28*) then
...
(defrule OR3-gwhip[28]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[28]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 27)
