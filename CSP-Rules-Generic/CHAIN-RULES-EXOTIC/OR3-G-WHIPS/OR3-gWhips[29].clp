...
(defrule activate-OR3-gwhip[29]
   (declare (salience ?*activate-OR3-gwhip[29]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW29))
   (assert (technique ?cont OR3-gwhip[29]))
   (assert (technique ?cont partial-whip[28]))
   (assert (technique ?cont partial-gwhip[28]))
   (assert (technique ?cont partial-OR3-gwhip[28]))
   (bind ?*technique* OR3gW[29])
)



(defrule track-OR3-gwhip[29]
   (declare (salience ?*activate-OR3-gwhip[29]-salience*))
   ?level <- (technique ?cont OR3-gwhip[29])
=>
...
(defrule OR3-gwhip[29]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[29]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L29* ?*print-OR3-gwhip* ?*print-OR3-gwhip-29*) then
...
(defrule OR3-gwhip[29]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[29]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 28)
