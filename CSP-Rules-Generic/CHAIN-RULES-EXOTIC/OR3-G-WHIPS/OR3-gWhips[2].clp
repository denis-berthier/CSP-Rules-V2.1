...
(defrule activate-OR3-gwhip[2]
   (declare (salience ?*activate-OR3-gwhip[2]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW2))
   (assert (technique ?cont OR3-gwhip[2]))
   (assert (technique ?cont partial-whip[1]))
   (assert (technique ?cont partial-gwhip[1]))
   (assert (technique ?cont partial-OR3-gwhip[1]))
   (bind ?*technique* OR3gW[2])
)



(defrule track-OR3-gwhip[2]
   (declare (salience ?*activate-OR3-gwhip[2]-salience*))
   ?level <- (technique ?cont OR3-gwhip[2])
=>
...
(defrule OR3-gwhip[2]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[2]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L2* ?*print-OR3-gwhip* ?*print-OR3-gwhip-2*) then
