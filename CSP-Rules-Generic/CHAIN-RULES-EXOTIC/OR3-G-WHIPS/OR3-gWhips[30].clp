...
(defrule activate-OR3-gwhip[30]
   (declare (salience ?*activate-OR3-gwhip[30]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW30))
   (assert (technique ?cont OR3-gwhip[30]))
   (assert (technique ?cont partial-whip[29]))
   (assert (technique ?cont partial-gwhip[29]))
   (assert (technique ?cont partial-OR3-gwhip[29]))
   (bind ?*technique* OR3gW[30])
)



(defrule track-OR3-gwhip[30]
   (declare (salience ?*activate-OR3-gwhip[30]-salience*))
   ?level <- (technique ?cont OR3-gwhip[30])
=>
...
(defrule OR3-gwhip[30]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[30]-salience-1*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 29)
      (target ?zzz)
      (length1 ?len1&:(= ?len1 28))
      (llcs1 $?llcs1)
      (rlcs1 $?rlcs1)
      (csp-vars1 $?csp-vars1)
      (last-rlc1 ?last-rlc1)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L30* ?*print-OR3-gwhip* ?*print-OR3-gwhip-30*) then
...
(defrule OR3-gwhip[30]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[30]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 29)
