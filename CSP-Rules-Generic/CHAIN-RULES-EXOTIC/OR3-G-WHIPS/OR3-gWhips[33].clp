...
(defrule activate-OR3-gwhip[33]
   (declare (salience ?*activate-OR3-gwhip[33]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW33))
   (assert (technique ?cont OR3-gwhip[33]))
   (assert (technique ?cont partial-whip[32]))
   (assert (technique ?cont partial-gwhip[32]))
   (assert (technique ?cont partial-OR3-gwhip[32]))
   (bind ?*technique* OR3gW[33])
)



(defrule track-OR3-gwhip[33]
   (declare (salience ?*activate-OR3-gwhip[33]-salience*))
   ?level <- (technique ?cont OR3-gwhip[33])
=>
...
(defrule OR3-gwhip[33]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[33]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L33* ?*print-OR3-gwhip* ?*print-OR3-gwhip-33*) then
...
(defrule OR3-gwhip[33]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[33]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 32)
