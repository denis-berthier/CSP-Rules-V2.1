...
(defrule activate-OR3-gwhip[34]
   (declare (salience ?*activate-OR3-gwhip[34]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW34))
   (assert (technique ?cont OR3-gwhip[34]))
   (assert (technique ?cont partial-whip[33]))
   (assert (technique ?cont partial-gwhip[33]))
   (assert (technique ?cont partial-OR3-gwhip[33]))
   (bind ?*technique* OR3gW[34])
)



(defrule track-OR3-gwhip[34]
   (declare (salience ?*activate-OR3-gwhip[34]-salience*))
   ?level <- (technique ?cont OR3-gwhip[34])
=>
...
(defrule OR3-gwhip[34]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[34]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L34* ?*print-OR3-gwhip* ?*print-OR3-gwhip-34*) then
...
(defrule OR3-gwhip[34]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[34]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 33)
