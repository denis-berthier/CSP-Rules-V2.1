...
(defrule activate-OR3-gwhip[35]
   (declare (salience ?*activate-OR3-gwhip[35]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW35))
   (assert (technique ?cont OR3-gwhip[35]))
   (assert (technique ?cont partial-whip[34]))
   (assert (technique ?cont partial-gwhip[34]))
   (assert (technique ?cont partial-OR3-gwhip[34]))
   (bind ?*technique* OR3gW[35])
)



(defrule track-OR3-gwhip[35]
   (declare (salience ?*activate-OR3-gwhip[35]-salience*))
   ?level <- (technique ?cont OR3-gwhip[35])
=>
...
(defrule OR3-gwhip[35]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[35]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L35* ?*print-OR3-gwhip* ?*print-OR3-gwhip-35*) then
...
(defrule OR3-gwhip[35]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[35]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 34)
      (target ?zzz)
      (length1 ?len1&:(<= ?len1 32))
      (llcs1 $?llcs1)
      (rlcs1 $?rlcs1)
      (csp-vars1 $?csp-vars1)
      (last-rlc1 ?last-rlc1)
