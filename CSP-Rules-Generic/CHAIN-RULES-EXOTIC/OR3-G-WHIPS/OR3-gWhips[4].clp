...
(defrule activate-OR3-gwhip[4]
   (declare (salience ?*activate-OR3-gwhip[4]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW4))
   (assert (technique ?cont OR3-gwhip[4]))
   (assert (technique ?cont partial-whip[3]))
   (assert (technique ?cont partial-gwhip[3]))
   (assert (technique ?cont partial-OR3-gwhip[3]))
   (bind ?*technique* OR3gW[4])
)



(defrule track-OR3-gwhip[4]
   (declare (salience ?*activate-OR3-gwhip[4]-salience*))
   ?level <- (technique ?cont OR3-gwhip[4])
=>
...
(defrule OR3-gwhip[4]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[4]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L4* ?*print-OR3-gwhip* ?*print-OR3-gwhip-4*) then
...
(defrule OR3-gwhip[4]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[4]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 3)
