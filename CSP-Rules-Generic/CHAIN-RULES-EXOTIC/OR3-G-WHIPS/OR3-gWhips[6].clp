...
(defrule activate-OR3-gwhip[6]
   (declare (salience ?*activate-OR3-gwhip[6]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW6))
   (assert (technique ?cont OR3-gwhip[6]))
   (assert (technique ?cont partial-whip[5]))
   (assert (technique ?cont partial-gwhip[5]))
   (assert (technique ?cont partial-OR3-gwhip[5]))
   (bind ?*technique* OR3gW[6])
)



(defrule track-OR3-gwhip[6]
   (declare (salience ?*activate-OR3-gwhip[6]-salience*))
   ?level <- (technique ?cont OR3-gwhip[6])
=>
...
(defrule OR3-gwhip[6]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[6]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L6* ?*print-OR3-gwhip* ?*print-OR3-gwhip-6*) then
...
(defrule OR3-gwhip[6]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[6]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 5)
