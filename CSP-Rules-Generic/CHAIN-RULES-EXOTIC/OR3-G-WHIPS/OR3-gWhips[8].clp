...
(defrule activate-OR3-gwhip[8]
   (declare (salience ?*activate-OR3-gwhip[8]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3gW8))
   (assert (technique ?cont OR3-gwhip[8]))
   (assert (technique ?cont partial-whip[7]))
   (assert (technique ?cont partial-gwhip[7]))
   (assert (technique ?cont partial-OR3-gwhip[7]))
   (bind ?*technique* OR3gW[8])
)



(defrule track-OR3-gwhip[8]
   (declare (salience ?*activate-OR3-gwhip[8]-salience*))
   ?level <- (technique ?cont OR3-gwhip[8])
=>
...
(defrule OR3-gwhip[8]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-gwhip[8]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L8* ?*print-OR3-gwhip* ?*print-OR3-gwhip-8*) then
...
(defrule OR3-gwhip[8]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-gwhip[8]-salience-2*))
   (ORk-chain
      (type partial-OR3-gwhip)
      (context ?cont)
      (length 7)
