...
(defrule activate-OR3-whip[12]
   (declare (salience ?*activate-OR3-whip[12]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W12))
   (assert (technique ?cont OR3-whip[12]))
   (assert (technique ?cont partial-whip[11]))
   (assert (technique ?cont partial-OR3-whip[11]))
   (bind ?*technique* OR3W[12])
)



(defrule track-OR3-whip[12]
   (declare (salience ?*activate-OR3-whip[12]-salience*))
   ?level <- (technique ?cont OR3-whip[12])
=>
...
(defrule OR3-whip[12]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[12]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L12* ?*print-OR3-whip* ?*print-OR3-whip-12*) then
...
(defrule OR3-whip[12]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-whip[12]-salience-2*))
   (ORk-chain
      (type partial-OR3-whip)
      (context ?cont)
      (length 11)
