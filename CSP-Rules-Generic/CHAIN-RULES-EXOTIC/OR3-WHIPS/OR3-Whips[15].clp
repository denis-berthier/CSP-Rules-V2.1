...
(defrule activate-OR3-whip[15]
   (declare (salience ?*activate-OR3-whip[15]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W15))
   (assert (technique ?cont OR3-whip[15]))
   (assert (technique ?cont partial-whip[14]))
   (assert (technique ?cont partial-OR3-whip[14]))
   (bind ?*technique* OR3W[15])
)



(defrule track-OR3-whip[15]
   (declare (salience ?*activate-OR3-whip[15]-salience*))
   ?level <- (technique ?cont OR3-whip[15])
=>
...
(defrule OR3-whip[15]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[15]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L15* ?*print-OR3-whip* ?*print-OR3-whip-15*) then
...
(defrule OR3-whip[15]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-whip[15]-salience-2*))
   (ORk-chain
      (type partial-OR3-whip)
      (context ?cont)
      (length 14)
