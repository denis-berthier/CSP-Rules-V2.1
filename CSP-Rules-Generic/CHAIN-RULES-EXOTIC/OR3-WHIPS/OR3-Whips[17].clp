...
(defrule activate-OR3-whip[17]
   (declare (salience ?*activate-OR3-whip[17]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W17))
   (assert (technique ?cont OR3-whip[17]))
   (assert (technique ?cont partial-whip[16]))
   (assert (technique ?cont partial-OR3-whip[16]))
   (bind ?*technique* OR3W[17])
)



(defrule track-OR3-whip[17]
   (declare (salience ?*activate-OR3-whip[17]-salience*))
   ?level <- (technique ?cont OR3-whip[17])
=>
...
(defrule OR3-whip[17]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[17]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L17* ?*print-OR3-whip* ?*print-OR3-whip-17*) then
...
(defrule OR3-whip[17]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-whip[17]-salience-2*))
   (ORk-chain
      (type partial-OR3-whip)
      (context ?cont)
      (length 16)
