...
(defrule activate-OR3-whip[18]
   (declare (salience ?*activate-OR3-whip[18]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W18))
   (assert (technique ?cont OR3-whip[18]))
   (assert (technique ?cont partial-whip[17]))
   (assert (technique ?cont partial-OR3-whip[17]))
   (bind ?*technique* OR3W[18])
)



(defrule track-OR3-whip[18]
   (declare (salience ?*activate-OR3-whip[18]-salience*))
   ?level <- (technique ?cont OR3-whip[18])
=>
...
(defrule OR3-whip[18]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[18]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L18* ?*print-OR3-whip* ?*print-OR3-whip-18*) then
...
(defrule OR3-whip[18]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-whip[18]-salience-2*))
   (ORk-chain
      (type partial-OR3-whip)
      (context ?cont)
      (length 17)
