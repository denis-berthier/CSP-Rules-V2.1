...
(defrule activate-OR3-whip[19]
   (declare (salience ?*activate-OR3-whip[19]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W19))
   (assert (technique ?cont OR3-whip[19]))
   (assert (technique ?cont partial-whip[18]))
   (assert (technique ?cont partial-OR3-whip[18]))
   (bind ?*technique* OR3W[19])
)



(defrule track-OR3-whip[19]
   (declare (salience ?*activate-OR3-whip[19]-salience*))
   ?level <- (technique ?cont OR3-whip[19])
=>
...
(defrule OR3-whip[19]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[19]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L19* ?*print-OR3-whip* ?*print-OR3-whip-19*) then
...
(defrule OR3-whip[19]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-whip[19]-salience-2*))
   (ORk-chain
      (type partial-OR3-whip)
      (context ?cont)
      (length 18)
