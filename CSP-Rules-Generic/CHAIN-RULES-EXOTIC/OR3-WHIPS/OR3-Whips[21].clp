...
(defrule activate-OR3-whip[21]
   (declare (salience ?*activate-OR3-whip[21]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W21))
   (assert (technique ?cont OR3-whip[21]))
   (assert (technique ?cont partial-whip[20]))
   (assert (technique ?cont partial-OR3-whip[20]))
   (bind ?*technique* OR3W[21])
)



(defrule track-OR3-whip[21]
   (declare (salience ?*activate-OR3-whip[21]-salience*))
   ?level <- (technique ?cont OR3-whip[21])
=>
...
(defrule OR3-whip[21]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[21]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L21* ?*print-OR3-whip* ?*print-OR3-whip-21*) then
...
(defrule OR3-whip[21]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-whip[21]-salience-2*))
   (ORk-chain
      (type partial-OR3-whip)
      (context ?cont)
      (length 20)
