...
(defrule activate-OR3-whip[22]
   (declare (salience ?*activate-OR3-whip[22]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W22))
   (assert (technique ?cont OR3-whip[22]))
   (assert (technique ?cont partial-whip[21]))
   (assert (technique ?cont partial-OR3-whip[21]))
   (bind ?*technique* OR3W[22])
)



(defrule track-OR3-whip[22]
   (declare (salience ?*activate-OR3-whip[22]-salience*))
   ?level <- (technique ?cont OR3-whip[22])
=>
...
(defrule OR3-whip[22]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[22]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L22* ?*print-OR3-whip* ?*print-OR3-whip-22*) then
...
(defrule OR3-whip[22]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-whip[22]-salience-2*))
   (ORk-chain
      (type partial-OR3-whip)
      (context ?cont)
      (length 21)
