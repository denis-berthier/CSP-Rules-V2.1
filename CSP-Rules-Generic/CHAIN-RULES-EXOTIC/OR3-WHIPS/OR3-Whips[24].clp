...
(defrule activate-OR3-whip[24]
   (declare (salience ?*activate-OR3-whip[24]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W24))
   (assert (technique ?cont OR3-whip[24]))
   (assert (technique ?cont partial-whip[23]))
   (assert (technique ?cont partial-OR3-whip[23]))
   (bind ?*technique* OR3W[24])
)



(defrule track-OR3-whip[24]
   (declare (salience ?*activate-OR3-whip[24]-salience*))
   ?level <- (technique ?cont OR3-whip[24])
=>
...
(defrule OR3-whip[24]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[24]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L24* ?*print-OR3-whip* ?*print-OR3-whip-24*) then
...
(defrule OR3-whip[24]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-whip[24]-salience-2*))
   (ORk-chain
      (type partial-OR3-whip)
      (context ?cont)
      (length 23)
