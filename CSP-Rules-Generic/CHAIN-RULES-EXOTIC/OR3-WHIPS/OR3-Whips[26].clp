...
(defrule activate-OR3-whip[26]
   (declare (salience ?*activate-OR3-whip[26]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W26))
   (assert (technique ?cont OR3-whip[26]))
   (assert (technique ?cont partial-whip[25]))
   (assert (technique ?cont partial-OR3-whip[25]))
   (bind ?*technique* OR3W[26])
)



(defrule track-OR3-whip[26]
   (declare (salience ?*activate-OR3-whip[26]-salience*))
   ?level <- (technique ?cont OR3-whip[26])
=>
...
(defrule OR3-whip[26]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[26]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L26* ?*print-OR3-whip* ?*print-OR3-whip-26*) then
...
(defrule OR3-whip[26]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-whip[26]-salience-2*))
   (ORk-chain
      (type partial-OR3-whip)
      (context ?cont)
      (length 25)
