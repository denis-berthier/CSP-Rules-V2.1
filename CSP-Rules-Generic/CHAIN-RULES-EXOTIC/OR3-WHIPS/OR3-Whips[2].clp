...
(defrule activate-OR3-whip[2]
   (declare (salience ?*activate-OR3-whip[2]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W2))
   (assert (technique ?cont OR3-whip[2]))
   (assert (technique ?cont partial-whip[1]))
   (assert (technique ?cont partial-OR3-whip[1]))
   (bind ?*technique* OR3W[2])
)



(defrule track-OR3-whip[2]
   (declare (salience ?*activate-OR3-whip[2]-salience*))
   ?level <- (technique ?cont OR3-whip[2])
=>
...
(defrule OR3-whip[2]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[2]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L2* ?*print-OR3-whip* ?*print-OR3-whip-2*) then
