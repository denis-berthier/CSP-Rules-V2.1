...
(defrule activate-OR3-whip[33]
   (declare (salience ?*activate-OR3-whip[33]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W33))
   (assert (technique ?cont OR3-whip[33]))
   (assert (technique ?cont partial-whip[32]))
   (assert (technique ?cont partial-OR3-whip[32]))
   (bind ?*technique* OR3W[33])
)



(defrule track-OR3-whip[33]
   (declare (salience ?*activate-OR3-whip[33]-salience*))
   ?level <- (technique ?cont OR3-whip[33])
=>
...
(defrule OR3-whip[33]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[33]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L33* ?*print-OR3-whip* ?*print-OR3-whip-33*) then
...
(defrule OR3-whip[33]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-whip[33]-salience-2*))
   (ORk-chain
      (type partial-OR3-whip)
      (context ?cont)
      (length 32)
