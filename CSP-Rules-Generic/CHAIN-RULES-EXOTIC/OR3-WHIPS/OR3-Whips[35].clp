...
(defrule activate-OR3-whip[35]
   (declare (salience ?*activate-OR3-whip[35]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W35))
   (assert (technique ?cont OR3-whip[35]))
   (assert (technique ?cont partial-whip[34]))
   (assert (technique ?cont partial-OR3-whip[34]))
   (bind ?*technique* OR3W[35])
)



(defrule track-OR3-whip[35]
   (declare (salience ?*activate-OR3-whip[35]-salience*))
   ?level <- (technique ?cont OR3-whip[35])
=>
...
(defrule OR3-whip[35]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[35]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L35* ?*print-OR3-whip* ?*print-OR3-whip-35*) then
...
(defrule OR3-whip[35]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-whip[35]-salience-2*))
   (ORk-chain
      (type partial-OR3-whip)
      (context ?cont)
      (length 34)
