...
(defrule activate-OR3-whip[3]
   (declare (salience ?*activate-OR3-whip[3]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W3))
   (assert (technique ?cont OR3-whip[3]))
   (assert (technique ?cont partial-whip[2]))
   (assert (technique ?cont partial-OR3-whip[2]))
   (bind ?*technique* OR3W[3])
)



(defrule track-OR3-whip[3]
   (declare (salience ?*activate-OR3-whip[3]-salience*))
   ?level <- (technique ?cont OR3-whip[3])
=>
...
(defrule OR3-whip[3]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[3]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L3* ?*print-OR3-whip* ?*print-OR3-whip-3*) then
...
(defrule OR3-whip[3]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-whip[3]-salience-2*))
   (ORk-chain
      (type partial-OR3-whip)
      (context ?cont)
      (length 2)
