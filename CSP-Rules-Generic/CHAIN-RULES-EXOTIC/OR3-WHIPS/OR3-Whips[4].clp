...
(defrule activate-OR3-whip[4]
   (declare (salience ?*activate-OR3-whip[4]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W4))
   (assert (technique ?cont OR3-whip[4]))
   (assert (technique ?cont partial-whip[3]))
   (assert (technique ?cont partial-OR3-whip[3]))
   (bind ?*technique* OR3W[4])
)



(defrule track-OR3-whip[4]
   (declare (salience ?*activate-OR3-whip[4]-salience*))
   ?level <- (technique ?cont OR3-whip[4])
=>
...
(defrule OR3-whip[4]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[4]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L4* ?*print-OR3-whip* ?*print-OR3-whip-4*) then
...
(defrule OR3-whip[4]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-whip[4]-salience-2*))
   (ORk-chain
      (type partial-OR3-whip)
      (context ?cont)
      (length 3)
