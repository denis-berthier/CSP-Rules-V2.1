...
(defrule activate-OR3-whip[6]
   (declare (salience ?*activate-OR3-whip[6]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W6))
   (assert (technique ?cont OR3-whip[6]))
   (assert (technique ?cont partial-whip[5]))
   (assert (technique ?cont partial-OR3-whip[5]))
   (bind ?*technique* OR3W[6])
)



(defrule track-OR3-whip[6]
   (declare (salience ?*activate-OR3-whip[6]-salience*))
   ?level <- (technique ?cont OR3-whip[6])
=>
...
(defrule OR3-whip[6]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[6]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L6* ?*print-OR3-whip* ?*print-OR3-whip-6*) then
...
(defrule OR3-whip[6]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-whip[6]-salience-2*))
   (ORk-chain
      (type partial-OR3-whip)
      (context ?cont)
      (length 5)
