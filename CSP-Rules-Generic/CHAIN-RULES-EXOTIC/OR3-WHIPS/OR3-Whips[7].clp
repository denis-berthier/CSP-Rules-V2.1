...
(defrule activate-OR3-whip[7]
   (declare (salience ?*activate-OR3-whip[7]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W7))
   (assert (technique ?cont OR3-whip[7]))
   (assert (technique ?cont partial-whip[6]))
   (assert (technique ?cont partial-OR3-whip[6]))
   (bind ?*technique* OR3W[7])
)



(defrule track-OR3-whip[7]
   (declare (salience ?*activate-OR3-whip[7]-salience*))
   ?level <- (technique ?cont OR3-whip[7])
=>
...
(defrule OR3-whip[7]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[7]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L7* ?*print-OR3-whip* ?*print-OR3-whip-7*) then
...
(defrule OR3-whip[7]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-whip[7]-salience-2*))
   (ORk-chain
      (type partial-OR3-whip)
      (context ?cont)
      (length 6)
