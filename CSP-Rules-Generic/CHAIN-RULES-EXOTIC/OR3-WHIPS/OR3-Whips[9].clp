...
(defrule activate-OR3-whip[9]
   (declare (salience ?*activate-OR3-whip[9]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR3W9))
   (assert (technique ?cont OR3-whip[9]))
   (assert (technique ?cont partial-whip[8]))
   (assert (technique ?cont partial-OR3-whip[8]))
   (bind ?*technique* OR3W[9])
)



(defrule track-OR3-whip[9]
   (declare (salience ?*activate-OR3-whip[9]-salience*))
   ?level <- (technique ?cont OR3-whip[9])
=>
...
(defrule OR3-whip[9]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR3-whip[9]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L9* ?*print-OR3-whip* ?*print-OR3-whip-9*) then
...
(defrule OR3-whip[9]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR3-whip[9]-salience-2*))
   (ORk-chain
      (type partial-OR3-whip)
      (context ?cont)
      (length 8)
