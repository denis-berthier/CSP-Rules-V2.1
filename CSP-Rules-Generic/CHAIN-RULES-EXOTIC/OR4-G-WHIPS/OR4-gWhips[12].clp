...
(defrule activate-OR4-gwhip[12]
   (declare (salience ?*activate-OR4-gwhip[12]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW12))
   (assert (technique ?cont OR4-gwhip[12]))
   (assert (technique ?cont partial-whip[11]))
   (assert (technique ?cont partial-gwhip[11]))
   (assert (technique ?cont partial-OR4-gwhip[11]))
   (bind ?*technique* OR4gW[12])
)



(defrule track-OR4-gwhip[12]
   (declare (salience ?*activate-OR4-gwhip[12]-salience*))
   ?level <- (technique ?cont OR4-gwhip[12])
=>
...
(defrule OR4-gwhip[12]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[12]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L12* ?*print-OR4-gwhip* ?*print-OR4-gwhip-12*) then
...
(defrule OR4-gwhip[12]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[12]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 11)
