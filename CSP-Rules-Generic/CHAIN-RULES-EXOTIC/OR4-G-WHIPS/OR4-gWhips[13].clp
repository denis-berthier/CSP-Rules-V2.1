...
(defrule activate-OR4-gwhip[13]
   (declare (salience ?*activate-OR4-gwhip[13]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW13))
   (assert (technique ?cont OR4-gwhip[13]))
   (assert (technique ?cont partial-whip[12]))
   (assert (technique ?cont partial-gwhip[12]))
   (assert (technique ?cont partial-OR4-gwhip[12]))
   (bind ?*technique* OR4gW[13])
)



(defrule track-OR4-gwhip[13]
   (declare (salience ?*activate-OR4-gwhip[13]-salience*))
   ?level <- (technique ?cont OR4-gwhip[13])
=>
...
(defrule OR4-gwhip[13]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[13]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L13* ?*print-OR4-gwhip* ?*print-OR4-gwhip-13*) then
...
(defrule OR4-gwhip[13]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[13]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 12)
