...
(defrule activate-OR4-gwhip[14]
   (declare (salience ?*activate-OR4-gwhip[14]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW14))
   (assert (technique ?cont OR4-gwhip[14]))
   (assert (technique ?cont partial-whip[13]))
   (assert (technique ?cont partial-gwhip[13]))
   (assert (technique ?cont partial-OR4-gwhip[13]))
   (bind ?*technique* OR4gW[14])
)



(defrule track-OR4-gwhip[14]
   (declare (salience ?*activate-OR4-gwhip[14]-salience*))
   ?level <- (technique ?cont OR4-gwhip[14])
=>
...
(defrule OR4-gwhip[14]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[14]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L14* ?*print-OR4-gwhip* ?*print-OR4-gwhip-14*) then
...
(defrule OR4-gwhip[14]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[14]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 13)
