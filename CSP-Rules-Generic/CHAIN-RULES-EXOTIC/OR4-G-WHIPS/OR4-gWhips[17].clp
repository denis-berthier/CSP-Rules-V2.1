...
(defrule activate-OR4-gwhip[17]
   (declare (salience ?*activate-OR4-gwhip[17]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW17))
   (assert (technique ?cont OR4-gwhip[17]))
   (assert (technique ?cont partial-whip[16]))
   (assert (technique ?cont partial-gwhip[16]))
   (assert (technique ?cont partial-OR4-gwhip[16]))
   (bind ?*technique* OR4gW[17])
)



(defrule track-OR4-gwhip[17]
   (declare (salience ?*activate-OR4-gwhip[17]-salience*))
   ?level <- (technique ?cont OR4-gwhip[17])
=>
...
(defrule OR4-gwhip[17]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[17]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L17* ?*print-OR4-gwhip* ?*print-OR4-gwhip-17*) then
...
(defrule OR4-gwhip[17]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[17]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 16)
