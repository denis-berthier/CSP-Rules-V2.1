...
(defrule activate-OR4-gwhip[20]
   (declare (salience ?*activate-OR4-gwhip[20]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW20))
   (assert (technique ?cont OR4-gwhip[20]))
   (assert (technique ?cont partial-whip[19]))
   (assert (technique ?cont partial-gwhip[19]))
   (assert (technique ?cont partial-OR4-gwhip[19]))
   (bind ?*technique* OR4gW[20])
)



(defrule track-OR4-gwhip[20]
   (declare (salience ?*activate-OR4-gwhip[20]-salience*))
   ?level <- (technique ?cont OR4-gwhip[20])
=>
...
(defrule OR4-gwhip[20]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[20]-salience-1*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 19)
      (target ?zzz)
      (length1 ?len1&:(= ?len1 18))
      (llcs1 $?llcs1)
      (rlcs1 $?rlcs1)
      (csp-vars1 $?csp-vars1)
      (last-rlc1 ?last-rlc1)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L20* ?*print-OR4-gwhip* ?*print-OR4-gwhip-20*) then
...
(defrule OR4-gwhip[20]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[20]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 19)
