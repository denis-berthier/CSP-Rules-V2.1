...
(defrule activate-OR4-gwhip[21]
   (declare (salience ?*activate-OR4-gwhip[21]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW21))
   (assert (technique ?cont OR4-gwhip[21]))
   (assert (technique ?cont partial-whip[20]))
   (assert (technique ?cont partial-gwhip[20]))
   (assert (technique ?cont partial-OR4-gwhip[20]))
   (bind ?*technique* OR4gW[21])
)



(defrule track-OR4-gwhip[21]
   (declare (salience ?*activate-OR4-gwhip[21]-salience*))
   ?level <- (technique ?cont OR4-gwhip[21])
=>
...
(defrule OR4-gwhip[21]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[21]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L21* ?*print-OR4-gwhip* ?*print-OR4-gwhip-21*) then
...
(defrule OR4-gwhip[21]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[21]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 20)
