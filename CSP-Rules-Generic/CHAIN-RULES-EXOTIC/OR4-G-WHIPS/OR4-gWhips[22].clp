...
(defrule activate-OR4-gwhip[22]
   (declare (salience ?*activate-OR4-gwhip[22]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW22))
   (assert (technique ?cont OR4-gwhip[22]))
   (assert (technique ?cont partial-whip[21]))
   (assert (technique ?cont partial-gwhip[21]))
   (assert (technique ?cont partial-OR4-gwhip[21]))
   (bind ?*technique* OR4gW[22])
)



(defrule track-OR4-gwhip[22]
   (declare (salience ?*activate-OR4-gwhip[22]-salience*))
   ?level <- (technique ?cont OR4-gwhip[22])
=>
...
(defrule OR4-gwhip[22]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[22]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L22* ?*print-OR4-gwhip* ?*print-OR4-gwhip-22*) then
...
(defrule OR4-gwhip[22]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[22]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 21)
