...
(defrule activate-OR4-gwhip[23]
   (declare (salience ?*activate-OR4-gwhip[23]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW23))
   (assert (technique ?cont OR4-gwhip[23]))
   (assert (technique ?cont partial-whip[22]))
   (assert (technique ?cont partial-gwhip[22]))
   (assert (technique ?cont partial-OR4-gwhip[22]))
   (bind ?*technique* OR4gW[23])
)



(defrule track-OR4-gwhip[23]
   (declare (salience ?*activate-OR4-gwhip[23]-salience*))
   ?level <- (technique ?cont OR4-gwhip[23])
=>
...
(defrule OR4-gwhip[23]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[23]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L23* ?*print-OR4-gwhip* ?*print-OR4-gwhip-23*) then
...
(defrule OR4-gwhip[23]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[23]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 22)
