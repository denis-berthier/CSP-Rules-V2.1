...
(defrule activate-OR4-gwhip[24]
   (declare (salience ?*activate-OR4-gwhip[24]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW24))
   (assert (technique ?cont OR4-gwhip[24]))
   (assert (technique ?cont partial-whip[23]))
   (assert (technique ?cont partial-gwhip[23]))
   (assert (technique ?cont partial-OR4-gwhip[23]))
   (bind ?*technique* OR4gW[24])
)



(defrule track-OR4-gwhip[24]
   (declare (salience ?*activate-OR4-gwhip[24]-salience*))
   ?level <- (technique ?cont OR4-gwhip[24])
=>
...
(defrule OR4-gwhip[24]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[24]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L24* ?*print-OR4-gwhip* ?*print-OR4-gwhip-24*) then
...
(defrule OR4-gwhip[24]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[24]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 23)
