...
(defrule activate-OR4-gwhip[25]
   (declare (salience ?*activate-OR4-gwhip[25]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW25))
   (assert (technique ?cont OR4-gwhip[25]))
   (assert (technique ?cont partial-whip[24]))
   (assert (technique ?cont partial-gwhip[24]))
   (assert (technique ?cont partial-OR4-gwhip[24]))
   (bind ?*technique* OR4gW[25])
)



(defrule track-OR4-gwhip[25]
   (declare (salience ?*activate-OR4-gwhip[25]-salience*))
   ?level <- (technique ?cont OR4-gwhip[25])
=>
...
(defrule OR4-gwhip[25]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[25]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L25* ?*print-OR4-gwhip* ?*print-OR4-gwhip-25*) then
...
(defrule OR4-gwhip[25]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[25]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 24)
