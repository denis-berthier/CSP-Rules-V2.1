...
(defrule activate-OR4-gwhip[26]
   (declare (salience ?*activate-OR4-gwhip[26]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW26))
   (assert (technique ?cont OR4-gwhip[26]))
   (assert (technique ?cont partial-whip[25]))
   (assert (technique ?cont partial-gwhip[25]))
   (assert (technique ?cont partial-OR4-gwhip[25]))
   (bind ?*technique* OR4gW[26])
)



(defrule track-OR4-gwhip[26]
   (declare (salience ?*activate-OR4-gwhip[26]-salience*))
   ?level <- (technique ?cont OR4-gwhip[26])
=>
...
(defrule OR4-gwhip[26]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[26]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L26* ?*print-OR4-gwhip* ?*print-OR4-gwhip-26*) then
...
(defrule OR4-gwhip[26]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[26]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 25)
