...
(defrule activate-OR4-gwhip[29]
   (declare (salience ?*activate-OR4-gwhip[29]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW29))
   (assert (technique ?cont OR4-gwhip[29]))
   (assert (technique ?cont partial-whip[28]))
   (assert (technique ?cont partial-gwhip[28]))
   (assert (technique ?cont partial-OR4-gwhip[28]))
   (bind ?*technique* OR4gW[29])
)



(defrule track-OR4-gwhip[29]
   (declare (salience ?*activate-OR4-gwhip[29]-salience*))
   ?level <- (technique ?cont OR4-gwhip[29])
=>
...
(defrule OR4-gwhip[29]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[29]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L29* ?*print-OR4-gwhip* ?*print-OR4-gwhip-29*) then
...
(defrule OR4-gwhip[29]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[29]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 28)
