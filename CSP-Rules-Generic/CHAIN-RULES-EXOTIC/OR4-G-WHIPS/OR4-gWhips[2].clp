...
(defrule activate-OR4-gwhip[2]
   (declare (salience ?*activate-OR4-gwhip[2]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW2))
   (assert (technique ?cont OR4-gwhip[2]))
   (assert (technique ?cont partial-whip[1]))
   (assert (technique ?cont partial-gwhip[1]))
   (assert (technique ?cont partial-OR4-gwhip[1]))
   (bind ?*technique* OR4gW[2])
)



(defrule track-OR4-gwhip[2]
   (declare (salience ?*activate-OR4-gwhip[2]-salience*))
   ?level <- (technique ?cont OR4-gwhip[2])
=>
...
(defrule OR4-gwhip[2]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[2]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L2* ?*print-OR4-gwhip* ?*print-OR4-gwhip-2*) then
