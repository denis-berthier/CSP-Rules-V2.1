...
(defrule activate-OR4-gwhip[30]
   (declare (salience ?*activate-OR4-gwhip[30]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW30))
   (assert (technique ?cont OR4-gwhip[30]))
   (assert (technique ?cont partial-whip[29]))
   (assert (technique ?cont partial-gwhip[29]))
   (assert (technique ?cont partial-OR4-gwhip[29]))
   (bind ?*technique* OR4gW[30])
)



(defrule track-OR4-gwhip[30]
   (declare (salience ?*activate-OR4-gwhip[30]-salience*))
   ?level <- (technique ?cont OR4-gwhip[30])
=>
...
(defrule OR4-gwhip[30]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[30]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L30* ?*print-OR4-gwhip* ?*print-OR4-gwhip-30*) then
...
(defrule OR4-gwhip[30]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[30]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 29)
