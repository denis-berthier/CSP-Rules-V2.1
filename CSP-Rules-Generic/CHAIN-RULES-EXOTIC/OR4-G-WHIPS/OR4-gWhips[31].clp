...
(defrule activate-OR4-gwhip[31]
   (declare (salience ?*activate-OR4-gwhip[31]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW31))
   (assert (technique ?cont OR4-gwhip[31]))
   (assert (technique ?cont partial-whip[30]))
   (assert (technique ?cont partial-gwhip[30]))
   (assert (technique ?cont partial-OR4-gwhip[30]))
   (bind ?*technique* OR4gW[31])
)



(defrule track-OR4-gwhip[31]
   (declare (salience ?*activate-OR4-gwhip[31]-salience*))
   ?level <- (technique ?cont OR4-gwhip[31])
=>
...
(defrule OR4-gwhip[31]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[31]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L31* ?*print-OR4-gwhip* ?*print-OR4-gwhip-31*) then
...
(defrule OR4-gwhip[31]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[31]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 30)
