...
(defrule activate-OR4-gwhip[33]
   (declare (salience ?*activate-OR4-gwhip[33]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW33))
   (assert (technique ?cont OR4-gwhip[33]))
   (assert (technique ?cont partial-whip[32]))
   (assert (technique ?cont partial-gwhip[32]))
   (assert (technique ?cont partial-OR4-gwhip[32]))
   (bind ?*technique* OR4gW[33])
)



(defrule track-OR4-gwhip[33]
   (declare (salience ?*activate-OR4-gwhip[33]-salience*))
   ?level <- (technique ?cont OR4-gwhip[33])
=>
...
(defrule OR4-gwhip[33]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[33]-salience-1*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 32)
      (target ?zzz)
      (length1 ?len1&:(= ?len1 31))
      (llcs1 $?llcs1)
      (rlcs1 $?rlcs1)
      (csp-vars1 $?csp-vars1)
      (last-rlc1 ?last-rlc1)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L33* ?*print-OR4-gwhip* ?*print-OR4-gwhip-33*) then
...
(defrule OR4-gwhip[33]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[33]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 32)
      (target ?zzz)
      (length1 ?len1&:(<= ?len1 30))
      (llcs1 $?llcs1)
      (rlcs1 $?rlcs1)
      (csp-vars1 $?csp-vars1)
      (last-rlc1 ?last-rlc1)
