...
(defrule activate-OR4-gwhip[34]
   (declare (salience ?*activate-OR4-gwhip[34]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW34))
   (assert (technique ?cont OR4-gwhip[34]))
   (assert (technique ?cont partial-whip[33]))
   (assert (technique ?cont partial-gwhip[33]))
   (assert (technique ?cont partial-OR4-gwhip[33]))
   (bind ?*technique* OR4gW[34])
)



(defrule track-OR4-gwhip[34]
   (declare (salience ?*activate-OR4-gwhip[34]-salience*))
   ?level <- (technique ?cont OR4-gwhip[34])
=>
...
(defrule OR4-gwhip[34]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[34]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L34* ?*print-OR4-gwhip* ?*print-OR4-gwhip-34*) then
...
(defrule OR4-gwhip[34]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[34]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 33)
