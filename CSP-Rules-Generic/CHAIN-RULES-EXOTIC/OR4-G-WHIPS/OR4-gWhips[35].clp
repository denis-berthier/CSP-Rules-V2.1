...
(defrule activate-OR4-gwhip[35]
   (declare (salience ?*activate-OR4-gwhip[35]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW35))
   (assert (technique ?cont OR4-gwhip[35]))
   (assert (technique ?cont partial-whip[34]))
   (assert (technique ?cont partial-gwhip[34]))
   (assert (technique ?cont partial-OR4-gwhip[34]))
   (bind ?*technique* OR4gW[35])
)



(defrule track-OR4-gwhip[35]
   (declare (salience ?*activate-OR4-gwhip[35]-salience*))
   ?level <- (technique ?cont OR4-gwhip[35])
=>
...
(defrule OR4-gwhip[35]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[35]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L35* ?*print-OR4-gwhip* ?*print-OR4-gwhip-35*) then
...
(defrule OR4-gwhip[35]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[35]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 34)
