...
(defrule activate-OR4-gwhip[36]
   (declare (salience ?*activate-OR4-gwhip[36]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW36))
   (assert (technique ?cont OR4-gwhip[36]))
   (assert (technique ?cont partial-whip[35]))
   (assert (technique ?cont partial-gwhip[35]))
   (assert (technique ?cont partial-OR4-gwhip[35]))
   (bind ?*technique* OR4gW[36])
)



(defrule track-OR4-gwhip[36]
   (declare (salience ?*activate-OR4-gwhip[36]-salience*))
   ?level <- (technique ?cont OR4-gwhip[36])
=>
...
(defrule OR4-gwhip[36]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[36]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L36* ?*print-OR4-gwhip* ?*print-OR4-gwhip-36*) then
...
(defrule OR4-gwhip[36]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[36]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 35)
