...
(defrule activate-OR4-gwhip[6]
   (declare (salience ?*activate-OR4-gwhip[6]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW6))
   (assert (technique ?cont OR4-gwhip[6]))
   (assert (technique ?cont partial-whip[5]))
   (assert (technique ?cont partial-gwhip[5]))
   (assert (technique ?cont partial-OR4-gwhip[5]))
   (bind ?*technique* OR4gW[6])
)



(defrule track-OR4-gwhip[6]
   (declare (salience ?*activate-OR4-gwhip[6]-salience*))
   ?level <- (technique ?cont OR4-gwhip[6])
=>
...
(defrule OR4-gwhip[6]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[6]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L6* ?*print-OR4-gwhip* ?*print-OR4-gwhip-6*) then
...
(defrule OR4-gwhip[6]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[6]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 5)
