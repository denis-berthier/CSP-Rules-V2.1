...
(defrule activate-OR4-gwhip[7]
   (declare (salience ?*activate-OR4-gwhip[7]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW7))
   (assert (technique ?cont OR4-gwhip[7]))
   (assert (technique ?cont partial-whip[6]))
   (assert (technique ?cont partial-gwhip[6]))
   (assert (technique ?cont partial-OR4-gwhip[6]))
   (bind ?*technique* OR4gW[7])
)



(defrule track-OR4-gwhip[7]
   (declare (salience ?*activate-OR4-gwhip[7]-salience*))
   ?level <- (technique ?cont OR4-gwhip[7])
=>
...
(defrule OR4-gwhip[7]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[7]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L7* ?*print-OR4-gwhip* ?*print-OR4-gwhip-7*) then
...
(defrule OR4-gwhip[7]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[7]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 6)
