...
(defrule activate-OR4-gwhip[9]
   (declare (salience ?*activate-OR4-gwhip[9]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4gW9))
   (assert (technique ?cont OR4-gwhip[9]))
   (assert (technique ?cont partial-whip[8]))
   (assert (technique ?cont partial-gwhip[8]))
   (assert (technique ?cont partial-OR4-gwhip[8]))
   (bind ?*technique* OR4gW[9])
)



(defrule track-OR4-gwhip[9]
   (declare (salience ?*activate-OR4-gwhip[9]-salience*))
   ?level <- (technique ?cont OR4-gwhip[9])
=>
...
(defrule OR4-gwhip[9]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-gwhip[9]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L9* ?*print-OR4-gwhip* ?*print-OR4-gwhip-9*) then
...
(defrule OR4-gwhip[9]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-gwhip[9]-salience-2*))
   (ORk-chain
      (type partial-OR4-gwhip)
      (context ?cont)
      (length 8)
