...
(defrule activate-OR4-whip[10]
   (declare (salience ?*activate-OR4-whip[10]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4W10))
   (assert (technique ?cont OR4-whip[10]))
   (assert (technique ?cont partial-whip[9]))
   (assert (technique ?cont partial-OR4-whip[9]))
   (bind ?*technique* OR4W[10])
)



(defrule track-OR4-whip[10]
   (declare (salience ?*activate-OR4-whip[10]-salience*))
   ?level <- (technique ?cont OR4-whip[10])
=>
...
(defrule OR4-whip[10]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-whip[10]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L10* ?*print-OR4-whip* ?*print-OR4-whip-10*) then
...
(defrule OR4-whip[10]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-whip[10]-salience-2*))
   (ORk-chain
      (type partial-OR4-whip)
      (context ?cont)
      (length 9)
