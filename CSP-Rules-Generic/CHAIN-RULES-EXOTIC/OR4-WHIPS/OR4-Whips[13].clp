...
(defrule activate-OR4-whip[13]
   (declare (salience ?*activate-OR4-whip[13]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4W13))
   (assert (technique ?cont OR4-whip[13]))
   (assert (technique ?cont partial-whip[12]))
   (assert (technique ?cont partial-OR4-whip[12]))
   (bind ?*technique* OR4W[13])
)



(defrule track-OR4-whip[13]
   (declare (salience ?*activate-OR4-whip[13]-salience*))
   ?level <- (technique ?cont OR4-whip[13])
=>
...
(defrule OR4-whip[13]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-whip[13]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L13* ?*print-OR4-whip* ?*print-OR4-whip-13*) then
...
(defrule OR4-whip[13]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-whip[13]-salience-2*))
   (ORk-chain
      (type partial-OR4-whip)
      (context ?cont)
      (length 12)
