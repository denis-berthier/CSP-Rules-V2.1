...
(defrule activate-OR4-whip[16]
   (declare (salience ?*activate-OR4-whip[16]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4W16))
   (assert (technique ?cont OR4-whip[16]))
   (assert (technique ?cont partial-whip[15]))
   (assert (technique ?cont partial-OR4-whip[15]))
   (bind ?*technique* OR4W[16])
)



(defrule track-OR4-whip[16]
   (declare (salience ?*activate-OR4-whip[16]-salience*))
   ?level <- (technique ?cont OR4-whip[16])
=>
...
(defrule OR4-whip[16]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-whip[16]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L16* ?*print-OR4-whip* ?*print-OR4-whip-16*) then
...
(defrule OR4-whip[16]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-whip[16]-salience-2*))
   (ORk-chain
      (type partial-OR4-whip)
      (context ?cont)
      (length 15)
