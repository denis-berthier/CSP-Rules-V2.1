...
(defrule activate-OR4-whip[19]
   (declare (salience ?*activate-OR4-whip[19]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4W19))
   (assert (technique ?cont OR4-whip[19]))
   (assert (technique ?cont partial-whip[18]))
   (assert (technique ?cont partial-OR4-whip[18]))
   (bind ?*technique* OR4W[19])
)



(defrule track-OR4-whip[19]
   (declare (salience ?*activate-OR4-whip[19]-salience*))
   ?level <- (technique ?cont OR4-whip[19])
=>
...
(defrule OR4-whip[19]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-whip[19]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L19* ?*print-OR4-whip* ?*print-OR4-whip-19*) then
...
(defrule OR4-whip[19]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-whip[19]-salience-2*))
   (ORk-chain
      (type partial-OR4-whip)
      (context ?cont)
      (length 18)
