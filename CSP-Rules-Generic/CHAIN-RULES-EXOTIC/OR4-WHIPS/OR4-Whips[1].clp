...
=>
   ...
   (if (or ?*print-actions* ?*print-L1* ?*print-OR4-whip* ?*print-OR4-whip-1*) then
       (print-ORk-whip[1]-without-crlf ?or-name 4 ?or-compl ?zzz ?zzz1 ?zzz2 ?zzz3 ?zzz4)
    )
   (if (not ?*blocked-Whips[1]*)
       then (printout t crlf)
       else (assert (apply-rule-as-a-pseudo-block ?cont))
           (assert (pseudo-blocked ?cont OR4-whip[1] ?zzz ?zzz1 ?zzz2 ?zzz3 ?zzz4))
   )
)



(defrule apply-OR4-whip[1]-to-more-targets
    (declare (salience ?*apply-a-blocked-rule-salience-1*))
    (pseudo-blocked ?cont OR4-whip[1] ?zzz ?zzz1 ?zzz2 ?zzz3 ?zzz4)
...
=>
    (retract ?cand)
    (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
    (if (or ?*print-actions* ?*print-L1* ?*print-whip* ?*print-OR4-whip-1*) then
        (printout t ", ")
