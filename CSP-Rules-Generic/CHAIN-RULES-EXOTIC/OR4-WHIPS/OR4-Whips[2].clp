...
(defrule activate-OR4-whip[2]
   (declare (salience ?*activate-OR4-whip[2]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4W2))
   (assert (technique ?cont OR4-whip[2]))
   (assert (technique ?cont partial-whip[1]))
   (assert (technique ?cont partial-OR4-whip[1]))
   (bind ?*technique* OR4W[2])
)



(defrule track-OR4-whip[2]
   (declare (salience ?*activate-OR4-whip[2]-salience*))
   ?level <- (technique ?cont OR4-whip[2])
=>
...
(defrule OR4-whip[2]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-whip[2]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L2* ?*print-OR4-whip* ?*print-OR4-whip-2*) then
