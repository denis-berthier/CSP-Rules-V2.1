...
(defrule activate-OR4-whip[8]
   (declare (salience ?*activate-OR4-whip[8]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR4W8))
   (assert (technique ?cont OR4-whip[8]))
   (assert (technique ?cont partial-whip[7]))
   (assert (technique ?cont partial-OR4-whip[7]))
   (bind ?*technique* OR4W[8])
)



(defrule track-OR4-whip[8]
   (declare (salience ?*activate-OR4-whip[8]-salience*))
   ?level <- (technique ?cont OR4-whip[8])
=>
...
(defrule OR4-whip[8]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR4-whip[8]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L8* ?*print-OR4-whip* ?*print-OR4-whip-8*) then
...
(defrule OR4-whip[8]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR4-whip[8]-salience-2*))
   (ORk-chain
      (type partial-OR4-whip)
      (context ?cont)
      (length 7)
