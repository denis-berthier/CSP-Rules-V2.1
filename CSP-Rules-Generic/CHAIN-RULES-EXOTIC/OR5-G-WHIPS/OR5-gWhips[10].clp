...
(defrule activate-OR5-gwhip[10]
   (declare (salience ?*activate-OR5-gwhip[10]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5gW10))
   (assert (technique ?cont OR5-gwhip[10]))
   (assert (technique ?cont partial-whip[9]))
   (assert (technique ?cont partial-gwhip[9]))
   (assert (technique ?cont partial-OR5-gwhip[9]))
   (bind ?*technique* OR5gW[10])
)



(defrule track-OR5-gwhip[10]
   (declare (salience ?*activate-OR5-gwhip[10]-salience*))
   ?level <- (technique ?cont OR5-gwhip[10])
=>
...
(defrule OR5-gwhip[10]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-gwhip[10]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L10* ?*print-OR5-gwhip* ?*print-OR5-gwhip-10*) then
...
(defrule OR5-gwhip[10]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-gwhip[10]-salience-2*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 9)
