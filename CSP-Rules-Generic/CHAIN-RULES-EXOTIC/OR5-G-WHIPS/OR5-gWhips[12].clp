...
(defrule activate-OR5-gwhip[12]
   (declare (salience ?*activate-OR5-gwhip[12]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5gW12))
   (assert (technique ?cont OR5-gwhip[12]))
   (assert (technique ?cont partial-whip[11]))
   (assert (technique ?cont partial-gwhip[11]))
   (assert (technique ?cont partial-OR5-gwhip[11]))
   (bind ?*technique* OR5gW[12])
)



(defrule track-OR5-gwhip[12]
   (declare (salience ?*activate-OR5-gwhip[12]-salience*))
   ?level <- (technique ?cont OR5-gwhip[12])
=>
...
(defrule OR5-gwhip[12]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-gwhip[12]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L12* ?*print-OR5-gwhip* ?*print-OR5-gwhip-12*) then
...
(defrule OR5-gwhip[12]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-gwhip[12]-salience-2*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 11)
