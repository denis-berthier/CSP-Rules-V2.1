...
(defrule activate-OR5-gwhip[13]
   (declare (salience ?*activate-OR5-gwhip[13]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5gW13))
   (assert (technique ?cont OR5-gwhip[13]))
   (assert (technique ?cont partial-whip[12]))
   (assert (technique ?cont partial-gwhip[12]))
   (assert (technique ?cont partial-OR5-gwhip[12]))
   (bind ?*technique* OR5gW[13])
)



(defrule track-OR5-gwhip[13]
   (declare (salience ?*activate-OR5-gwhip[13]-salience*))
   ?level <- (technique ?cont OR5-gwhip[13])
=>
...
(defrule OR5-gwhip[13]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-gwhip[13]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L13* ?*print-OR5-gwhip* ?*print-OR5-gwhip-13*) then
...
(defrule OR5-gwhip[13]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-gwhip[13]-salience-2*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 12)
