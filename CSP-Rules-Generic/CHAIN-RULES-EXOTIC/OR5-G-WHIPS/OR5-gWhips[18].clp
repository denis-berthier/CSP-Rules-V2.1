...
(defrule activate-OR5-gwhip[18]
   (declare (salience ?*activate-OR5-gwhip[18]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5gW18))
   (assert (technique ?cont OR5-gwhip[18]))
   (assert (technique ?cont partial-whip[17]))
   (assert (technique ?cont partial-gwhip[17]))
   (assert (technique ?cont partial-OR5-gwhip[17]))
   (bind ?*technique* OR5gW[18])
)



(defrule track-OR5-gwhip[18]
   (declare (salience ?*activate-OR5-gwhip[18]-salience*))
   ?level <- (technique ?cont OR5-gwhip[18])
=>
...
(defrule OR5-gwhip[18]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-gwhip[18]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L18* ?*print-OR5-gwhip* ?*print-OR5-gwhip-18*) then
...
(defrule OR5-gwhip[18]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-gwhip[18]-salience-2*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 17)
