...
(defrule activate-OR5-gwhip[22]
   (declare (salience ?*activate-OR5-gwhip[22]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5gW22))
   (assert (technique ?cont OR5-gwhip[22]))
   (assert (technique ?cont partial-whip[21]))
   (assert (technique ?cont partial-gwhip[21]))
   (assert (technique ?cont partial-OR5-gwhip[21]))
   (bind ?*technique* OR5gW[22])
)



(defrule track-OR5-gwhip[22]
   (declare (salience ?*activate-OR5-gwhip[22]-salience*))
   ?level <- (technique ?cont OR5-gwhip[22])
=>
...
(defrule OR5-gwhip[22]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-gwhip[22]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L22* ?*print-OR5-gwhip* ?*print-OR5-gwhip-22*) then
...
(defrule OR5-gwhip[22]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-gwhip[22]-salience-2*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 21)
