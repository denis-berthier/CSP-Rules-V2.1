...
(defrule activate-OR5-gwhip[25]
   (declare (salience ?*activate-OR5-gwhip[25]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5gW25))
   (assert (technique ?cont OR5-gwhip[25]))
   (assert (technique ?cont partial-whip[24]))
   (assert (technique ?cont partial-gwhip[24]))
   (assert (technique ?cont partial-OR5-gwhip[24]))
   (bind ?*technique* OR5gW[25])
)



(defrule track-OR5-gwhip[25]
   (declare (salience ?*activate-OR5-gwhip[25]-salience*))
   ?level <- (technique ?cont OR5-gwhip[25])
=>
...
(defrule OR5-gwhip[25]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-gwhip[25]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L25* ?*print-OR5-gwhip* ?*print-OR5-gwhip-25*) then
...
(defrule OR5-gwhip[25]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-gwhip[25]-salience-2*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 24)
