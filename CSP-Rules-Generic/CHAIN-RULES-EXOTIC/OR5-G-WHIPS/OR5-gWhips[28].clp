...
(defrule activate-OR5-gwhip[28]
   (declare (salience ?*activate-OR5-gwhip[28]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5gW28))
   (assert (technique ?cont OR5-gwhip[28]))
   (assert (technique ?cont partial-whip[27]))
   (assert (technique ?cont partial-gwhip[27]))
   (assert (technique ?cont partial-OR5-gwhip[27]))
   (bind ?*technique* OR5gW[28])
)



(defrule track-OR5-gwhip[28]
   (declare (salience ?*activate-OR5-gwhip[28]-salience*))
   ?level <- (technique ?cont OR5-gwhip[28])
=>
...
(defrule OR5-gwhip[28]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-gwhip[28]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L28* ?*print-OR5-gwhip* ?*print-OR5-gwhip-28*) then
...
(defrule OR5-gwhip[28]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-gwhip[28]-salience-2*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 27)
