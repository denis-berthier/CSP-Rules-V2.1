...
(defrule activate-OR5-gwhip[29]
   (declare (salience ?*activate-OR5-gwhip[29]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5gW29))
   (assert (technique ?cont OR5-gwhip[29]))
   (assert (technique ?cont partial-whip[28]))
   (assert (technique ?cont partial-gwhip[28]))
   (assert (technique ?cont partial-OR5-gwhip[28]))
   (bind ?*technique* OR5gW[29])
)



(defrule track-OR5-gwhip[29]
   (declare (salience ?*activate-OR5-gwhip[29]-salience*))
   ?level <- (technique ?cont OR5-gwhip[29])
=>
...
(defrule OR5-gwhip[29]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-gwhip[29]-salience-1*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 28)
      (target ?zzz)
      (length1 ?len1&:(= ?len1 27))
      (llcs1 $?llcs1)
      (rlcs1 $?rlcs1)
      (csp-vars1 $?csp-vars1)
      (last-rlc1 ?last-rlc1)
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L29* ?*print-OR5-gwhip* ?*print-OR5-gwhip-29*) then
...
(defrule OR5-gwhip[29]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-gwhip[29]-salience-2*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 28)
