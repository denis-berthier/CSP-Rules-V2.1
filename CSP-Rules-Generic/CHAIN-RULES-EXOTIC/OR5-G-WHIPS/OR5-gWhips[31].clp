...
(defrule activate-OR5-gwhip[31]
   (declare (salience ?*activate-OR5-gwhip[31]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5gW31))
   (assert (technique ?cont OR5-gwhip[31]))
   (assert (technique ?cont partial-whip[30]))
   (assert (technique ?cont partial-gwhip[30]))
   (assert (technique ?cont partial-OR5-gwhip[30]))
   (bind ?*technique* OR5gW[31])
)



(defrule track-OR5-gwhip[31]
   (declare (salience ?*activate-OR5-gwhip[31]-salience*))
   ?level <- (technique ?cont OR5-gwhip[31])
=>
...
(defrule OR5-gwhip[31]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-gwhip[31]-salience-1*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 30)
      (target ?zzz)
      (length1 ?len1&:(= ?len1 29))
      (llcs1 $?llcs1)
      (rlcs1 $?rlcs1)
      (csp-vars1 $?csp-vars1)
      (last-rlc1 ?last-rlc1)
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L31* ?*print-OR5-gwhip* ?*print-OR5-gwhip-31*) then
...
(defrule OR5-gwhip[31]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-gwhip[31]-salience-2*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 30)
