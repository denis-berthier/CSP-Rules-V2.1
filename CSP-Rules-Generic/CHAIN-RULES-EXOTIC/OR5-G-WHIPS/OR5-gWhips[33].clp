...
(defrule activate-OR5-gwhip[33]
   (declare (salience ?*activate-OR5-gwhip[33]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5gW33))
   (assert (technique ?cont OR5-gwhip[33]))
   (assert (technique ?cont partial-whip[32]))
   (assert (technique ?cont partial-gwhip[32]))
   (assert (technique ?cont partial-OR5-gwhip[32]))
   (bind ?*technique* OR5gW[33])
)



(defrule track-OR5-gwhip[33]
   (declare (salience ?*activate-OR5-gwhip[33]-salience*))
   ?level <- (technique ?cont OR5-gwhip[33])
=>
...
(defrule OR5-gwhip[33]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-gwhip[33]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L33* ?*print-OR5-gwhip* ?*print-OR5-gwhip-33*) then
...
(defrule OR5-gwhip[33]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-gwhip[33]-salience-2*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 32)
