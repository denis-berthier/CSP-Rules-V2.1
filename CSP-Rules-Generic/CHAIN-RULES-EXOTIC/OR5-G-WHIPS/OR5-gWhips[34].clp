...
(defrule activate-OR5-gwhip[34]
   (declare (salience ?*activate-OR5-gwhip[34]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5gW34))
   (assert (technique ?cont OR5-gwhip[34]))
   (assert (technique ?cont partial-whip[33]))
   (assert (technique ?cont partial-gwhip[33]))
   (assert (technique ?cont partial-OR5-gwhip[33]))
   (bind ?*technique* OR5gW[34])
)



(defrule track-OR5-gwhip[34]
   (declare (salience ?*activate-OR5-gwhip[34]-salience*))
   ?level <- (technique ?cont OR5-gwhip[34])
=>
...
(defrule OR5-gwhip[34]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-gwhip[34]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L34* ?*print-OR5-gwhip* ?*print-OR5-gwhip-34*) then
...
(defrule OR5-gwhip[34]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-gwhip[34]-salience-2*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 33)
