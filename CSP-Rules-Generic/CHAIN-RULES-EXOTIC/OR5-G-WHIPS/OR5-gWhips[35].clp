...
(defrule activate-OR5-gwhip[35]
   (declare (salience ?*activate-OR5-gwhip[35]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5gW35))
   (assert (technique ?cont OR5-gwhip[35]))
   (assert (technique ?cont partial-whip[34]))
   (assert (technique ?cont partial-gwhip[34]))
   (assert (technique ?cont partial-OR5-gwhip[34]))
   (bind ?*technique* OR5gW[35])
)



(defrule track-OR5-gwhip[35]
   (declare (salience ?*activate-OR5-gwhip[35]-salience*))
   ?level <- (technique ?cont OR5-gwhip[35])
=>
...
(defrule OR5-gwhip[35]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-gwhip[35]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L35* ?*print-OR5-gwhip* ?*print-OR5-gwhip-35*) then
...
(defrule OR5-gwhip[35]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-gwhip[35]-salience-2*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 34)
