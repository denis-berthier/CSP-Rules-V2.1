...
(defrule activate-OR5-gwhip[3]
   (declare (salience ?*activate-OR5-gwhip[3]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5gW3))
   (assert (technique ?cont OR5-gwhip[3]))
   (assert (technique ?cont partial-whip[2]))
   (assert (technique ?cont partial-gwhip[2]))
   (assert (technique ?cont partial-OR5-gwhip[2]))
   (bind ?*technique* OR5gW[3])
)



(defrule track-OR5-gwhip[3]
   (declare (salience ?*activate-OR5-gwhip[3]-salience*))
   ?level <- (technique ?cont OR5-gwhip[3])
=>
...
(defrule OR5-gwhip[3]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-gwhip[3]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L3* ?*print-OR5-gwhip* ?*print-OR5-gwhip-3*) then
...
(defrule OR5-gwhip[3]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-gwhip[3]-salience-2*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 2)
