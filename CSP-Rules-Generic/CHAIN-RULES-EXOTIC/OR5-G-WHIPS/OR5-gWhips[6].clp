...
(defrule activate-OR5-gwhip[6]
   (declare (salience ?*activate-OR5-gwhip[6]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5gW6))
   (assert (technique ?cont OR5-gwhip[6]))
   (assert (technique ?cont partial-whip[5]))
   (assert (technique ?cont partial-gwhip[5]))
   (assert (technique ?cont partial-OR5-gwhip[5]))
   (bind ?*technique* OR5gW[6])
)



(defrule track-OR5-gwhip[6]
   (declare (salience ?*activate-OR5-gwhip[6]-salience*))
   ?level <- (technique ?cont OR5-gwhip[6])
=>
...
(defrule OR5-gwhip[6]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-gwhip[6]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L6* ?*print-OR5-gwhip* ?*print-OR5-gwhip-6*) then
...
(defrule OR5-gwhip[6]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-gwhip[6]-salience-2*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 5)
