...
(defrule activate-OR5-gwhip[9]
   (declare (salience ?*activate-OR5-gwhip[9]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5gW9))
   (assert (technique ?cont OR5-gwhip[9]))
   (assert (technique ?cont partial-whip[8]))
   (assert (technique ?cont partial-gwhip[8]))
   (assert (technique ?cont partial-OR5-gwhip[8]))
   (bind ?*technique* OR5gW[9])
)



(defrule track-OR5-gwhip[9]
   (declare (salience ?*activate-OR5-gwhip[9]-salience*))
   ?level <- (technique ?cont OR5-gwhip[9])
=>
...
(defrule OR5-gwhip[9]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-gwhip[9]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L9* ?*print-OR5-gwhip* ?*print-OR5-gwhip-9*) then
...
(defrule OR5-gwhip[9]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-gwhip[9]-salience-2*))
   (ORk-chain
      (type partial-OR5-gwhip)
      (context ?cont)
      (length 8)
