...
(defrule activate-OR5-whip[16]
   (declare (salience ?*activate-OR5-whip[16]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5W16))
   (assert (technique ?cont OR5-whip[16]))
   (assert (technique ?cont partial-whip[15]))
   (assert (technique ?cont partial-OR5-whip[15]))
   (bind ?*technique* OR5W[16])
)



(defrule track-OR5-whip[16]
   (declare (salience ?*activate-OR5-whip[16]-salience*))
   ?level <- (technique ?cont OR5-whip[16])
=>
...
(defrule OR5-whip[16]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-whip[16]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L16* ?*print-OR5-whip* ?*print-OR5-whip-16*) then
...
(defrule OR5-whip[16]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-whip[16]-salience-2*))
   (ORk-chain
      (type partial-OR5-whip)
      (context ?cont)
      (length 15)
