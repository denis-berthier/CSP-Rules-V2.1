...
(defrule activate-OR5-whip[32]
   (declare (salience ?*activate-OR5-whip[32]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR5W32))
   (assert (technique ?cont OR5-whip[32]))
   (assert (technique ?cont partial-whip[31]))
   (assert (technique ?cont partial-OR5-whip[31]))
   (bind ?*technique* OR5W[32])
)



(defrule track-OR5-whip[32]
   (declare (salience ?*activate-OR5-whip[32]-salience*))
   ?level <- (technique ?cont OR5-whip[32])
=>
...
(defrule OR5-whip[32]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR5-whip[32]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L32* ?*print-OR5-whip* ?*print-OR5-whip-32*) then
...
(defrule OR5-whip[32]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR5-whip[32]-salience-2*))
   (ORk-chain
      (type partial-OR5-whip)
      (context ?cont)
      (length 31)
