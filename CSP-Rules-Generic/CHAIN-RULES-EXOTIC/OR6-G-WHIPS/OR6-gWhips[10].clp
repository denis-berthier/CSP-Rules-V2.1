...
(defrule activate-OR6-gwhip[10]
   (declare (salience ?*activate-OR6-gwhip[10]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6gW10))
   (assert (technique ?cont OR6-gwhip[10]))
   (assert (technique ?cont partial-whip[9]))
   (assert (technique ?cont partial-gwhip[9]))
   (assert (technique ?cont partial-OR6-gwhip[9]))
   (bind ?*technique* OR6gW[10])
)



(defrule track-OR6-gwhip[10]
   (declare (salience ?*activate-OR6-gwhip[10]-salience*))
   ?level <- (technique ?cont OR6-gwhip[10])
=>
...
(defrule OR6-gwhip[10]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR6-gwhip[10]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L10* ?*print-OR6-gwhip* ?*print-OR6-gwhip-10*) then
...
(defrule OR6-gwhip[10]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR6-gwhip[10]-salience-2*))
   (ORk-chain
      (type partial-OR6-gwhip)
      (context ?cont)
      (length 9)
