...
(defrule activate-OR6-gwhip[18]
   (declare (salience ?*activate-OR6-gwhip[18]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6gW18))
   (assert (technique ?cont OR6-gwhip[18]))
   (assert (technique ?cont partial-whip[17]))
   (assert (technique ?cont partial-gwhip[17]))
   (assert (technique ?cont partial-OR6-gwhip[17]))
   (bind ?*technique* OR6gW[18])
)



(defrule track-OR6-gwhip[18]
   (declare (salience ?*activate-OR6-gwhip[18]-salience*))
   ?level <- (technique ?cont OR6-gwhip[18])
=>
...
(defrule OR6-gwhip[18]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR6-gwhip[18]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L18* ?*print-OR6-gwhip* ?*print-OR6-gwhip-18*) then
...
(defrule OR6-gwhip[18]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR6-gwhip[18]-salience-2*))
   (ORk-chain
      (type partial-OR6-gwhip)
      (context ?cont)
      (length 17)
