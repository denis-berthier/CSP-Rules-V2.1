...
(defrule activate-OR6-gwhip[20]
   (declare (salience ?*activate-OR6-gwhip[20]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6gW20))
   (assert (technique ?cont OR6-gwhip[20]))
   (assert (technique ?cont partial-whip[19]))
   (assert (technique ?cont partial-gwhip[19]))
   (assert (technique ?cont partial-OR6-gwhip[19]))
   (bind ?*technique* OR6gW[20])
)



(defrule track-OR6-gwhip[20]
   (declare (salience ?*activate-OR6-gwhip[20]-salience*))
   ?level <- (technique ?cont OR6-gwhip[20])
=>
...
(defrule OR6-gwhip[20]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR6-gwhip[20]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L20* ?*print-OR6-gwhip* ?*print-OR6-gwhip-20*) then
...
(defrule OR6-gwhip[20]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR6-gwhip[20]-salience-2*))
   (ORk-chain
      (type partial-OR6-gwhip)
      (context ?cont)
      (length 19)
