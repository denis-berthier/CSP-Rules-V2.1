...
(defrule activate-OR6-gwhip[21]
   (declare (salience ?*activate-OR6-gwhip[21]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6gW21))
   (assert (technique ?cont OR6-gwhip[21]))
   (assert (technique ?cont partial-whip[20]))
   (assert (technique ?cont partial-gwhip[20]))
   (assert (technique ?cont partial-OR6-gwhip[20]))
   (bind ?*technique* OR6gW[21])
)



(defrule track-OR6-gwhip[21]
   (declare (salience ?*activate-OR6-gwhip[21]-salience*))
   ?level <- (technique ?cont OR6-gwhip[21])
=>
...
(defrule OR6-gwhip[21]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR6-gwhip[21]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L21* ?*print-OR6-gwhip* ?*print-OR6-gwhip-21*) then
...
(defrule OR6-gwhip[21]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR6-gwhip[21]-salience-2*))
   (ORk-chain
      (type partial-OR6-gwhip)
      (context ?cont)
      (length 20)
