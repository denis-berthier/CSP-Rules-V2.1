...
(defrule activate-OR6-gwhip[25]
   (declare (salience ?*activate-OR6-gwhip[25]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6gW25))
   (assert (technique ?cont OR6-gwhip[25]))
   (assert (technique ?cont partial-whip[24]))
   (assert (technique ?cont partial-gwhip[24]))
   (assert (technique ?cont partial-OR6-gwhip[24]))
   (bind ?*technique* OR6gW[25])
)



(defrule track-OR6-gwhip[25]
   (declare (salience ?*activate-OR6-gwhip[25]-salience*))
   ?level <- (technique ?cont OR6-gwhip[25])
=>
...
(defrule OR6-gwhip[25]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR6-gwhip[25]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L25* ?*print-OR6-gwhip* ?*print-OR6-gwhip-25*) then
...
(defrule OR6-gwhip[25]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR6-gwhip[25]-salience-2*))
   (ORk-chain
      (type partial-OR6-gwhip)
      (context ?cont)
      (length 24)
