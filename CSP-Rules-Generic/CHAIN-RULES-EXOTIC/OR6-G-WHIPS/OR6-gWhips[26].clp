...
(defrule activate-OR6-gwhip[26]
   (declare (salience ?*activate-OR6-gwhip[26]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6gW26))
   (assert (technique ?cont OR6-gwhip[26]))
   (assert (technique ?cont partial-whip[25]))
   (assert (technique ?cont partial-gwhip[25]))
   (assert (technique ?cont partial-OR6-gwhip[25]))
   (bind ?*technique* OR6gW[26])
)



(defrule track-OR6-gwhip[26]
   (declare (salience ?*activate-OR6-gwhip[26]-salience*))
   ?level <- (technique ?cont OR6-gwhip[26])
=>
...
(defrule OR6-gwhip[26]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR6-gwhip[26]-salience-1*))
   (ORk-chain
      (type partial-OR6-gwhip)
      (context ?cont)
      (length 25)
      (target ?zzz)
      (length1 ?len1&:(= ?len1 24))
      (llcs1 $?llcs1)
      (rlcs1 $?rlcs1)
      (csp-vars1 $?csp-vars1)
      (last-rlc1 ?last-rlc1)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L26* ?*print-OR6-gwhip* ?*print-OR6-gwhip-26*) then
...
(defrule OR6-gwhip[26]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR6-gwhip[26]-salience-2*))
   (ORk-chain
      (type partial-OR6-gwhip)
      (context ?cont)
      (length 25)
