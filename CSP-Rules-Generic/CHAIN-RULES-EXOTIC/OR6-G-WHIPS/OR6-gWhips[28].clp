...
(defrule activate-OR6-gwhip[28]
   (declare (salience ?*activate-OR6-gwhip[28]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6gW28))
   (assert (technique ?cont OR6-gwhip[28]))
   (assert (technique ?cont partial-whip[27]))
   (assert (technique ?cont partial-gwhip[27]))
   (assert (technique ?cont partial-OR6-gwhip[27]))
   (bind ?*technique* OR6gW[28])
)



(defrule track-OR6-gwhip[28]
   (declare (salience ?*activate-OR6-gwhip[28]-salience*))
   ?level <- (technique ?cont OR6-gwhip[28])
=>
...
(defrule OR6-gwhip[28]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR6-gwhip[28]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L28* ?*print-OR6-gwhip* ?*print-OR6-gwhip-28*) then
...
(defrule OR6-gwhip[28]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR6-gwhip[28]-salience-2*))
   (ORk-chain
      (type partial-OR6-gwhip)
      (context ?cont)
      (length 27)
