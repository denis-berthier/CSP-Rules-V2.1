...
(defrule activate-OR6-gwhip[2]
   (declare (salience ?*activate-OR6-gwhip[2]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6gW2))
   (assert (technique ?cont OR6-gwhip[2]))
   (assert (technique ?cont partial-whip[1]))
   (assert (technique ?cont partial-gwhip[1]))
   (assert (technique ?cont partial-OR6-gwhip[1]))
   (bind ?*technique* OR6gW[2])
)



(defrule track-OR6-gwhip[2]
   (declare (salience ?*activate-OR6-gwhip[2]-salience*))
   ?level <- (technique ?cont OR6-gwhip[2])
=>
...
(defrule OR6-gwhip[2]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR6-gwhip[2]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L2* ?*print-OR6-gwhip* ?*print-OR6-gwhip-2*) then
