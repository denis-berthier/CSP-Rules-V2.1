...
(defrule activate-OR6-gwhip[33]
   (declare (salience ?*activate-OR6-gwhip[33]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6gW33))
   (assert (technique ?cont OR6-gwhip[33]))
   (assert (technique ?cont partial-whip[32]))
   (assert (technique ?cont partial-gwhip[32]))
   (assert (technique ?cont partial-OR6-gwhip[32]))
   (bind ?*technique* OR6gW[33])
)



(defrule track-OR6-gwhip[33]
   (declare (salience ?*activate-OR6-gwhip[33]-salience*))
   ?level <- (technique ?cont OR6-gwhip[33])
=>
...
(defrule OR6-gwhip[33]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR6-gwhip[33]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L33* ?*print-OR6-gwhip* ?*print-OR6-gwhip-33*) then
...
(defrule OR6-gwhip[33]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR6-gwhip[33]-salience-2*))
   (ORk-chain
      (type partial-OR6-gwhip)
      (context ?cont)
      (length 32)
