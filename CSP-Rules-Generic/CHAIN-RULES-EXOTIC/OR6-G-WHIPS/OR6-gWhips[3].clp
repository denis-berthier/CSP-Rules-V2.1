...
(defrule activate-OR6-gwhip[3]
   (declare (salience ?*activate-OR6-gwhip[3]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6gW3))
   (assert (technique ?cont OR6-gwhip[3]))
   (assert (technique ?cont partial-whip[2]))
   (assert (technique ?cont partial-gwhip[2]))
   (assert (technique ?cont partial-OR6-gwhip[2]))
   (bind ?*technique* OR6gW[3])
)



(defrule track-OR6-gwhip[3]
   (declare (salience ?*activate-OR6-gwhip[3]-salience*))
   ?level <- (technique ?cont OR6-gwhip[3])
=>
...
(defrule OR6-gwhip[3]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR6-gwhip[3]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L3* ?*print-OR6-gwhip* ?*print-OR6-gwhip-3*) then
...
(defrule OR6-gwhip[3]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR6-gwhip[3]-salience-2*))
   (ORk-chain
      (type partial-OR6-gwhip)
      (context ?cont)
      (length 2)
