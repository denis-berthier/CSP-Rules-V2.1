...
(defrule activate-OR6-gwhip[4]
   (declare (salience ?*activate-OR6-gwhip[4]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6gW4))
   (assert (technique ?cont OR6-gwhip[4]))
   (assert (technique ?cont partial-whip[3]))
   (assert (technique ?cont partial-gwhip[3]))
   (assert (technique ?cont partial-OR6-gwhip[3]))
   (bind ?*technique* OR6gW[4])
)



(defrule track-OR6-gwhip[4]
   (declare (salience ?*activate-OR6-gwhip[4]-salience*))
   ?level <- (technique ?cont OR6-gwhip[4])
=>
...
(defrule OR6-gwhip[4]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR6-gwhip[4]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
   (if (or ?*print-actions* ?*print-L4* ?*print-OR6-gwhip* ?*print-OR6-gwhip-4*) then
...
(defrule OR6-gwhip[4]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR6-gwhip[4]-salience-2*))
   (ORk-chain
      (type partial-OR6-gwhip)
      (context ?cont)
      (length 3)
