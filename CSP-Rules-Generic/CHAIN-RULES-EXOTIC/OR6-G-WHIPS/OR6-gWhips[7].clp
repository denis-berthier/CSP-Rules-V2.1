...
(defrule activate-OR6-gwhip[7]
   (declare (salience ?*activate-OR6-gwhip[7]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6gW7))
   (assert (technique ?cont OR6-gwhip[7]))
   (assert (technique ?cont partial-whip[6]))
   (assert (technique ?cont partial-gwhip[6]))
   (assert (technique ?cont partial-OR6-gwhip[6]))
   (bind ?*technique* OR6gW[7])
)



(defrule track-OR6-gwhip[7]
   (declare (salience ?*activate-OR6-gwhip[7]-salience*))
   ?level <- (technique ?cont OR6-gwhip[7])
=>
...
(defrule OR6-gwhip[7]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR6-gwhip[7]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L7* ?*print-OR6-gwhip* ?*print-OR6-gwhip-7*) then
...
(defrule OR6-gwhip[7]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR6-gwhip[7]-salience-2*))
   (ORk-chain
      (type partial-OR6-gwhip)
      (context ?cont)
      (length 6)
