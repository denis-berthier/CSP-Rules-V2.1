...
(defrule activate-OR6-whip[11]
   (declare (salience ?*activate-OR6-whip[11]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6W11))
   (assert (technique ?cont OR6-whip[11]))
   (assert (technique ?cont partial-whip[10]))
   (assert (technique ?cont partial-OR6-whip[10]))
   (bind ?*technique* OR6W[11])
)



(defrule track-OR6-whip[11]
   (declare (salience ?*activate-OR6-whip[11]-salience*))
   ?level <- (technique ?cont OR6-whip[11])
=>
...
(defrule OR6-whip[11]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR6-whip[11]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L11* ?*print-OR6-whip* ?*print-OR6-whip-11*) then
...
(defrule OR6-whip[11]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR6-whip[11]-salience-2*))
   (ORk-chain
      (type partial-OR6-whip)
      (context ?cont)
      (length 10)
