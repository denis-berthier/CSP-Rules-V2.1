...
(defrule activate-OR6-whip[20]
   (declare (salience ?*activate-OR6-whip[20]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6W20))
   (assert (technique ?cont OR6-whip[20]))
   (assert (technique ?cont partial-whip[19]))
   (assert (technique ?cont partial-OR6-whip[19]))
   (bind ?*technique* OR6W[20])
)



(defrule track-OR6-whip[20]
   (declare (salience ?*activate-OR6-whip[20]-salience*))
   ?level <- (technique ?cont OR6-whip[20])
=>
...
(defrule OR6-whip[20]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR6-whip[20]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L20* ?*print-OR6-whip* ?*print-OR6-whip-20*) then
...
(defrule OR6-whip[20]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR6-whip[20]-salience-2*))
   (ORk-chain
      (type partial-OR6-whip)
      (context ?cont)
      (length 19)
