...
(defrule activate-OR6-whip[31]
   (declare (salience ?*activate-OR6-whip[31]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6W31))
   (assert (technique ?cont OR6-whip[31]))
   (assert (technique ?cont partial-whip[30]))
   (assert (technique ?cont partial-OR6-whip[30]))
   (bind ?*technique* OR6W[31])
)



(defrule track-OR6-whip[31]
   (declare (salience ?*activate-OR6-whip[31]-salience*))
   ?level <- (technique ?cont OR6-whip[31])
=>
...
(defrule OR6-whip[31]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR6-whip[31]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L31* ?*print-OR6-whip* ?*print-OR6-whip-31*) then
...
(defrule OR6-whip[31]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR6-whip[31]-salience-2*))
   (ORk-chain
      (type partial-OR6-whip)
      (context ?cont)
      (length 30)
