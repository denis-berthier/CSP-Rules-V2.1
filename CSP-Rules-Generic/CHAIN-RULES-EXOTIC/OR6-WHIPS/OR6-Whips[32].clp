...
(defrule activate-OR6-whip[32]
   (declare (salience ?*activate-OR6-whip[32]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_OR6W32))
   (assert (technique ?cont OR6-whip[32]))
   (assert (technique ?cont partial-whip[31]))
   (assert (technique ?cont partial-OR6-whip[31]))
   (bind ?*technique* OR6W[32])
)



(defrule track-OR6-whip[32]
   (declare (salience ?*activate-OR6-whip[32]-salience*))
   ?level <- (technique ?cont OR6-whip[32])
=>
...
(defrule OR6-whip[32]-1
   ;;; case ?len2 = 0,  ?len > 1
   (declare (salience ?*OR6-whip[32]-salience-1*))
   (ORk-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L32* ?*print-OR6-whip* ?*print-OR6-whip-32*) then
...
(defrule OR6-whip[32]-2
   ;;; case ?len2 ≥ 1
   (declare (salience ?*OR6-whip[32]-salience-2*))
   (ORk-chain
      (type partial-OR6-whip)
      (context ?cont)
      (length 31)
