...
;;; 1- extension of an existing partial-OR2-gwhip with a candidate

(defrule partial-OR2-gwhip[14]-1
   (declare (salience ?*partial-OR2-gwhip[14]-salience-1*))
   (logical
      ?ORk <- (ORk-chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont OR2-gwhip[14])
      ;;; because OR2-gwhips[14] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as OR2-gwhips[14] based on ?zzz have been eliminated (because they have higher salience),
   ...
   ;;; do not assert a partial OR2-gwhip with the same sequences of rlc's or the same OR-rlc or the same last-rlc
   (not (and
      (ORk-chain
         (type partial-OR2-gwhip)
         (context ?cont)
...
;;; 2- extension of an existing partial-OR2- whip or gwhip with a g-candidate

(defrule partial-OR2-gwhip[14]-2
   (declare (salience ?*partial-OR2-gwhip[14]-salience-2*))
   (logical
      ?ORk <- (ORk-chain
         (type partial-OR2-whip|partial-OR2-gwhip)
         (context ?cont)
         (length 13)
         ;;; we don't have to check the other lengths:
         ;;; they are recursively guaranteed by the previous partial-OR2-whips
...
;;; 3- extension of an existing partial-gwhip with an OR2 relation

(defrule partial-OR2-gwhip[14]-3
   ;;; in this case ?len2 = 0
   (declare (salience ?*partial-OR2-gwhip[14]-salience-3*))
   (logical
      (chain
         (type partial-gwhip)
         (context ?cont)
   ...
   ;;; do not assert a partial OR2-gwhip with the same sequences of rlc's or the same OR-rlc or the same last-rlc1
   (not
      (ORk-chain
         (type partial-OR2-gwhip)
         (context ?cont)
