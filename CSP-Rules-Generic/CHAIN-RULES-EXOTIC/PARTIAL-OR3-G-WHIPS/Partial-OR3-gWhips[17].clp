...
;;; 1- extension of an existing partial-OR3-gwhip with a candidate

(defrule partial-OR3-gwhip[17]-1
   (declare (salience ?*partial-OR3-gwhip[17]-salience-1*))
   (logical
      ?ORk <- (ORk-chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont OR3-gwhip[17])
      ;;; because OR3-gwhips[17] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as OR3-gwhips[17] based on ?zzz have been eliminated (because they have higher salience),
   ...
   ;;; do not assert a partial OR3-gwhip with the same sequences of rlc's or the same OR-rlc or the same last-rlc
   (not (and
      (ORk-chain
         (type partial-OR3-gwhip)
         (context ?cont)
...
;;; 2- extension of an existing partial-OR3- whip or gwhip with a g-candidate

(defrule partial-OR3-gwhip[17]-2
   (declare (salience ?*partial-OR3-gwhip[17]-salience-2*))
   (logical
      ?ORk <- (ORk-chain
         (type partial-OR3-whip|partial-OR3-gwhip)
         (context ?cont)
         (length 16)
         ;;; we don't have to check the other lengths:
         ;;; they are recursively guaranteed by the previous partial-OR3-whips
...
;;; 3- extension of an existing partial-gwhip with an OR3 relation

(defrule partial-OR3-gwhip[17]-3
   ;;; in this case ?len2 = 0
   (declare (salience ?*partial-OR3-gwhip[17]-salience-3*))
   (logical
      (chain
         (type partial-gwhip)
         (context ?cont)
      ...
      ;;; useless: (not (exists-link ?cont ?last-rlc1 ??zzz3))
      
   ) ; end logical
   ;;; do not assert a partial OR3-gwhip with the same sequences of rlc's or the same OR-rlc or the same last-rlc1
   (not
      (ORk-chain
         (type partial-OR3-gwhip)
         (context ?cont)
