...
(defrule partial-OR4-gwhip[16]-1
   (declare (salience ?*partial-OR4-gwhip[16]-salience-1*))
   (logical
      ?ORk <- (ORk-chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont OR4-gwhip[16])
      ;;; because OR4-gwhips[16] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as OR4-gwhips[16] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule partial-OR4-gwhip[16]-2
   (declare (salience ?*partial-OR4-gwhip[16]-salience-2*))
   (logical
      ?ORk <- (ORk-chain
...
(defrule partial-OR4-gwhip[16]-3
   ;;; in this case ?len2 = 0
   (declare (salience ?*partial-OR4-gwhip[16]-salience-3*))
   (logical
      (chain
         (type partial-gwhip)
         (context ?cont)
   ...
   ;;; do not assert different partial OR4-gwhips with the same sequences of rlc's
   (not
      (ORk-chain
         (type partial-OR4-gwhip)
         (context ?cont)
