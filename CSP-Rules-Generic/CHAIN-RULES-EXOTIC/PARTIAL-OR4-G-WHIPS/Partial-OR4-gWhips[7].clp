...
(defrule partial-OR4-gwhip[7]-1
   (declare (salience ?*partial-OR4-gwhip[7]-salience-1*))
   (logical
      ?ORk <- (ORk-chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont OR4-gwhip[7])
      ;;; because OR4-gwhips[7] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as OR4-gwhips[7] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule partial-OR4-gwhip[7]-2
   (declare (salience ?*partial-OR4-gwhip[7]-salience-2*))
   (logical
      ?ORk <- (ORk-chain
...
(defrule partial-OR4-gwhip[7]-3
   ;;; in this case ?len2 = 0
   (declare (salience ?*partial-OR4-gwhip[7]-salience-3*))
   (logical
      (chain
         (type partial-gwhip)
         (context ?cont)
