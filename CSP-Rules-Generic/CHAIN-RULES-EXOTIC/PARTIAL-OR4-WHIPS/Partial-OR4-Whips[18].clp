...
(defrule partial-OR4-whip[18]-1
   (declare (salience ?*partial-OR4-whip[18]-salience-1*))
   (logical
      ?ORk-chain <- (ORk-chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont OR4-whip[18])
      ;;; because OR4-whips[18] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as OR4-whips[18] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule partial-OR4-whip[18]-2
   ;;; in this case ?len2 = 0
   (declare (salience ?*partial-OR4-whip[18]-salience-2*))
   (logical
      (chain
         (type partial-whip)
         (context ?cont)
   ...
   ;;; do not assert a partial OR4-whip with the same sequences of rlc's or the same last-rlc1 or the same OR-rlc
   (not
      (ORk-chain
         (type partial-OR4-whip)
         (context ?cont)
