...
;;; 1- extension of an existing partial-OR5-gwhip with a candidate

(defrule partial-OR5-gwhip[29]-1
   (declare (salience ?*partial-OR5-gwhip[29]-salience-1*))
   (logical
      ?ORk <- (ORk-chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont OR5-gwhip[29])
      ;;; because OR5-gwhips[29] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as OR5-gwhips[29] based on ?zzz have been eliminated (because they have higher salience),
   ...
   ;;; do not assert a partial OR5-gwhip with the same sequences of rlc's or the same OR-rlc or the same last-rlc
   (not (and
      (ORk-chain
         (type partial-OR5-gwhip)
         (context ?cont)
...
;;; 2- extension of an existing partial-OR5- whip or gwhip with a g-candidate

(defrule partial-OR5-gwhip[29]-2
   (declare (salience ?*partial-OR5-gwhip[29]-salience-2*))
   (logical
      ?ORk <- (ORk-chain
         (type partial-OR5-whip|partial-OR5-gwhip)
         (context ?cont)
         (length 28)
         ;;; we don't have to check the other lengths:
         ;;; they are recursively guaranteed by the previous partial-OR5-whips
...
;;; 3- extension of an existing partial-gwhip with an OR5 relation

(defrule partial-OR5-gwhip[29]-3
   ;;; in this case ?len2 = 0
   (declare (salience ?*partial-OR5-gwhip[29]-salience-3*))
   (logical
      (chain
         (type partial-gwhip)
         (context ?cont)
      ...
      ;;; useless: (not (exists-link ?cont ?last-rlc1 ??zzz5))
      
   ) ; end logical
   ;;; do not assert a partial OR5-gwhip with the same sequences of rlc's or the same OR-rlc or the same last-rlc1
   (not
      (ORk-chain
         (type partial-OR5-gwhip)
         (context ?cont)
