...
(defrule partial-OR6-whip[12]-1
   (declare (salience ?*partial-OR6-whip[12]-salience-1*))
   (logical
      ?ORk-chain <- (ORk-chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont OR6-whip[12])
      ;;; because OR6-whips[12] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as OR6-whips[12] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule partial-OR6-whip[12]-2
   ;;; in this case ?len2 = 0
   (declare (salience ?*partial-OR6-whip[12]-salience-2*))
   (logical
      (chain
         (type partial-whip)
         (context ?cont)
   ...
   ;;; do not assert different partial OR6-whips with the same sequences of rlc's
   (not
      (ORk-chain
         (type partial-OR6-whip)
         (context ?cont)
