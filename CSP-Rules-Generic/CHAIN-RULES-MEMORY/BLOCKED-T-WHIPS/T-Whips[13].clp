...
(defrule activate-t-whip[13]
   (declare (salience ?*activate-t-whip[13]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_tW13))
   (assert (technique ?cont partial-whip[12]))
   (assert (technique ?cont t-whip[13]))
   (bind ?*technique* tW[13])
)



(defrule track-t-whip[13]
   (declare (salience ?*activate-t-whip[13]-salience*))
   ?level <- (technique ?cont t-whip[13])
=>
...
(defrule detect-t-whip[13]
   (declare (salience ?*t-whip[13]-salience*))
   (chain
      (type partial-whip)
      (context ?cont)
      (length 12)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L13* ?*print-t-whip* ?*print-t-whip-13*) then
      (print-t-whip-without-crlf 13 ?zzz 
   ...
   (assert (pseudo-blocked ?cont t-whip[13] ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1))
)



;;; apply-t-whip[13]-to-more-targets

(defrule apply-t-whip[13]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience-1*))
   (pseudo-blocked ?cont t-whip[13] ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1)
