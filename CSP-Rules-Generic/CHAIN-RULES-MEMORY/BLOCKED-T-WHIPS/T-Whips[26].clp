...
(defrule activate-t-whip[26]
   (declare (salience ?*activate-t-whip[26]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_tW26))
   (assert (technique ?cont partial-whip[25]))
   (assert (technique ?cont t-whip[26]))
   (bind ?*technique* tW[26])
)



(defrule track-t-whip[26]
   (declare (salience ?*activate-t-whip[26]-salience*))
   ?level <- (technique ?cont t-whip[26])
=>
...
(defrule detect-t-whip[26]
   (declare (salience ?*t-whip[26]-salience*))
   (chain
      (type partial-whip)
      (context ?cont)
      (length 25)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L26* ?*print-t-whip* ?*print-t-whip-26*) then
      (print-t-whip-without-crlf 26 ?zzz 
   ...
   (assert (pseudo-blocked ?cont t-whip[26] ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1))
)



;;; apply-t-whip[26]-to-more-targets

(defrule apply-t-whip[26]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience-1*))
   (pseudo-blocked ?cont t-whip[26] ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1)
