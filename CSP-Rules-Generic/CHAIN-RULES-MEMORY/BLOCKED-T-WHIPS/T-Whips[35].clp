...
(defrule activate-t-whip[35]
   (declare (salience ?*activate-t-whip[35]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_tW35))
   (assert (technique ?cont partial-whip[34]))
   (assert (technique ?cont t-whip[35]))
   (bind ?*technique* tW[35])
)



(defrule track-t-whip[35]
   (declare (salience ?*activate-t-whip[35]-salience*))
   ?level <- (technique ?cont t-whip[35])
=>
...
(defrule detect-t-whip[35]
   (declare (salience ?*t-whip[35]-salience*))
   (chain
      (type partial-whip)
      (context ?cont)
      (length 34)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L35* ?*print-t-whip* ?*print-t-whip-35*) then
      (print-t-whip-without-crlf 35 ?zzz 
   ...
   (assert (pseudo-blocked ?cont t-whip[35] ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1))
)



;;; apply-t-whip[35]-to-more-targets

(defrule apply-t-whip[35]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience-1*))
   (pseudo-blocked ?cont t-whip[35] ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1)
