...
(defrule activate-t-whip[4]
   (declare (salience ?*activate-t-whip[4]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont t-whip))
=>
   (if ?*print-levels* then (printout t Entering_level_tW4))
   (assert (technique ?cont partial-whip[3]))
   (assert (technique ?cont t-whip[4]))
   (bind ?*technique* tW[4])
)



(defrule track-t-whip[4]
   (declare (salience ?*activate-t-whip[4]-salience*))
   ?level <- (technique ?cont t-whip[4])
=>
...
(defrule detect-t-whip[4]
   (declare (salience ?*t-whip[4]-salience*))
   (chain
      (type partial-whip)
      (context ?cont)
      (length 3)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L4* ?*print-t-whip* ?*print-t-whip-4*) then
      (print-t-whip-without-crlf 4 ?zzz 
   ...
   (assert (pseudo-blocked ?cont t-whip[4] ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1))
)



;;; apply-t-whip[4]-to-more-targets

(defrule apply-t-whip[4]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience-1*))
   (pseudo-blocked ?cont t-whip[4] ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1)
