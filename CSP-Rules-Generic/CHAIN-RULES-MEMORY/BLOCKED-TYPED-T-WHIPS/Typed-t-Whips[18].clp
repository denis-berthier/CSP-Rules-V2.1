...
(defrule activate-typed-t-whip[18]
   (declare (salience ?*activate-typed-t-whip[18]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_TytW18))
   (assert (technique ?cont typed-partial-whip[17]))
   (assert (technique ?cont typed-t-whip[18]))
   (bind ?*technique* TytW[18])
)



(defrule track-typed-t-whip[18]
   (declare (salience ?*activate-typed-t-whip[18]-salience*))
   ?level <- (technique ?cont typed-t-whip[18])
=>
...
(defrule detect-typed-t-whip[18]
   (declare (salience ?*typed-t-whip[18]-salience*))
   (typed-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L18* ?*print-typed-t-whip* ?*print-typed-t-whip-18*) then
      (print-typed-t-whip-without-crlf ?csp-type 18 ?zzz 
   ...
   (assert (pseudo-blocked ?cont typed-t-whip[18]  ?csp-type ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1))
)



;;; apply-typed-t-whip[18]-to-more-targets

(defrule apply-typed-t-whip[18]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience-1*))
   (pseudo-blocked ?cont typed-t-whip[18]  ?csp-type ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1)
