...
(defrule activate-typed-t-whip[28]
   (declare (salience ?*activate-typed-t-whip[28]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_TytW28))
   (assert (technique ?cont typed-partial-whip[27]))
   (assert (technique ?cont typed-t-whip[28]))
   (bind ?*technique* TytW[28])
)



(defrule track-typed-t-whip[28]
   (declare (salience ?*activate-typed-t-whip[28]-salience*))
   ?level <- (technique ?cont typed-t-whip[28])
=>
...
(defrule detect-typed-t-whip[28]
   (declare (salience ?*typed-t-whip[28]-salience*))
   (typed-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L28* ?*print-typed-t-whip* ?*print-typed-t-whip-28*) then
      (print-typed-t-whip-without-crlf ?csp-type 28 ?zzz 
   ...
   (assert (pseudo-blocked ?cont typed-t-whip[28]  ?csp-type ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1))
)



;;; apply-typed-t-whip[28]-to-more-targets

(defrule apply-typed-t-whip[28]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience-1*))
   (pseudo-blocked ?cont typed-t-whip[28]  ?csp-type ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1)
