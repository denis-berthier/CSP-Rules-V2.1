...
(defrule activate-typed-t-whip[6]
   (declare (salience ?*activate-typed-t-whip[6]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_TytW6))
   (assert (technique ?cont typed-partial-whip[5]))
   (assert (technique ?cont typed-t-whip[6]))
   (bind ?*technique* TytW[6])
)



(defrule track-typed-t-whip[6]
   (declare (salience ?*activate-typed-t-whip[6]-salience*))
   ?level <- (technique ?cont typed-t-whip[6])
=>
...
(defrule detect-typed-t-whip[6]
   (declare (salience ?*typed-t-whip[6]-salience*))
   (typed-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L6* ?*print-typed-t-whip* ?*print-typed-t-whip-6*) then
      (print-typed-t-whip-without-crlf ?csp-type 6 ?zzz 
   ...
   (assert (pseudo-blocked ?cont typed-t-whip[6]  ?csp-type ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1))
)



;;; apply-typed-t-whip[6]-to-more-targets

(defrule apply-typed-t-whip[6]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience-1*))
   (pseudo-blocked ?cont typed-t-whip[6]  ?csp-type ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1)
