...
(defrule activate-braid[10]
   (declare (salience ?*activate-braid[10]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_B10))
   (assert (technique ?cont braid[10]))
   (bind ?*technique* B[10])
)



(defrule track-braid[10]
   (declare (salience ?*activate-braid[10]-salience*))
   ?level <- (technique ?cont braid[10])
=>
...
(defrule partial-braid[9]
   (declare (salience ?*partial-braid[9]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[10])
      ;;; because braids[10] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[9] have been eliminated (because they have higher salience),
