...
(defrule activate-braid[15]
   (declare (salience ?*activate-braid[15]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont braid))
=>
   (if ?*print-levels* then (printout t Entering_level_B15))
   (assert (technique ?cont braid[15]))
   (bind ?*technique* B[15])
)



(defrule track-braid[15]
   (declare (salience ?*activate-braid[15]-salience*))
   ?level <- (technique ?cont braid[15])
=>
...
(defrule braid[15]
   (declare (salience ?*braid[15]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L15* ?*print-braid* ?*print-braid-15*) then
      (print-braid 15 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[14]
   (declare (salience ?*partial-braid[14]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[15])
      ;;; because braids[15] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[14] have been eliminated (because they have higher salience),
