...
(defrule activate-braid[17]
   (declare (salience ?*activate-braid[17]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_B17))
   (assert (technique ?cont braid[17]))
   (bind ?*technique* B[17])
)



(defrule track-braid[17]
   (declare (salience ?*activate-braid[17]-salience*))
   ?level <- (technique ?cont braid[17])
=>
...
(defrule braid[17]
   (declare (salience ?*braid[17]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L17* ?*print-braid* ?*print-braid-17*) then
      (print-braid 17 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[16]
   (declare (salience ?*partial-braid[16]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[17])
      ;;; because braids[17] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[16] have been eliminated (because they have higher salience),
