...
(defrule activate-braid[25]
   (declare (salience ?*activate-braid[25]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_B25))
   (assert (technique ?cont braid[25]))
   (bind ?*technique* B[25])
)



(defrule track-braid[25]
   (declare (salience ?*activate-braid[25]-salience*))
   ?level <- (technique ?cont braid[25])
=>
...
(defrule braid[25]
   (declare (salience ?*braid[25]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L25* ?*print-braid* ?*print-braid-25*) then
      (print-braid 25 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[24]
   (declare (salience ?*partial-braid[24]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[25])
      ;;; because braids[25] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[24] have been eliminated (because they have higher salience),
