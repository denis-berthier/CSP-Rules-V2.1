...
(defrule activate-braid[28]
   (declare (salience ?*activate-braid[28]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont braid))
=>
   (if ?*print-levels* then (printout t Entering_level_B28))
   (assert (technique ?cont braid[28]))
   (bind ?*technique* B[28])
)



(defrule track-braid[28]
   (declare (salience ?*activate-braid[28]-salience*))
   ?level <- (technique ?cont braid[28])
=>
...
(defrule braid[28]
   (declare (salience ?*braid[28]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L28* ?*print-braid* ?*print-braid-28*) then
      (print-braid 28 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[27]
   (declare (salience ?*partial-braid[27]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[28])
      ;;; because braids[28] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[27] have been eliminated (because they have higher salience),
