...
(defrule activate-braid[31]
   (declare (salience ?*activate-braid[31]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_B31))
   (assert (technique ?cont braid[31]))
   (bind ?*technique* B[31])
)



(defrule track-braid[31]
   (declare (salience ?*activate-braid[31]-salience*))
   ?level <- (technique ?cont braid[31])
=>
...
(defrule braid[31]
   (declare (salience ?*braid[31]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L31* ?*print-braid* ?*print-braid-31*) then
      (print-braid 31 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[30]
   (declare (salience ?*partial-braid[30]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[31])
      ;;; because braids[31] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[30] have been eliminated (because they have higher salience),
