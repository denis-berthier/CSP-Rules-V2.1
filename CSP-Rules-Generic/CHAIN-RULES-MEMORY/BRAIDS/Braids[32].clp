...
(defrule activate-braid[32]
   (declare (salience ?*activate-braid[32]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_B32))
   (assert (technique ?cont braid[32]))
   (bind ?*technique* B[32])
)



(defrule track-braid[32]
   (declare (salience ?*activate-braid[32]-salience*))
   ?level <- (technique ?cont braid[32])
=>
...
(defrule braid[32]
   (declare (salience ?*braid[32]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L32* ?*print-braid* ?*print-braid-32*) then
      (print-braid 32 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[31]
   (declare (salience ?*partial-braid[31]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[32])
      ;;; because braids[32] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[31] have been eliminated (because they have higher salience),
