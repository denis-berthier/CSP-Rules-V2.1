...
(defrule activate-braid[35]
   (declare (salience ?*activate-braid[35]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont braid))
=>
   (if ?*print-levels* then (printout t Entering_level_B35))
   (assert (technique ?cont braid[35]))
   (bind ?*technique* B[35])
)



(defrule track-braid[35]
   (declare (salience ?*activate-braid[35]-salience*))
   ?level <- (technique ?cont braid[35])
=>
...
(defrule braid[35]
   (declare (salience ?*braid[35]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L35* ?*print-braid* ?*print-braid-35*) then
      (print-braid 35 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[34]
   (declare (salience ?*partial-braid[34]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[35])
      ;;; because braids[35] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[34] have been eliminated (because they have higher salience),
