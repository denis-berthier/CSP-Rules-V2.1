...
(defrule activate-g-bivalue-chain[10]
   (declare (salience ?*activate-g-bivalue-chain[10]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gBC10))
   (assert (technique ?cont g-bivalue-chain[10]))
   (bind ?*technique* gBC[10])
)



(defrule track-g-bivalue-chain[10]
   (declare (salience ?*activate-g-bivalue-chain[10]-salience*))
   ?level <- (technique ?cont g-bivalue-chain[10])
=>
...
(defrule g-bivalue-chain[10]
   (declare (salience ?*g-bivalue-chain[10]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L10* ?*print-g-bivalue-chain* ?*print-g-bivalue-chain-10*) then
      (print-g-bivalue-chain 10 ?zzz $?llcs $?rlcs $?csp-vars)
   )
)
...
(defrule partial-g-bivalue-chain[10]-1
   (declare (salience ?*partial-g-bivalue-chain[10]-salience-1*))
   (logical
      (chain
         (type g-bivalue-chain)
         (context ?cont)
      ...
      ;;; ?new-llc
      (technique ?cont g-bivalue-chain[10])
      (or
...
(defrule partial-g-bivalue-chain[10]-2
   (declare (salience ?*partial-g-bivalue-chain[10]-salience-2*))
   (logical
      (chain
