...
(defrule activate-g-bivalue-chain[11]
   (declare (salience ?*activate-g-bivalue-chain[11]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gBC11))
   (assert (technique ?cont g-bivalue-chain[11]))
   (bind ?*technique* gBC[11])
)



(defrule track-g-bivalue-chain[11]
   (declare (salience ?*activate-g-bivalue-chain[11]-salience*))
   ?level <- (technique ?cont g-bivalue-chain[11])
=>
...
(defrule g-bivalue-chain[11]
   (declare (salience ?*g-bivalue-chain[11]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L11* ?*print-g-bivalue-chain* ?*print-g-bivalue-chain-11*) then
      (print-g-bivalue-chain 11 ?zzz $?llcs $?rlcs $?csp-vars)
   )
)
...
(defrule partial-g-bivalue-chain[11]-1
   (declare (salience ?*partial-g-bivalue-chain[11]-salience-1*))
   (logical
      (chain
         (type g-bivalue-chain)
         (context ?cont)
      ...
      ;;; ?new-llc
      (technique ?cont g-bivalue-chain[11])
      (or
...
(defrule partial-g-bivalue-chain[11]-2
   (declare (salience ?*partial-g-bivalue-chain[11]-salience-2*))
   (logical
      (chain
