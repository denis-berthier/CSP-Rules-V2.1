...
(defrule activate-g-bivalue-chain[12]
   (declare (salience ?*activate-g-bivalue-chain[12]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont g-bivalue-chain))
=>
   (if ?*print-levels* then (printout t Entering_level_gBC12))
   (assert (technique ?cont g-bivalue-chain[12]))
   (bind ?*technique* gBC[12])
)



(defrule track-g-bivalue-chain[12]
   (declare (salience ?*activate-g-bivalue-chain[12]-salience*))
   ?level <- (technique ?cont g-bivalue-chain[12])
=>
...
(defrule g-bivalue-chain[12]
   (declare (salience ?*g-bivalue-chain[12]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L12* ?*print-g-bivalue-chain* ?*print-g-bivalue-chain-12*) then
      (print-g-bivalue-chain 12 ?zzz $?llcs $?rlcs $?csp-vars)
   )
)
...
(defrule partial-g-bivalue-chain[12]-1
   (declare (salience ?*partial-g-bivalue-chain[12]-salience-1*))
   (logical
      (chain
         (type g-bivalue-chain)
         (context ?cont)
      ...
      ;;; ?new-llc
      (technique ?cont g-bivalue-chain[12])
      (or
...
(defrule partial-g-bivalue-chain[12]-2
   (declare (salience ?*partial-g-bivalue-chain[12]-salience-2*))
   (logical
      (chain
