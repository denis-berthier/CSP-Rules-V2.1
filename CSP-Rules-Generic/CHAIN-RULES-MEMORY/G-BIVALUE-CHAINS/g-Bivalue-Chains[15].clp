...
(defrule activate-g-bivalue-chain[15]
   (declare (salience ?*activate-g-bivalue-chain[15]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gBC15))
   (assert (technique ?cont g-bivalue-chain[15]))
   (bind ?*technique* gBC[15])
)



(defrule track-g-bivalue-chain[15]
   (declare (salience ?*activate-g-bivalue-chain[15]-salience*))
   ?level <- (technique ?cont g-bivalue-chain[15])
=>
...
(defrule g-bivalue-chain[15]
   (declare (salience ?*g-bivalue-chain[15]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L15* ?*print-g-bivalue-chain* ?*print-g-bivalue-chain-15*) then
      (print-g-bivalue-chain 15 ?zzz $?llcs $?rlcs $?csp-vars)
   )
)
...
(defrule partial-g-bivalue-chain[15]-1
   (declare (salience ?*partial-g-bivalue-chain[15]-salience-1*))
   (logical
      (chain
         (type g-bivalue-chain)
         (context ?cont)
      ...
      ;;; ?new-llc
      (technique ?cont g-bivalue-chain[15])
      (or
...
(defrule partial-g-bivalue-chain[15]-2
   (declare (salience ?*partial-g-bivalue-chain[15]-salience-2*))
   (logical
      (chain
