...
(defrule activate-g-bivalue-chain[16]
   (declare (salience ?*activate-g-bivalue-chain[16]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gBC16))
   (assert (technique ?cont g-bivalue-chain[16]))
   (bind ?*technique* gBC[16])
)



(defrule track-g-bivalue-chain[16]
   (declare (salience ?*activate-g-bivalue-chain[16]-salience*))
   ?level <- (technique ?cont g-bivalue-chain[16])
=>
...
(defrule g-bivalue-chain[16]
   (declare (salience ?*g-bivalue-chain[16]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L16* ?*print-g-bivalue-chain* ?*print-g-bivalue-chain-16*) then
      (print-g-bivalue-chain 16 ?zzz $?llcs $?rlcs $?csp-vars)
   )
)
...
(defrule partial-g-bivalue-chain[16]-1
   (declare (salience ?*partial-g-bivalue-chain[16]-salience-1*))
   (logical
      (chain
         (type g-bivalue-chain)
         (context ?cont)
      ...
      ;;; ?new-llc
      (technique ?cont g-bivalue-chain[16])
      (or
...
(defrule partial-g-bivalue-chain[16]-2
   (declare (salience ?*partial-g-bivalue-chain[16]-salience-2*))
   (logical
      (chain
