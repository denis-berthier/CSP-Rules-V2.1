...
(defrule activate-g-bivalue-chain[19]
   (declare (salience ?*activate-g-bivalue-chain[19]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gBC19))
   (assert (technique ?cont g-bivalue-chain[19]))
   (bind ?*technique* gBC[19])
)



(defrule track-g-bivalue-chain[19]
   (declare (salience ?*activate-g-bivalue-chain[19]-salience*))
   ?level <- (technique ?cont g-bivalue-chain[19])
=>
...
(defrule g-bivalue-chain[19]
   (declare (salience ?*g-bivalue-chain[19]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L19* ?*print-g-bivalue-chain* ?*print-g-bivalue-chain-19*) then
      (print-g-bivalue-chain 19 ?zzz $?llcs $?rlcs $?csp-vars)
   )
)
...
(defrule partial-g-bivalue-chain[19]-1
   (declare (salience ?*partial-g-bivalue-chain[19]-salience-1*))
   (logical
      (chain
         (type g-bivalue-chain)
         (context ?cont)
      ...
      ;;; ?new-llc
      (technique ?cont g-bivalue-chain[19])
      (or
...
(defrule partial-g-bivalue-chain[19]-2
   (declare (salience ?*partial-g-bivalue-chain[19]-salience-2*))
   (logical
      (chain
