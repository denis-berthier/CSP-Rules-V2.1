...
(defrule activate-g-bivalue-chain[3]
   (declare (salience ?*activate-g-bivalue-chain[3]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gBC3))
   (assert (technique ?cont g-bivalue-chain[3]))
   (bind ?*technique* gBC[3])
)



(defrule track-g-bivalue-chain[3]
   (declare (salience ?*activate-g-bivalue-chain[3]-salience*))
   ?level <- (technique ?cont g-bivalue-chain[3])
=>
...
(defrule g-bivalue-chain[3]
   (declare (salience ?*g-bivalue-chain[3]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L3* ?*print-g-bivalue-chain* ?*print-g-bivalue-chain-3*) then
      (print-g-bivalue-chain 3 ?zzz $?llcs $?rlcs $?csp-vars)
   )
)
...
(defrule partial-g-bivalue-chain[3]-1
   (declare (salience ?*partial-g-bivalue-chain[3]-salience-1*))
   (logical
      (chain
         (type g-bivalue-chain)
         (context ?cont)
      ...
      ;;; ?new-llc
      (technique ?cont g-bivalue-chain[3])
      (or
...
(defrule partial-g-bivalue-chain[3]-2
   (declare (salience ?*partial-g-bivalue-chain[3]-salience-2*))
   (logical
      (chain
