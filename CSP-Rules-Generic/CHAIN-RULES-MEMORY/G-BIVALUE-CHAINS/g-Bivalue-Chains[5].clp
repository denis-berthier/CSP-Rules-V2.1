...
(defrule activate-g-bivalue-chain[5]
   (declare (salience ?*activate-g-bivalue-chain[5]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont g-bivalue-chain))
=>
   (if ?*print-levels* then (printout t Entering_level_gBC5))
   (assert (technique ?cont g-bivalue-chain[5]))
   (bind ?*technique* gBC[5])
)



(defrule track-g-bivalue-chain[5]
   (declare (salience ?*activate-g-bivalue-chain[5]-salience*))
   ?level <- (technique ?cont g-bivalue-chain[5])
=>
...
(defrule g-bivalue-chain[5]
   (declare (salience ?*g-bivalue-chain[5]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L5* ?*print-g-bivalue-chain* ?*print-g-bivalue-chain-5*) then
      (print-g-bivalue-chain 5 ?zzz $?llcs $?rlcs $?csp-vars)
   )
)
...
(defrule partial-g-bivalue-chain[5]-1
   (declare (salience ?*partial-g-bivalue-chain[5]-salience-1*))
   (logical
      (chain
         (type g-bivalue-chain)
         (context ?cont)
      ...
      ;;; ?new-llc
      (technique ?cont g-bivalue-chain[5])
      (or
...
(defrule partial-g-bivalue-chain[5]-2
   (declare (salience ?*partial-g-bivalue-chain[5]-salience-2*))
   (logical
      (chain
