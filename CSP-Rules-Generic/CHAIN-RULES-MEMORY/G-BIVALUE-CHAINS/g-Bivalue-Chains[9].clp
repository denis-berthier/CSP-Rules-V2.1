...
=>
   ...
   (bind ?*technique* gBC[9])
)



(defrule track-g-bivalue-chain[9]
   (declare (salience ?*activate-g-bivalue-chain[9]-salience*))
   ?level <- (technique ?cont g-bivalue-chain[9])
=>
...
(defrule g-bivalue-chain[9]
   (declare (salience ?*g-bivalue-chain[9]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L9* ?*print-g-bivalue-chain* ?*print-g-bivalue-chain-9*) then
      (print-g-bivalue-chain 9 ?zzz $?llcs $?rlcs $?csp-vars)
   )
)
...
(defrule partial-g-bivalue-chain[9]-1
   (declare (salience ?*partial-g-bivalue-chain[9]-salience-1*))
   (logical
      (chain
         (type g-bivalue-chain)
         (context ?cont)
      ...
      ;;; ?new-llc
      (technique ?cont g-bivalue-chain[9])
      (or
...
(defrule partial-g-bivalue-chain[9]-2
