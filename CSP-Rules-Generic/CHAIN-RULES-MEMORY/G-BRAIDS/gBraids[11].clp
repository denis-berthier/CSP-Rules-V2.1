...
(defrule activate-gbraid[11]
   (declare (salience ?*activate-gbraid[11]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gB11))
   (assert (technique ?cont gbraid[11]))
   (bind ?*technique* gB[11])
)



(defrule track-gbraid[11]
   (declare (salience ?*activate-gbraid[11]-salience*))
   ?level <- (technique ?cont gbraid[11])
=>
...
(defrule gbraid[11]
   (declare (salience ?*gbraid[11]-salience*))
   (chain
      (type partial-gbraid)
      (context ?cont)
      (length 10)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L11* ?*print-gbraid* ?*print-gbraid-11*) then
      (print-gbraid 11 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gbraid{10]-1
   (declare (salience ?*partial-gbraid[10]-salience-1*))
   (logical
      (chain
...
(defrule partial-gbraid[10]-2
   (declare (salience ?*partial-gbraid[10]-salience-2*))
   (logical
      (chain
...
(defrule partial-gbraid[10]-3
   (declare (salience ?*partial-gbraid[10]-salience-3*))
   (logical
      (chain
         (type partial-gwhip|partial-gbraid)
         (context ?cont)
