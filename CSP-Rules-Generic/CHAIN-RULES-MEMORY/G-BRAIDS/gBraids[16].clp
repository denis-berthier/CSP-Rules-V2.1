...
(defrule activate-gbraid[16]
   (declare (salience ?*activate-gbraid[16]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gB16))
   (assert (technique ?cont gbraid[16]))
   (bind ?*technique* gB[16])
)



(defrule track-gbraid[16]
   (declare (salience ?*activate-gbraid[16]-salience*))
   ?level <- (technique ?cont gbraid[16])
=>
...
(defrule gbraid[16]
   (declare (salience ?*gbraid[16]-salience*))
   (chain
      (type partial-gbraid)
      (context ?cont)
      (length 15)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L16* ?*print-gbraid* ?*print-gbraid-16*) then
      (print-gbraid 16 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gbraid{15]-1
   (declare (salience ?*partial-gbraid[15]-salience-1*))
   (logical
      (chain
...
(defrule partial-gbraid[15]-2
   (declare (salience ?*partial-gbraid[15]-salience-2*))
   (logical
      (chain
...
(defrule partial-gbraid[15]-3
   (declare (salience ?*partial-gbraid[15]-salience-3*))
   (logical
      (chain
         (type partial-gwhip|partial-gbraid)
         (context ?cont)
