...
(defrule activate-gbraid[20]
   (declare (salience ?*activate-gbraid[20]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gB20))
   (assert (technique ?cont gbraid[20]))
   (bind ?*technique* gB[20])
)



(defrule track-gbraid[20]
   (declare (salience ?*activate-gbraid[20]-salience*))
   ?level <- (technique ?cont gbraid[20])
=>
...
(defrule gbraid[20]
   (declare (salience ?*gbraid[20]-salience*))
   (chain
      (type partial-gbraid)
      (context ?cont)
      (length 19)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L20* ?*print-gbraid* ?*print-gbraid-20*) then
      (print-gbraid 20 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gbraid{19]-1
   (declare (salience ?*partial-gbraid[19]-salience-1*))
   (logical
      (chain
...
(defrule partial-gbraid[19]-2
   (declare (salience ?*partial-gbraid[19]-salience-2*))
   (logical
      (chain
...
(defrule partial-gbraid[19]-3
   (declare (salience ?*partial-gbraid[19]-salience-3*))
   (logical
      (chain
         (type partial-gwhip|partial-gbraid)
         (context ?cont)
