...
(defrule activate-gbraid[21]
   (declare (salience ?*activate-gbraid[21]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont gbraid))
=>
   (if ?*print-levels* then (printout t Entering_level_gB21))
   (assert (technique ?cont gbraid[21]))
   (bind ?*technique* gB[21])
)



(defrule track-gbraid[21]
   (declare (salience ?*activate-gbraid[21]-salience*))
   ?level <- (technique ?cont gbraid[21])
=>
...
(defrule gbraid[21]
   (declare (salience ?*gbraid[21]-salience*))
   (chain
      (type partial-gbraid)
      (context ?cont)
      (length 20)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L21* ?*print-gbraid* ?*print-gbraid-21*) then
      (print-gbraid 21 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gbraid{20]-1
   (declare (salience ?*partial-gbraid[20]-salience-1*))
   (logical
      (chain
...
(defrule partial-gbraid[20]-2
   (declare (salience ?*partial-gbraid[20]-salience-2*))
   (logical
      (chain
...
(defrule partial-gbraid[20]-3
   (declare (salience ?*partial-gbraid[20]-salience-3*))
   (logical
      (chain
         (type partial-gwhip|partial-gbraid)
         (context ?cont)
