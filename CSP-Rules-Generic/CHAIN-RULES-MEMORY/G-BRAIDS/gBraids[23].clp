...
(defrule activate-gbraid[23]
   (declare (salience ?*activate-gbraid[23]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gB23))
   (assert (technique ?cont gbraid[23]))
   (bind ?*technique* gB[23])
)



(defrule track-gbraid[23]
   (declare (salience ?*activate-gbraid[23]-salience*))
   ?level <- (technique ?cont gbraid[23])
=>
...
(defrule gbraid[23]
   (declare (salience ?*gbraid[23]-salience*))
   (chain
      (type partial-gbraid)
      (context ?cont)
      (length 22)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L23* ?*print-gbraid* ?*print-gbraid-23*) then
      (print-gbraid 23 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gbraid{22]-1
   (declare (salience ?*partial-gbraid[22]-salience-1*))
   (logical
      (chain
...
(defrule partial-gbraid[22]-2
   (declare (salience ?*partial-gbraid[22]-salience-2*))
   (logical
      (chain
...
(defrule partial-gbraid[22]-3
   (declare (salience ?*partial-gbraid[22]-salience-3*))
   (logical
      (chain
         (type partial-gwhip|partial-gbraid)
         (context ?cont)
