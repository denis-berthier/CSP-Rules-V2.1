...
(defrule activate-gbraid[26]
   (declare (salience ?*activate-gbraid[26]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gB26))
   (assert (technique ?cont gbraid[26]))
   (bind ?*technique* gB[26])
)



(defrule track-gbraid[26]
   (declare (salience ?*activate-gbraid[26]-salience*))
   ?level <- (technique ?cont gbraid[26])
=>
...
(defrule gbraid[26]
   (declare (salience ?*gbraid[26]-salience*))
   (chain
      (type partial-gbraid)
      (context ?cont)
      (length 25)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L26* ?*print-gbraid* ?*print-gbraid-26*) then
      (print-gbraid 26 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gbraid{25]-1
   (declare (salience ?*partial-gbraid[25]-salience-1*))
   (logical
      (chain
...
(defrule partial-gbraid[25]-2
   (declare (salience ?*partial-gbraid[25]-salience-2*))
   (logical
      (chain
...
(defrule partial-gbraid[25]-3
   (declare (salience ?*partial-gbraid[25]-salience-3*))
   (logical
      (chain
         (type partial-gwhip|partial-gbraid)
         (context ?cont)
