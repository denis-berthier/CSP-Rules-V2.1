...
(defrule activate-gbraid[30]
   (declare (salience ?*activate-gbraid[30]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gB30))
   (assert (technique ?cont gbraid[30]))
   (bind ?*technique* gB[30])
)



(defrule track-gbraid[30]
   (declare (salience ?*activate-gbraid[30]-salience*))
   ?level <- (technique ?cont gbraid[30])
=>
...
(defrule gbraid[30]
   (declare (salience ?*gbraid[30]-salience*))
   (chain
      (type partial-gbraid)
      (context ?cont)
      (length 29)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L30* ?*print-gbraid* ?*print-gbraid-30*) then
      (print-gbraid 30 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gbraid{29]-1
   (declare (salience ?*partial-gbraid[29]-salience-1*))
   (logical
      (chain
...
(defrule partial-gbraid[29]-2
   (declare (salience ?*partial-gbraid[29]-salience-2*))
   (logical
      (chain
...
(defrule partial-gbraid[29]-3
   (declare (salience ?*partial-gbraid[29]-salience-3*))
   (logical
      (chain
         (type partial-gwhip|partial-gbraid)
         (context ?cont)
