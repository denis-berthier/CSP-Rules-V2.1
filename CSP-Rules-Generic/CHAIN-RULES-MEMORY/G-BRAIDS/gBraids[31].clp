...
(defrule activate-gbraid[31]
   (declare (salience ?*activate-gbraid[31]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont gbraid))
=>
   (if ?*print-levels* then (printout t Entering_level_gB31))
   (assert (technique ?cont gbraid[31]))
   (bind ?*technique* gB[31])
)



(defrule track-gbraid[31]
   (declare (salience ?*activate-gbraid[31]-salience*))
   ?level <- (technique ?cont gbraid[31])
=>
...
(defrule gbraid[31]
   (declare (salience ?*gbraid[31]-salience*))
   (chain
      (type partial-gbraid)
      (context ?cont)
      (length 30)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L31* ?*print-gbraid* ?*print-gbraid-31*) then
      (print-gbraid 31 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gbraid{30]-1
   (declare (salience ?*partial-gbraid[30]-salience-1*))
   (logical
      (chain
...
(defrule partial-gbraid[30]-2
   (declare (salience ?*partial-gbraid[30]-salience-2*))
   (logical
      (chain
...
(defrule partial-gbraid[30]-3
   (declare (salience ?*partial-gbraid[30]-salience-3*))
   (logical
      (chain
         (type partial-gwhip|partial-gbraid)
         (context ?cont)
