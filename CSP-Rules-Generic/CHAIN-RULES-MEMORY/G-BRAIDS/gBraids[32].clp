...
(defrule activate-gbraid[32]
   (declare (salience ?*activate-gbraid[32]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gB32))
   (assert (technique ?cont gbraid[32]))
   (bind ?*technique* gB[32])
)



(defrule track-gbraid[32]
   (declare (salience ?*activate-gbraid[32]-salience*))
   ?level <- (technique ?cont gbraid[32])
=>
...
(defrule gbraid[32]
   (declare (salience ?*gbraid[32]-salience*))
   (chain
      (type partial-gbraid)
      (context ?cont)
      (length 31)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L32* ?*print-gbraid* ?*print-gbraid-32*) then
      (print-gbraid 32 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gbraid{31]-1
   (declare (salience ?*partial-gbraid[31]-salience-1*))
   (logical
      (chain
...
(defrule partial-gbraid[31]-2
   (declare (salience ?*partial-gbraid[31]-salience-2*))
   (logical
      (chain
...
(defrule partial-gbraid[31]-3
   (declare (salience ?*partial-gbraid[31]-salience-3*))
   (logical
      (chain
         (type partial-gwhip|partial-gbraid)
         (context ?cont)
