...
(defrule activate-gbraid[8]
   (declare (salience ?*activate-gbraid[8]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gB8))
   (assert (technique ?cont gbraid[8]))
   (bind ?*technique* gB[8])
)



(defrule track-gbraid[8]
   (declare (salience ?*activate-gbraid[8]-salience*))
   ?level <- (technique ?cont gbraid[8])
=>
...
(defrule gbraid[8]
   (declare (salience ?*gbraid[8]-salience*))
   (chain
      (type partial-gbraid)
      (context ?cont)
      (length 7)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L8* ?*print-gbraid* ?*print-gbraid-8*) then
      (print-gbraid 8 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gbraid{7]-1
   (declare (salience ?*partial-gbraid[7]-salience-1*))
   (logical
      (chain
...
(defrule partial-gbraid[7]-2
   (declare (salience ?*partial-gbraid[7]-salience-2*))
   (logical
      (chain
...
(defrule partial-gbraid[7]-3
   (declare (salience ?*partial-gbraid[7]-salience-3*))
   (logical
      (chain
         (type partial-gwhip|partial-gbraid)
         (context ?cont)
