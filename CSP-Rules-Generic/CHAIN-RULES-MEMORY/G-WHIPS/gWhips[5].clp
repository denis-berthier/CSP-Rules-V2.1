...
(defrule activate-gwhip[5]
   (declare (salience ?*activate-gwhip[5]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gW5))
   (assert (technique ?cont gwhip[5]))
   (bind ?*technique* gW[5])
)



(defrule track-gwhip[5]
   (declare (salience ?*activate-gwhip[5]-salience*))
   ?level <- (technique ?cont gwhip[5])
=>
...
(defrule gwhip[5]
   (declare (salience ?*gwhip[5]-salience*))
   (chain
      (type partial-gwhip)
      (context ?cont)
      (length 4)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L5* ?*print-gwhip* ?*print-gwhip-5*) then
      (print-gwhip 5 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gwhip[4]-1
   (declare (salience ?*partial-gwhip[4]-salience-1*))
   (logical
      (chain
         (type partial-gwhip)
         (context ?cont)
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont gwhip[5])
      ;;; because gwhips[4] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as gwhips[4] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule partial-gwhip[4]-2
   (declare (salience ?*partial-gwhip[4]-salience-2*))
   (logical
      (chain
         (type partial-whip)
         (context ?cont)
   ...
   ;;; as gwhips[4] based on ?zzz have been eliminated (because they have higher salience),
   ;;; and gwhips[3] extendable by a single candidate have already been generated (because they have higher salience),
...
(defrule partial-gwhip[4]-3
   (declare (salience ?*partial-gwhip[4]-salience-3*))
   (logical
      (chain
         (type partial-gwhip)
         (context ?cont)
