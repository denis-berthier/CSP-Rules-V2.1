...
(defrule activate-gwhip[9]
   (declare (salience ?*activate-gwhip[9]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont gwhip))
=>
   (if ?*print-levels* then (printout t Entering_level_gW9))
   (assert (technique ?cont gwhip[9]))
   (bind ?*technique* gW[9])
)



(defrule track-gwhip[9]
   (declare (salience ?*activate-gwhip[9]-salience*))
   ?level <- (technique ?cont gwhip[9])
=>
...
(defrule gwhip[9]
   (declare (salience ?*gwhip[9]-salience*))
   (chain
      (type partial-gwhip)
      (context ?cont)
      (length 8)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L9* ?*print-gwhip* ?*print-gwhip-9*) then
      (print-gwhip 9 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gwhip[8]-1
   (declare (salience ?*partial-gwhip[8]-salience-1*))
   (logical
      (chain
         (type partial-gwhip)
         (context ?cont)
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont gwhip[9])
      ;;; because gwhips[8] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as gwhips[8] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule partial-gwhip[8]-2
   (declare (salience ?*partial-gwhip[8]-salience-2*))
   (logical
      (chain
         (type partial-whip)
         (context ?cont)
   ...
   ;;; as gwhips[8] based on ?zzz have been eliminated (because they have higher salience),
   ;;; and gwhips[7] extendable by a single candidate have already been generated (because they have higher salience),
...
(defrule partial-gwhip[8]-3
   (declare (salience ?*partial-gwhip[8]-salience-3*))
   (logical
      (chain
         (type partial-gwhip)
         (context ?cont)
