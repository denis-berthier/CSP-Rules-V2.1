...
(defrule activate-g2whip[10]
   (declare (salience ?*activate-g2whip[10]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont g2whip))
=>
   (if ?*print-levels* then (printout t Entering_level_g2W10))
   (assert (technique ?cont g2whip[10]))
   (bind ?*technique* g2W[10])
)



(defrule track-g2whip[10]
   (declare (salience ?*activate-g2whip[10]-salience*))
   ?level <- (technique ?cont g2whip[10])
=>
...
(defrule g2whip[10]
   (declare (salience ?*g2whip[10]-salience*))
   (chain2r
      (type partial-g2whip)
      (context ?cont)
      (length 9)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L10* ?*print-g2whip* ?*print-g2whip-10*) then
      (print-g2whip 10 ?zzz $?llcs $?rlcsa $?rlcsb $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-g2whip[9]-1
   (declare (salience ?*partial-g2whip[9]-salience-1*))
   (logical
      (chain2r
         (type partial-g2whip)
         (context ?cont)
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont g2whip[10])
      ;;; because whips[9] and g2-whips[9] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as g2-whips[9] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule partial-g2whip[9]-2
   (declare (salience ?*partial-g2whip[9]-salience-2*))
   (logical
      (chain
         (type partial-whip)
         (context ?cont)
   ...
   ;;; as g2whips[9] based on ?zzz have been eliminated (because they have higher salience),
   ;;; and g2whips[8] extendable by a single candidate have already been generated (because they have higher salience),
   ...
   (not
      (chain2r
         ...
         (target ?zzz)
         (rlcsa $?rlcs ?new-rlca)
         (rlcsb $?seq&:(eq $?seq (create-list-of-0s 8)) ?new-rlcb)
      )
   )
=>
   (assert
      (chain2r
         ...
         (llcs $?llcs ?new-llc)
         (rlcsa $?rlcs ?new-rlca)
         (rlcsb (create-list-of-0s 8) ?new-rlcb)
         (csp-vars $?csp-vars ?new-csp)
         (last-rlca ?new-rlca)
         (last-rlcb ?new-rlcb)
      )
...
(defrule partial-g2whip[9]-3
   (declare (salience ?*partial-g2whip[9]-salience-3*))
   (logical
      (chain2r
         (type partial-g2whip)
         (context ?cont)
