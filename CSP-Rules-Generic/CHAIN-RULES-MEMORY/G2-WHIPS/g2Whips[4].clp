...
(defrule activate-g2whip[4]
   (declare (salience ?*activate-g2whip[4]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_g2W4))
   (assert (technique ?cont g2whip[4]))
   (bind ?*technique* g2W[4])
)



(defrule track-g2whip[4]
   (declare (salience ?*activate-g2whip[4]-salience*))
   ?level <- (technique ?cont g2whip[4])
=>
...
(defrule g2whip[4]
   (declare (salience ?*g2whip[4]-salience*))
   (chain2r
      (type partial-g2whip)
      (context ?cont)
      (length 3)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L4* ?*print-g2whip* ?*print-g2whip-4*) then
      (print-g2whip 4 ?zzz $?llcs $?rlcsa $?rlcsb $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-g2whip[3]-1
   (declare (salience ?*partial-g2whip[3]-salience-1*))
   (logical
      (chain2r
         (type partial-g2whip)
         (context ?cont)
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont g2whip[4])
      ;;; because whips[3] and g2-whips[3] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as g2-whips[3] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule partial-g2whip[3]-2
   (declare (salience ?*partial-g2whip[3]-salience-2*))
   (logical
      (chain
         (type partial-whip)
         (context ?cont)
   ...
   ;;; as g2whips[3] based on ?zzz have been eliminated (because they have higher salience),
   ;;; and g2whips[2] extendable by a single candidate have already been generated (because they have higher salience),
   ...
   (not
      (chain2r
         ...
         (target ?zzz)
         (rlcsa $?rlcs ?new-rlca)
         (rlcsb $?seq&:(eq $?seq (create-list-of-0s 2)) ?new-rlcb)
      )
   )
=>
   (assert
      (chain2r
         ...
         (llcs $?llcs ?new-llc)
         (rlcsa $?rlcs ?new-rlca)
         (rlcsb (create-list-of-0s 2) ?new-rlcb)
         (csp-vars $?csp-vars ?new-csp)
         (last-rlca ?new-rlca)
         (last-rlcb ?new-rlcb)
      )
...
(defrule partial-g2whip[3]-3
   (declare (salience ?*partial-g2whip[3]-salience-3*))
   (logical
      (chain2r
         (type partial-g2whip)
         (context ?cont)
