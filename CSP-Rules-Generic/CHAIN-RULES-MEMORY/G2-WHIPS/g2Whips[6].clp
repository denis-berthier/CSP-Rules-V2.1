...
(defrule activate-g2whip[6]
   (declare (salience ?*activate-g2whip[6]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_g2W6))
   (assert (technique ?cont g2whip[6]))
   (bind ?*technique* g2W[6])
)



(defrule track-g2whip[6]
   (declare (salience ?*activate-g2whip[6]-salience*))
   ?level <- (technique ?cont g2whip[6])
=>
...
(defrule g2whip[6]
   (declare (salience ?*g2whip[6]-salience*))
   (chain2r
      (type partial-g2whip)
      (context ?cont)
      (length 5)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L6* ?*print-g2whip* ?*print-g2whip-6*) then
      (print-g2whip 6 ?zzz $?llcs $?rlcsa $?rlcsb $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-g2whip[5]-1
   (declare (salience ?*partial-g2whip[5]-salience-1*))
   (logical
      (chain2r
         (type partial-g2whip)
         (context ?cont)
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont g2whip[6])
      ;;; because whips[5] and g2-whips[5] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as g2-whips[5] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule partial-g2whip[5]-2
   (declare (salience ?*partial-g2whip[5]-salience-2*))
   (logical
      (chain
         (type partial-whip)
         (context ?cont)
   ...
   ;;; as g2whips[5] based on ?zzz have been eliminated (because they have higher salience),
   ;;; and g2whips[4] extendable by a single candidate have already been generated (because they have higher salience),
   ...
   (not
      (chain2r
         ...
         (target ?zzz)
         (rlcsa $?rlcs ?new-rlca)
         (rlcsb $?seq&:(eq $?seq (create-list-of-0s 4)) ?new-rlcb)
      )
   )
=>
   (assert
      (chain2r
         ...
         (llcs $?llcs ?new-llc)
         (rlcsa $?rlcs ?new-rlca)
         (rlcsb (create-list-of-0s 4) ?new-rlcb)
         (csp-vars $?csp-vars ?new-csp)
         (last-rlca ?new-rlca)
         (last-rlcb ?new-rlcb)
      )
...
(defrule partial-g2whip[5]-3
   (declare (salience ?*partial-g2whip[5]-salience-3*))
   (logical
      (chain2r
         (type partial-g2whip)
         (context ?cont)
