...
(defrule activate-g2whip[8]
   (declare (salience ?*activate-g2whip[8]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont g2whip))
=>
   (if ?*print-levels* then (printout t Entering_level_g2W8))
   (assert (technique ?cont g2whip[8]))
   (bind ?*technique* g2W[8])
)



(defrule track-g2whip[8]
   (declare (salience ?*activate-g2whip[8]-salience*))
   ?level <- (technique ?cont g2whip[8])
=>
...
(defrule g2whip[8]
   (declare (salience ?*g2whip[8]-salience*))
   (chain2r
      (type partial-g2whip)
      (context ?cont)
      (length 7)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L8* ?*print-g2whip* ?*print-g2whip-8*) then
      (print-g2whip 8 ?zzz $?llcs $?rlcsa $?rlcsb $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-g2whip[7]-1
   (declare (salience ?*partial-g2whip[7]-salience-1*))
   (logical
      (chain2r
         (type partial-g2whip)
         (context ?cont)
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont g2whip[8])
      ;;; because whips[7] and g2-whips[7] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as g2-whips[7] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule partial-g2whip[7]-2
   (declare (salience ?*partial-g2whip[7]-salience-2*))
   (logical
      (chain
         (type partial-whip)
         (context ?cont)
   ...
   ;;; as g2whips[7] based on ?zzz have been eliminated (because they have higher salience),
   ;;; and g2whips[6] extendable by a single candidate have already been generated (because they have higher salience),
   ...
   (not
      (chain2r
         ...
         (target ?zzz)
         (rlcsa $?rlcs ?new-rlca)
         (rlcsb $?seq&:(eq $?seq (create-list-of-0s 6)) ?new-rlcb)
      )
   )
=>
   (assert
      (chain2r
         ...
         (llcs $?llcs ?new-llc)
         (rlcsa $?rlcs ?new-rlca)
         (rlcsb (create-list-of-0s 6) ?new-rlcb)
         (csp-vars $?csp-vars ?new-csp)
         (last-rlca ?new-rlca)
         (last-rlcb ?new-rlcb)
      )
...
(defrule partial-g2whip[7]-3
   (declare (salience ?*partial-g2whip[7]-salience-3*))
   (logical
      (chain2r
         (type partial-g2whip)
         (context ?cont)
