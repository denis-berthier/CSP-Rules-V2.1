...
=>
   (if ?*print-levels* then (printout t Entering_level_TyZ11))
   (assert (technique ?cont typed-z-chain[11]))
   (assert (technique ?cont typed-partial-z-chain[10]))
   (bind ?*technique* typed-z-chain[11])
)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L11* ?*print-typed-z-chain* ?*print-typed-z-chain-11*) then
      (print-typed-z-chain ?csp-type 11 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
