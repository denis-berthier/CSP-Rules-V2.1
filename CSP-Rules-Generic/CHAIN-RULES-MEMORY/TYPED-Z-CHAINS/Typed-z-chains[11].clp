...
(defrule activate-typed-z-chain[11]
   (declare (salience ?*activate-typed-z-chain[11]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   ...
   (assert (technique ?cont typed-z-chain[11]))
   (assert (technique ?cont partial-typed-z-chain[10]))
   (bind ?*technique* TyZ[11])
)



(defrule track-typed-z-chain[11]
   (declare (salience ?*activate-typed-z-chain[11]-salience*))
   ?level <- (technique ?cont typed-z-chain[11])
=>
...
(defrule typed-partial-z-chain[10]
   (declare (salience ?*typed-partial-z-chain[10]-salience*))
   (logical
      (typed-chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont typed-partial-z-chain[10])
      ;;; because typed-z-chains[10] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as typed-z-chains[10] have been eliminated (because they have higher salience),
...
(defrule typed-z-chain[11]
   (declare (salience ?*typed-z-chain[11]-salience*))
   (typed-chain
      (type typed-partial-z-chain)
      (csp-type ?csp-type)
      (context ?cont)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L11* ?*print-typed-z-chain* ?*print-typed-z-chain-11*) then
      (print-typed-z-chain-without-crlf ?csp-type 11 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      ...
      (assert (pseudo-blocked ?cont typed-z-chain[11] ?csp-type ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-typed-z-chain[11]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont typed-z-chain[11] ?csp-type ?zzz ?llc1 $?z-cands)
...
(defrule typed-z-chain[11]-print-z-candidates
