...
=>
   (if ?*print-levels* then (printout t Entering_level_TyZ12))
   (assert (technique ?cont typed-z-chain[12]))
   (assert (technique ?cont typed-partial-z-chain[11]))
   (bind ?*technique* typed-z-chain[12])
)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L12* ?*print-typed-z-chain* ?*print-typed-z-chain-12*) then
      (print-typed-z-chain ?csp-type 12 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
