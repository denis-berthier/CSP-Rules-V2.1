...
=>
   (if ?*print-levels* then (printout t Entering_level_TyZ13))
   (assert (technique ?cont typed-z-chain[13]))
   (assert (technique ?cont typed-partial-z-chain[12]))
   (bind ?*technique* typed-z-chain[13])
)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L13* ?*print-typed-z-chain* ?*print-typed-z-chain-13*) then
      (print-typed-z-chain ?csp-type 13 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
