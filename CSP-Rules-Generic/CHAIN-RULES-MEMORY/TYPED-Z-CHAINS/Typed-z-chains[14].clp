...
=>
   (if ?*print-levels* then (printout t Entering_level_TyZ14))
   (assert (technique ?cont typed-z-chain[14]))
   (assert (technique ?cont typed-partial-z-chain[13]))
   (bind ?*technique* typed-z-chain[14])
)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L14* ?*print-typed-z-chain* ?*print-typed-z-chain-14*) then
      (print-typed-z-chain ?csp-type 14 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
