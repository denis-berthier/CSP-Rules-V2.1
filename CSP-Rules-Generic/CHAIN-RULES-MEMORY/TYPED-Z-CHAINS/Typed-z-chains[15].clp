...
(defrule activate-typed-z-chain[15]
   (declare (salience ?*activate-typed-z-chain[15]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_Z15))
   (assert (technique ?cont typed-z-chain[15]))
   (assert (technique ?cont partial-typed-z-chain[14]))
   (bind ?*technique* TyZ[15])
)



(defrule track-typed-z-chain[15]
   (declare (salience ?*activate-typed-z-chain[15]-salience*))
   ?level <- (technique ?cont typed-z-chain[15])
=>
...
(defrule typed-partial-z-chain[14]
   (declare (salience ?*typed-partial-z-chain[14]-salience*))
   (logical
      (typed-chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont typed-partial-z-chain[14])
      ;;; because typed-z-chains[14] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as typed-z-chains[14] have been eliminated (because they have higher salience),
...
(defrule typed-z-chain[15]
   (declare (salience ?*typed-z-chain[15]-salience*))
   (typed-chain
      (type typed-partial-z-chain)
      (csp-type ?csp-type)
      (context ?cont)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L15* ?*print-typed-z-chain* ?*print-typed-z-chain-15*) then
      (print-typed-z-chain-without-crlf ?csp-type 15 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      ...
      (assert (pseudo-blocked ?cont typed-z-chain[15] ?csp-type ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-typed-z-chain[15]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont typed-z-chain[15] ?csp-type ?zzz ?llc1 $?z-cands)
...
(defrule typed-z-chain[15]-print-z-candidates
