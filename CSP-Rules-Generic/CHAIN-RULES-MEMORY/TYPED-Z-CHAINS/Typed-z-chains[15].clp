...
=>
   (if ?*print-levels* then (printout t Entering_level_TyZ15))
   (assert (technique ?cont typed-z-chain[15]))
   (assert (technique ?cont typed-partial-z-chain[14]))
   (bind ?*technique* typed-z-chain[15])
)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L15* ?*print-typed-z-chain* ?*print-typed-z-chain-15*) then
      (print-typed-z-chain ?csp-type 15 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
