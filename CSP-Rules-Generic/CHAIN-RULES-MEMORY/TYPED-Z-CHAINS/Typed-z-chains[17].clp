...
=>
   (if ?*print-levels* then (printout t Entering_level_TyZ17))
   (assert (technique ?cont typed-z-chain[17]))
   (assert (technique ?cont typed-partial-z-chain[16]))
   (bind ?*technique* typed-z-chain[17])
)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L17* ?*print-typed-z-chain* ?*print-typed-z-chain-17*) then
      (print-typed-z-chain ?csp-type 17 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
