...
=>
   (if ?*print-levels* then (printout t Entering_level_TyZ18))
   (assert (technique ?cont typed-z-chain[18]))
   (assert (technique ?cont typed-partial-z-chain[17]))
   (bind ?*technique* typed-z-chain[18])
)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L18* ?*print-typed-z-chain* ?*print-typed-z-chain-18*) then
      (print-typed-z-chain ?csp-type 18 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
