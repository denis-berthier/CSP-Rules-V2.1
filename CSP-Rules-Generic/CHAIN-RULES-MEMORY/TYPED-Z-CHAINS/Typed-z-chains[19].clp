...
=>
   (if ?*print-levels* then (printout t Entering_level_TyZ19))
   (assert (technique ?cont typed-z-chain[19]))
   (assert (technique ?cont typed-partial-z-chain[18]))
   (bind ?*technique* typed-z-chain[19])
)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L19* ?*print-typed-z-chain* ?*print-typed-z-chain-19*) then
      (print-typed-z-chain ?csp-type 19 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
