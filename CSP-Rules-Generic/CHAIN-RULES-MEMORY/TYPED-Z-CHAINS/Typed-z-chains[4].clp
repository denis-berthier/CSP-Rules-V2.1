...
(defrule activate-typed-z-chain[4]
   (declare (salience ?*activate-typed-z-chain[4]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   ...
   (assert (technique ?cont typed-z-chain[4]))
   (assert (technique ?cont partial-typed-z-chain[3]))
   (bind ?*technique* TyZ[4])
)



(defrule track-typed-z-chain[4]
   (declare (salience ?*activate-typed-z-chain[4]-salience*))
   ?level <- (technique ?cont typed-z-chain[4])
=>
...
(defrule typed-partial-z-chain[3]
   (declare (salience ?*typed-partial-z-chain[3]-salience*))
   (logical
      (typed-chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont typed-partial-z-chain[3])
      ;;; because typed-z-chains[3] based on ?zzz (if any) have already been eliminated,
...
(defrule typed-z-chain[4]
   (declare (salience ?*typed-z-chain[4]-salience*))
   (typed-chain
      (type typed-partial-z-chain)
      (csp-type ?csp-type)
      (context ?cont)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L4* ?*print-typed-z-chain* ?*print-typed-z-chain-4*) then
      (print-typed-z-chain-without-crlf ?csp-type 4 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      ...
      (assert (pseudo-blocked ?cont typed-z-chain[4] ?csp-type ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-typed-z-chain[4]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont typed-z-chain[4] ?csp-type ?zzz ?llc1 $?z-cands)
...
(defrule typed-z-chain[4]-print-z-candidates
