...
(defrule activate-typed-z-chain[6]
   (declare (salience ?*activate-typed-z-chain[6]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_Z6))
   (assert (technique ?cont typed-z-chain[6]))
   (assert (technique ?cont partial-typed-z-chain[5]))
   (bind ?*technique* TyZ[6])
)



(defrule track-typed-z-chain[6]
   (declare (salience ?*activate-typed-z-chain[6]-salience*))
   ?level <- (technique ?cont typed-z-chain[6])
=>
...
(defrule typed-partial-z-chain[5]
   (declare (salience ?*typed-partial-z-chain[5]-salience*))
   (logical
      (typed-chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont typed-partial-z-chain[5])
      ;;; because typed-z-chains[5] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as typed-z-chains[5] have been eliminated (because they have higher salience),
...
(defrule typed-z-chain[6]
   (declare (salience ?*typed-z-chain[6]-salience*))
   (typed-chain
      (type typed-partial-z-chain)
      (csp-type ?csp-type)
      (context ?cont)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L6* ?*print-typed-z-chain* ?*print-typed-z-chain-6*) then
      (print-typed-z-chain-without-crlf ?csp-type 6 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      ...
      (assert (pseudo-blocked ?cont typed-z-chain[6] ?csp-type ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-typed-z-chain[6]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont typed-z-chain[6] ?csp-type ?zzz ?llc1 $?z-cands)
...
(defrule typed-z-chain[6]-print-z-candidates
