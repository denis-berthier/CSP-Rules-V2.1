...
=>
   (if ?*print-levels* then (printout t Entering_level_TyZ8))
   (assert (technique ?cont typed-z-chain[8]))
   (assert (technique ?cont typed-partial-z-chain[7]))
   (bind ?*technique* typed-z-chain[8])
)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L8* ?*print-typed-z-chain* ?*print-typed-z-chain-8*) then
      (print-typed-z-chain ?csp-type 8 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
