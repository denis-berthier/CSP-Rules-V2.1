...
=>
   ...
   (assert (technique ?cont partial-typed-z-chain[8]))
   (bind ?*technique* TyZ[9])
)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L9* ?*print-typed-z-chain* ?*print-typed-z-chain-9*) then
      (print-typed-z-chain-without-crlf ?csp-type 9 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      ...
      (assert (pseudo-blocked ?cont typed-z-chain[9] ?csp-type ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-typed-z-chain[9]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont typed-z-chain[9] ?csp-type ?zzz ?llc1 $?z-cands)
...
(defrule typed-z-chain[9]-print-z-candidates
