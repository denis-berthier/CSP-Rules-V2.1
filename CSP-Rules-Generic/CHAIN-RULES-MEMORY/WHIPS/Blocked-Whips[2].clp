...
(defrule apply-whip[2]-to-more-targets
    ...
    (forall (csp-linked ?cont ?xxx ?rlc1 ?csp1) (test (linked ?xxx ?zzz2)))
    ...
    ; (test (not (linked ?zzz2 ?rlc1)))
    
    ;;; check the conditions for a whip[2]
    (forall (csp-linked ?cont ?yyy ?llc2 ?csp2) (test (linked-or ?yyy ?zzz2 ?rlc1)))
