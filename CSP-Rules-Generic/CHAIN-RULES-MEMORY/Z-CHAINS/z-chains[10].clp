...
(defrule activate-z-chain[10]
   (declare (salience ?*activate-z-chain[10]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_Z10))
   (assert (technique ?cont z-chain[10]))
   (assert (technique ?cont partial-z-chain[9]))
   (bind ?*technique* Z[10])
)



(defrule track-z-chain[10]
   (declare (salience ?*activate-z-chain[10]-salience*))
   ?level <- (technique ?cont z-chain[10])
=>
...
(defrule partial-z-chain[9]
   (declare (salience ?*partial-z-chain[9]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont partial-z-chain[9])
      ;;; because z-chains[9] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as z-chains[9] have been eliminated (because they have higher salience),
...
(defrule z-chain[10]
   (declare (salience ?*z-chain[10]-salience*))
   (chain
      (type partial-z-chain)
      (context ?cont)
      (length 9)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L10* ?*print-z-chain* ?*print-z-chain-10*) then
      (print-z-chain-without-crlf 10 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont z-chain[10] ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-z-chain[10]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont z-chain[10] ?zzz ?llc1 $?z-cands)
...
(defrule z-chain[10]-print-z-candidates
