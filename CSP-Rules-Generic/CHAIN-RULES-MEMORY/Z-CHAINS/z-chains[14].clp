...
(defrule activate-z-chain[14]
   (declare (salience ?*activate-z-chain[14]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   ...
   (assert (technique ?cont z-chain[14]))
   (assert (technique ?cont partial-z-chain[13]))
   (bind ?*technique* Z[14])
)



(defrule track-z-chain[14]
   (declare (salience ?*activate-z-chain[14]-salience*))
   ?level <- (technique ?cont z-chain[14])
=>
...
(defrule partial-z-chain[13]
   (declare (salience ?*partial-z-chain[13]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont partial-z-chain[13])
      ;;; because z-chains[13] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as z-chains[13] have been eliminated (because they have higher salience),
...
(defrule z-chain[14]
   (declare (salience ?*z-chain[14]-salience*))
   (chain
      (type partial-z-chain)
      (context ?cont)
      (length 13)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L14* ?*print-z-chain* ?*print-z-chain-14*) then
      (print-z-chain-without-crlf 14 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont z-chain[14] ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-z-chain[14]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont z-chain[14] ?zzz ?llc1 $?z-cands)
...
(defrule z-chain[14]-print-z-candidates
