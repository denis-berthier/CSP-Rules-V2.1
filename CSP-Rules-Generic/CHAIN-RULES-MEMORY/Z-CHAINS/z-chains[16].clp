...
(defrule activate-z-chain[16]
   (declare (salience ?*activate-z-chain[16]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   ...
   (assert (technique ?cont z-chain[16]))
   (assert (technique ?cont partial-z-chain[15]))
   (bind ?*technique* Z[16])
)



(defrule track-z-chain[16]
   (declare (salience ?*activate-z-chain[16]-salience*))
   ?level <- (technique ?cont z-chain[16])
=>
...
(defrule partial-z-chain[15]
   (declare (salience ?*partial-z-chain[15]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont partial-z-chain[15])
      ;;; because z-chains[15] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as z-chains[15] have been eliminated (because they have higher salience),
...
(defrule z-chain[16]
   (declare (salience ?*z-chain[16]-salience*))
   (chain
      (type partial-z-chain)
      (context ?cont)
      (length 15)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L16* ?*print-z-chain* ?*print-z-chain-16*) then
      (print-z-chain-without-crlf 16 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont z-chain[16] ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-z-chain[16]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont z-chain[16] ?zzz ?llc1 $?z-cands)
...
(defrule z-chain[16]-print-z-candidates
