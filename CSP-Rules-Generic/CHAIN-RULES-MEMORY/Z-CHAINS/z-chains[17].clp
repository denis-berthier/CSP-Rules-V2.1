...
(defrule activate-z-chain[17]
   (declare (salience ?*activate-z-chain[17]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   ...
   (assert (technique ?cont z-chain[17]))
   (assert (technique ?cont partial-z-chain[16]))
   (bind ?*technique* Z[17])
)



(defrule track-z-chain[17]
   (declare (salience ?*activate-z-chain[17]-salience*))
   ?level <- (technique ?cont z-chain[17])
=>
...
(defrule partial-z-chain[16]
   (declare (salience ?*partial-z-chain[16]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont partial-z-chain[16])
      ;;; because z-chains[16] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as z-chains[16] have been eliminated (because they have higher salience),
...
(defrule z-chain[17]
   (declare (salience ?*z-chain[17]-salience*))
   (chain
      (type partial-z-chain)
      (context ?cont)
      (length 16)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L17* ?*print-z-chain* ?*print-z-chain-17*) then
      (print-z-chain-without-crlf 17 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont z-chain[17] ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-z-chain[17]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont z-chain[17] ?zzz ?llc1 $?z-cands)
...
(defrule z-chain[17]-print-z-candidates
