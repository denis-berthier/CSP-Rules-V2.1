...
(defrule activate-z-chain[18]
   (declare (salience ?*activate-z-chain[18]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   ...
   (assert (technique ?cont z-chain[18]))
   (assert (technique ?cont partial-z-chain[17]))
   (bind ?*technique* Z[18])
)



(defrule track-z-chain[18]
   (declare (salience ?*activate-z-chain[18]-salience*))
   ?level <- (technique ?cont z-chain[18])
=>
...
(defrule partial-z-chain[17]
   (declare (salience ?*partial-z-chain[17]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont partial-z-chain[17])
      ;;; because z-chains[17] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as z-chains[17] have been eliminated (because they have higher salience),
...
(defrule z-chain[18]
   (declare (salience ?*z-chain[18]-salience*))
   (chain
      (type partial-z-chain)
      (context ?cont)
      (length 17)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L18* ?*print-z-chain* ?*print-z-chain-18*) then
      (print-z-chain-without-crlf 18 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont z-chain[18] ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-z-chain[18]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont z-chain[18] ?zzz ?llc1 $?z-cands)
...
(defrule z-chain[18]-print-z-candidates
