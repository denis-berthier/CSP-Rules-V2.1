...
=>
   ...
   (if (or ?*print-actions* ?*print-L2* ?*print-z-chain* ?*print-z-chain-2*) then
      (print-z-chain 2 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-z-chain[1]
   (declare (salience ?*partial-z-chain[1]-salience*))
   (logical
      (chain
         (type partial-z-chain)
         (context ?cont)
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont partial-z-chain[1])
      ;;; because z-chains[1] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as z-chains[1] have been eliminated (because they have higher salience),
