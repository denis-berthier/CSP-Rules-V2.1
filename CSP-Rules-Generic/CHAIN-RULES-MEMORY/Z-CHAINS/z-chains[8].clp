...
(defrule activate-z-chain[8]
   (declare (salience ?*activate-z-chain[8]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   ...
   (assert (technique ?cont z-chain[8]))
   (assert (technique ?cont partial-z-chain[7]))
   (bind ?*technique* Z[8])
)



(defrule track-z-chain[8]
   (declare (salience ?*activate-z-chain[8]-salience*))
   ?level <- (technique ?cont z-chain[8])
=>
...
(defrule partial-z-chain[7]
   (declare (salience ?*partial-z-chain[7]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont partial-z-chain[7])
      ;;; because z-chains[7] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as z-chains[7] have been eliminated (because they have higher salience),
...
(defrule z-chain[8]
   (declare (salience ?*z-chain[8]-salience*))
   (chain
      (type partial-z-chain)
      (context ?cont)
      (length 7)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L8* ?*print-z-chain* ?*print-z-chain-8*) then
      (print-z-chain-without-crlf 8 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont z-chain[8] ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-z-chain[8]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont z-chain[8] ?zzz ?llc1 $?z-cands)
...
(defrule z-chain[8]-print-z-candidates
