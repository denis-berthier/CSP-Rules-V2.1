...
(defrule activate-t-whip[23]
   (declare (salience ?*activate-t-whip[23]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont t-whip))
=>
   (if ?*print-levels* then (printout t Entering_level_tW23))
   (assert (technique ?cont partial-whip[22]))
   (assert (technique ?cont t-whip[23]))
   (bind ?*technique* tW[23])
)



(defrule track-t-whip[23]
   (declare (salience ?*activate-t-whip[23]-salience*))
   ?level <- (technique ?cont t-whip[23])
=>
...
(defrule detect-t-whip[23]
   (declare (salience ?*t-whip[23]-salience*))
   (chain
      (type partial-whip)
      (context ?cont)
      (length 22)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L23* ?*print-t-whip* ?*print-t-whip-23*) then
      (print-t-whip-without-crlf 23 ?zzz 
   ...
   (assert (pseudo-blocked ?cont t-whip[23] ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1))
)



;;; apply-t-whip[23]-to-more-targets

(defrule apply-t-whip[23]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience-1*))
   (pseudo-blocked ?cont t-whip[23] ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1)
