...
(defrule activate-typed-t-whip[27]
   (declare (salience ?*activate-typed-t-whip[27]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont typed-t-whip))
=>
   (if ?*print-levels* then (printout t Entering_level_TytW27))
   (assert (technique ?cont typed-partial-whip[26]))
   (assert (technique ?cont typed-t-whip[27]))
   (bind ?*technique* TytW[27])
)



(defrule track-typed-t-whip[27]
   (declare (salience ?*activate-typed-t-whip[27]-salience*))
   ?level <- (technique ?cont typed-t-whip[27])
=>
...
(defrule detect-typed-t-whip[27]
   (declare (salience ?*typed-t-whip[27]-salience*))
   (typed-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L27* ?*print-typed-t-whip* ?*print-typed-t-whip-27*) then
      (print-typed-t-whip-without-crlf ?csp-type 27 ?zzz 
   ...
   (assert (pseudo-blocked ?cont typed-t-whip[27]  ?csp-type ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1))
)



;;; apply-typed-t-whip[27]-to-more-targets

(defrule apply-typed-t-whip[27]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience-1*))
   (pseudo-blocked ?cont typed-t-whip[27]  ?csp-type ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1)
