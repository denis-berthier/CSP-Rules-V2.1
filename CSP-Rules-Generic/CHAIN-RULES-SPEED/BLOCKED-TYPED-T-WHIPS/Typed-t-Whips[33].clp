...
(defrule activate-typed-t-whip[33]
   (declare (salience ?*activate-typed-t-whip[33]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_TytW33))
   (assert (technique ?cont typed-partial-whip[32]))
   (assert (technique ?cont typed-t-whip[33]))
   (bind ?*technique* TytW[33])
)



(defrule track-typed-t-whip[33]
   (declare (salience ?*activate-typed-t-whip[33]-salience*))
   ?level <- (technique ?cont typed-t-whip[33])
=>
...
(defrule detect-typed-t-whip[33]
   (declare (salience ?*typed-t-whip[33]-salience*))
   (typed-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L33* ?*print-typed-t-whip* ?*print-typed-t-whip-33*) then
      (print-typed-t-whip-without-crlf ?csp-type 33 ?zzz 
   ...
   (assert (pseudo-blocked ?cont typed-t-whip[33]  ?csp-type ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1))
)



;;; apply-typed-t-whip[33]-to-more-targets

(defrule apply-typed-t-whip[33]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience-1*))
   (pseudo-blocked ?cont typed-t-whip[33]  ?csp-type ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1)
