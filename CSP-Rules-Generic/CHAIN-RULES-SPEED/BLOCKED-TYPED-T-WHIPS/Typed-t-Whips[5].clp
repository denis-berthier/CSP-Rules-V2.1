...
(defrule activate-typed-t-whip[5]
   (declare (salience ?*activate-typed-t-whip[5]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_TytW5))
   (assert (technique ?cont typed-partial-whip[4]))
   (assert (technique ?cont typed-t-whip[5]))
   (bind ?*technique* TytW[5])
)



(defrule track-typed-t-whip[5]
   (declare (salience ?*activate-typed-t-whip[5]-salience*))
   ?level <- (technique ?cont typed-t-whip[5])
=>
...
(defrule detect-typed-t-whip[5]
   (declare (salience ?*typed-t-whip[5]-salience*))
   (typed-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L5* ?*print-typed-t-whip* ?*print-typed-t-whip-5*) then
      (print-typed-t-whip-without-crlf ?csp-type 5 ?zzz 
   ...
   (assert (pseudo-blocked ?cont typed-t-whip[5]  ?csp-type ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1))
)



;;; apply-typed-t-whip[5]-to-more-targets

(defrule apply-typed-t-whip[5]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience-1*))
   (pseudo-blocked ?cont typed-t-whip[5]  ?csp-type ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1)
