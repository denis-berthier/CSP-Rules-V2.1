...
(defrule activate-typed-t-whip[9]
   (declare (salience ?*activate-typed-t-whip[9]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_TytW9))
   (assert (technique ?cont typed-partial-whip[8]))
   (assert (technique ?cont typed-t-whip[9]))
   (bind ?*technique* TytW[9])
)



(defrule track-typed-t-whip[9]
   (declare (salience ?*activate-typed-t-whip[9]-salience*))
   ?level <- (technique ?cont typed-t-whip[9])
=>
...
(defrule detect-typed-t-whip[9]
   (declare (salience ?*typed-t-whip[9]-salience*))
   (typed-chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L9* ?*print-typed-t-whip* ?*print-typed-t-whip-9*) then
      (print-typed-t-whip-without-crlf ?csp-type 9 ?zzz 
   ...
   (assert (pseudo-blocked ?cont typed-t-whip[9]  ?csp-type ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1))
)



;;; apply-typed-t-whip[9]-to-more-targets

(defrule apply-typed-t-whip[9]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience-1*))
   (pseudo-blocked ?cont typed-t-whip[9]  ?csp-type ?zzz ?zzz1 $?csp-vars $?llcs $?rlcs ?last-rlc ?zzz ?csp0 ?llc0 ?zzz1)
