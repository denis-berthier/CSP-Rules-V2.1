...
(defrule activate-braid[12]
   (declare (salience ?*activate-braid[12]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont braid))
=>
   (if ?*print-levels* then (printout t Entering_level_B12))
   (assert (technique ?cont braid[12]))
   (bind ?*technique* B[12])
)



(defrule track-braid[12]
   (declare (salience ?*activate-braid[12]-salience*))
   ?level <- (technique ?cont braid[12])
=>
...
(defrule braid[12]
   (declare (salience ?*braid[12]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L12* ?*print-braid* ?*print-braid-12*) then
      (print-braid 12 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[11]
   (declare (salience ?*partial-braid[11]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[12])
      ;;; because braids[12] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[11] have been eliminated (because they have higher salience),
