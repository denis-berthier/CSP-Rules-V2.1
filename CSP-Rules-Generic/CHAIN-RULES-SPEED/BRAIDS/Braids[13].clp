...
(defrule activate-braid[13]
   (declare (salience ?*activate-braid[13]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_B13))
   (assert (technique ?cont braid[13]))
   (bind ?*technique* B[13])
)



(defrule track-braid[13]
   (declare (salience ?*activate-braid[13]-salience*))
   ?level <- (technique ?cont braid[13])
=>
...
(defrule braid[13]
   (declare (salience ?*braid[13]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L13* ?*print-braid* ?*print-braid-13*) then
      (print-braid 13 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[12]
   (declare (salience ?*partial-braid[12]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[13])
      ;;; because braids[13] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[12] have been eliminated (because they have higher salience),
