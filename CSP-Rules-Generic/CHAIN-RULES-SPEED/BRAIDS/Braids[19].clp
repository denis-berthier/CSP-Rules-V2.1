...
(defrule activate-braid[19]
   (declare (salience ?*activate-braid[19]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont braid))
=>
   (if ?*print-levels* then (printout t Entering_level_B19))
   (assert (technique ?cont braid[19]))
   (bind ?*technique* B[19])
)



(defrule track-braid[19]
   (declare (salience ?*activate-braid[19]-salience*))
   ?level <- (technique ?cont braid[19])
=>
...
(defrule braid[19]
   (declare (salience ?*braid[19]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L19* ?*print-braid* ?*print-braid-19*) then
      (print-braid 19 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[18]
   (declare (salience ?*partial-braid[18]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[19])
      ;;; because braids[19] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[18] have been eliminated (because they have higher salience),
