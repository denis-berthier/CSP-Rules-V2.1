...
(defrule activate-braid[23]
   (declare (salience ?*activate-braid[23]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_B23))
   (assert (technique ?cont braid[23]))
   (bind ?*technique* B[23])
)



(defrule track-braid[23]
   (declare (salience ?*activate-braid[23]-salience*))
   ?level <- (technique ?cont braid[23])
=>
...
(defrule braid[23]
   (declare (salience ?*braid[23]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L23* ?*print-braid* ?*print-braid-23*) then
      (print-braid 23 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[22]
   (declare (salience ?*partial-braid[22]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[23])
      ;;; because braids[23] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[22] have been eliminated (because they have higher salience),
