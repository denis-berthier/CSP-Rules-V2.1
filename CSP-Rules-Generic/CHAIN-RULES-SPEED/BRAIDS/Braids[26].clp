...
(defrule activate-braid[26]
   (declare (salience ?*activate-braid[26]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_B26))
   (assert (technique ?cont braid[26]))
   (bind ?*technique* B[26])
)



(defrule track-braid[26]
   (declare (salience ?*activate-braid[26]-salience*))
   ?level <- (technique ?cont braid[26])
=>
...
(defrule braid[26]
   (declare (salience ?*braid[26]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L26* ?*print-braid* ?*print-braid-26*) then
      (print-braid 26 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[25]
   (declare (salience ?*partial-braid[25]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[26])
      ;;; because braids[26] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[25] have been eliminated (because they have higher salience),
