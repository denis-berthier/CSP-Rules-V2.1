...
(defrule activate-braid[27]
   (declare (salience ?*activate-braid[27]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont braid))
=>
   (if ?*print-levels* then (printout t Entering_level_B27))
   (assert (technique ?cont braid[27]))
   (bind ?*technique* B[27])
)



(defrule track-braid[27]
   (declare (salience ?*activate-braid[27]-salience*))
   ?level <- (technique ?cont braid[27])
=>
...
(defrule braid[27]
   (declare (salience ?*braid[27]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L27* ?*print-braid* ?*print-braid-27*) then
      (print-braid 27 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[26]
   (declare (salience ?*partial-braid[26]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[27])
      ;;; because braids[27] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[26] have been eliminated (because they have higher salience),
