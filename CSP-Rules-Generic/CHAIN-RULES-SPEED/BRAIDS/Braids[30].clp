...
(defrule activate-braid[30]
   (declare (salience ?*activate-braid[30]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_B30))
   (assert (technique ?cont braid[30]))
   (bind ?*technique* B[30])
)



(defrule track-braid[30]
   (declare (salience ?*activate-braid[30]-salience*))
   ?level <- (technique ?cont braid[30])
=>
...
(defrule braid[30]
   (declare (salience ?*braid[30]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L30* ?*print-braid* ?*print-braid-30*) then
      (print-braid 30 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[29]
   (declare (salience ?*partial-braid[29]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[30])
      ;;; because braids[30] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[29] have been eliminated (because they have higher salience),
