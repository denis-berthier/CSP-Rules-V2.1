...
(defrule activate-braid[33]
   (declare (salience ?*activate-braid[33]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_B33))
   (assert (technique ?cont braid[33]))
   (bind ?*technique* B[33])
)



(defrule track-braid[33]
   (declare (salience ?*activate-braid[33]-salience*))
   ?level <- (technique ?cont braid[33])
=>
...
(defrule braid[33]
   (declare (salience ?*braid[33]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L33* ?*print-braid* ?*print-braid-33*) then
      (print-braid 33 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[32]
   (declare (salience ?*partial-braid[32]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[33])
      ;;; because braids[33] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[32] have been eliminated (because they have higher salience),
