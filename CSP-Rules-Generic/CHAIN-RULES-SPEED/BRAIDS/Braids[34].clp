...
(defrule activate-braid[34]
   (declare (salience ?*activate-braid[34]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_B34))
   (assert (technique ?cont braid[34]))
   (bind ?*technique* B[34])
)



(defrule track-braid[34]
   (declare (salience ?*activate-braid[34]-salience*))
   ?level <- (technique ?cont braid[34])
=>
...
(defrule braid[34]
   (declare (salience ?*braid[34]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L34* ?*print-braid* ?*print-braid-34*) then
      (print-braid 34 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[33]
   (declare (salience ?*partial-braid[33]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[34])
      ;;; because braids[34] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[33] have been eliminated (because they have higher salience),
