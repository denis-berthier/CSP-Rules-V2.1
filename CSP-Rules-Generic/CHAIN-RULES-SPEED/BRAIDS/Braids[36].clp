...
(defrule activate-braid[36]
   (declare (salience ?*activate-braid[36]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_B36))
   (assert (technique ?cont braid[36]))
   (bind ?*technique* B[36])
)



(defrule track-braid[36]
   (declare (salience ?*activate-braid[36]-salience*))
   ?level <- (technique ?cont braid[36])
=>
...
(defrule braid[36]
   (declare (salience ?*braid[36]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L36* ?*print-braid* ?*print-braid-36*) then
      (print-braid 36 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[35]
   (declare (salience ?*partial-braid[35]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[36])
      ;;; because braids[36] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[35] have been eliminated (because they have higher salience),
