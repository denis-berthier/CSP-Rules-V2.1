...
(defrule activate-braid[5]
   (declare (salience ?*activate-braid[5]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_B5))
   (assert (technique ?cont braid[5]))
   (bind ?*technique* B[5])
)



(defrule track-braid[5]
   (declare (salience ?*activate-braid[5]-salience*))
   ?level <- (technique ?cont braid[5])
=>
...
=>
   ...
   (if (or ?*print-actions* ?*print-L5* ?*print-braid* ?*print-braid-5*) then
      (print-braid 5 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[4]
   ...
   (logical
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[5])
      ;;; because braids[5] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[4] have been eliminated (because they have higher salience),
