...
(defrule activate-braid[6]
   (declare (salience ?*activate-braid[6]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont braid))
=>
   (if ?*print-levels* then (printout t Entering_level_B6))
   (assert (technique ?cont braid[6]))
   (bind ?*technique* B[6])
)



(defrule track-braid[6]
   (declare (salience ?*activate-braid[6]-salience*))
   ?level <- (technique ?cont braid[6])
=>
...
(defrule braid[6]
   (declare (salience ?*braid[6]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L6* ?*print-braid* ?*print-braid-6*) then
      (print-braid 6 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[5]
   (declare (salience ?*partial-braid[5]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[6])
      ;;; because braids[6] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[5] have been eliminated (because they have higher salience),
