...
(defrule activate-braid[8]
   (declare (salience ?*activate-braid[8]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont braid))
=>
   (if ?*print-levels* then (printout t Entering_level_B8))
   (assert (technique ?cont braid[8]))
   (bind ?*technique* B[8])
)



(defrule track-braid[8]
   (declare (salience ?*activate-braid[8]-salience*))
   ?level <- (technique ?cont braid[8])
=>
...
(defrule braid[8]
   (declare (salience ?*braid[8]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L8* ?*print-braid* ?*print-braid-8*) then
      (print-braid 8 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[7]
   (declare (salience ?*partial-braid[7]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[8])
      ;;; because braids[8] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[7] have been eliminated (because they have higher salience),
