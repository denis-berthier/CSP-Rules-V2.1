...
(defrule activate-braid[9]
   (declare (salience ?*activate-braid[9]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_B9))
   (assert (technique ?cont braid[9]))
   (bind ?*technique* B[9])
)



(defrule track-braid[9]
   (declare (salience ?*activate-braid[9]-salience*))
   ?level <- (technique ?cont braid[9])
=>
...
(defrule braid[9]
   (declare (salience ?*braid[9]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L9* ?*print-braid* ?*print-braid-9*) then
      (print-braid 9 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-braid[8]
   (declare (salience ?*partial-braid[8]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont braid[9])
      ;;; because braids[9] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as braids[8] have been eliminated (because they have higher salience),
