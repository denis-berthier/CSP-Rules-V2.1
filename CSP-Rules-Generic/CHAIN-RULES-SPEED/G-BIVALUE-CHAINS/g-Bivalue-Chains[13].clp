...
(defrule activate-g-bivalue-chain[13]
   (declare (salience ?*activate-g-bivalue-chain[13]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gBC13))
   (assert (technique ?cont g-bivalue-chain[13]))
   (bind ?*technique* gBC[13])
)



(defrule track-g-bivalue-chain[13]
   (declare (salience ?*activate-g-bivalue-chain[13]-salience*))
   ?level <- (technique ?cont g-bivalue-chain[13])
=>
...
(defrule g-bivalue-chain[13]
   (declare (salience ?*g-bivalue-chain[13]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L13* ?*print-g-bivalue-chain* ?*print-g-bivalue-chain-13*) then
      (print-g-bivalue-chain 13 ?zzz $?llcs $?rlcs $?csp-vars)
   )
)
...
(defrule partial-g-bivalue-chain[13]-1
   (declare (salience ?*partial-g-bivalue-chain[13]-salience-1*))
   (logical
      (chain
         (type g-bivalue-chain)
         (context ?cont)
      ...
      ;;; ?new-llc
      (technique ?cont g-bivalue-chain[13])
      (or
...
(defrule partial-g-bivalue-chain[13]-2
   (declare (salience ?*partial-g-bivalue-chain[13]-salience-2*))
   (logical
      (chain
