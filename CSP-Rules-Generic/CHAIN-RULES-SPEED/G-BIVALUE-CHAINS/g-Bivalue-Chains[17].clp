...
(defrule activate-g-bivalue-chain[17]
   (declare (salience ?*activate-g-bivalue-chain[17]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gBC17))
   (assert (technique ?cont g-bivalue-chain[17]))
   (bind ?*technique* gBC[17])
)



(defrule track-g-bivalue-chain[17]
   (declare (salience ?*activate-g-bivalue-chain[17]-salience*))
   ?level <- (technique ?cont g-bivalue-chain[17])
=>
...
(defrule g-bivalue-chain[17]
   (declare (salience ?*g-bivalue-chain[17]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L17* ?*print-g-bivalue-chain* ?*print-g-bivalue-chain-17*) then
      (print-g-bivalue-chain 17 ?zzz $?llcs $?rlcs $?csp-vars)
   )
)
...
(defrule partial-g-bivalue-chain[17]-1
   (declare (salience ?*partial-g-bivalue-chain[17]-salience-1*))
   (logical
      (chain
         (type g-bivalue-chain)
         (context ?cont)
      ...
      ;;; ?new-llc
      (technique ?cont g-bivalue-chain[17])
      (or
...
(defrule partial-g-bivalue-chain[17]-2
   (declare (salience ?*partial-g-bivalue-chain[17]-salience-2*))
   (logical
      (chain
