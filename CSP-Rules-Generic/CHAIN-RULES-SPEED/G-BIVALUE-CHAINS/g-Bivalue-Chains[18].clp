...
(defrule activate-g-bivalue-chain[18]
   (declare (salience ?*activate-g-bivalue-chain[18]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont g-bivalue-chain))
=>
   (if ?*print-levels* then (printout t Entering_level_gBC18))
   (assert (technique ?cont g-bivalue-chain[18]))
   (bind ?*technique* gBC[18])
)



(defrule track-g-bivalue-chain[18]
   (declare (salience ?*activate-g-bivalue-chain[18]-salience*))
   ?level <- (technique ?cont g-bivalue-chain[18])
=>
...
(defrule g-bivalue-chain[18]
   (declare (salience ?*g-bivalue-chain[18]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L18* ?*print-g-bivalue-chain* ?*print-g-bivalue-chain-18*) then
      (print-g-bivalue-chain 18 ?zzz $?llcs $?rlcs $?csp-vars)
   )
)
...
(defrule partial-g-bivalue-chain[18]-1
   (declare (salience ?*partial-g-bivalue-chain[18]-salience-1*))
   (logical
      (chain
         (type g-bivalue-chain)
         (context ?cont)
      ...
      ;;; ?new-llc
      (technique ?cont g-bivalue-chain[18])
      (or
...
(defrule partial-g-bivalue-chain[18]-2
