...
(defrule activate-g-bivalue-chain[6]
   (declare (salience ?*activate-g-bivalue-chain[6]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont g-bivalue-chain))
=>
   (if ?*print-levels* then (printout t Entering_level_gBC6))
   (assert (technique ?cont g-bivalue-chain[6]))
   (bind ?*technique* gBC[6])
)



(defrule track-g-bivalue-chain[6]
   (declare (salience ?*activate-g-bivalue-chain[6]-salience*))
   ?level <- (technique ?cont g-bivalue-chain[6])
=>
...
(defrule g-bivalue-chain[6]
   (declare (salience ?*g-bivalue-chain[6]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L6* ?*print-g-bivalue-chain* ?*print-g-bivalue-chain-6*) then
      (print-g-bivalue-chain 6 ?zzz $?llcs $?rlcs $?csp-vars)
   )
)
...
(defrule partial-g-bivalue-chain[6]-1
   (declare (salience ?*partial-g-bivalue-chain[6]-salience-1*))
   (logical
      (chain
         (type g-bivalue-chain)
         (context ?cont)
      ...
      ;;; ?new-llc
      (technique ?cont g-bivalue-chain[6])
      (or
...
(defrule partial-g-bivalue-chain[6]-2
   (declare (salience ?*partial-g-bivalue-chain[6]-salience-2*))
   (logical
      (chain
