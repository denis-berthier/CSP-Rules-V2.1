...
(defrule activate-g-bivalue-chain[7]
   (declare (salience ?*activate-g-bivalue-chain[7]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gBC7))
   (assert (technique ?cont g-bivalue-chain[7]))
   (bind ?*technique* gBC[7])
)



(defrule track-g-bivalue-chain[7]
   (declare (salience ?*activate-g-bivalue-chain[7]-salience*))
   ?level <- (technique ?cont g-bivalue-chain[7])
=>
...
(defrule g-bivalue-chain[7]
   (declare (salience ?*g-bivalue-chain[7]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L7* ?*print-g-bivalue-chain* ?*print-g-bivalue-chain-7*) then
      (print-g-bivalue-chain 7 ?zzz $?llcs $?rlcs $?csp-vars)
   )
)
...
(defrule partial-g-bivalue-chain[7]-1
   (declare (salience ?*partial-g-bivalue-chain[7]-salience-1*))
   (logical
      (chain
         (type g-bivalue-chain)
         (context ?cont)
      ...
      ;;; ?new-llc
      (technique ?cont g-bivalue-chain[7])
      (or
...
(defrule partial-g-bivalue-chain[7]-2
   (declare (salience ?*partial-g-bivalue-chain[7]-salience-2*))
   (logical
      (chain
