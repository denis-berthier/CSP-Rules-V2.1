...
(defrule activate-g-bivalue-chain[8]
   (declare (salience ?*activate-g-bivalue-chain[8]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gBC8))
   (assert (technique ?cont g-bivalue-chain[8]))
   (bind ?*technique* gBC[8])
)



(defrule track-g-bivalue-chain[8]
   (declare (salience ?*activate-g-bivalue-chain[8]-salience*))
   ?level <- (technique ?cont g-bivalue-chain[8])
=>
...
(defrule g-bivalue-chain[8]
   (declare (salience ?*g-bivalue-chain[8]-salience*))
   (chain
...
=>
   ...
   (if (or ?*print-actions* ?*print-L8* ?*print-g-bivalue-chain* ?*print-g-bivalue-chain-8*) then
      (print-g-bivalue-chain 8 ?zzz $?llcs $?rlcs $?csp-vars)
   )
)
...
(defrule partial-g-bivalue-chain[8]-1
   (declare (salience ?*partial-g-bivalue-chain[8]-salience-1*))
   (logical
      (chain
         (type g-bivalue-chain)
         (context ?cont)
      ...
      ;;; ?new-llc
      (technique ?cont g-bivalue-chain[8])
      (or
...
(defrule partial-g-bivalue-chain[8]-2
   (declare (salience ?*partial-g-bivalue-chain[8]-salience-2*))
   (logical
      (chain
