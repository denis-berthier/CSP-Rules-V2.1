...
(defrule activate-gbraid[17]
   (declare (salience ?*activate-gbraid[17]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gB17))
   (assert (technique ?cont gbraid[17]))
   (bind ?*technique* gB[17])
)



(defrule track-gbraid[17]
   (declare (salience ?*activate-gbraid[17]-salience*))
   ?level <- (technique ?cont gbraid[17])
=>
...
(defrule gbraid[17]
   (declare (salience ?*gbraid[17]-salience*))
   (chain
      (type partial-gbraid)
      (context ?cont)
      (length 16)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L17* ?*print-gbraid* ?*print-gbraid-17*) then
      (print-gbraid 17 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gbraid{16]-1
   (declare (salience ?*partial-gbraid[16]-salience-1*))
   (logical
      (chain
...
(defrule partial-gbraid[16]-2
   (declare (salience ?*partial-gbraid[16]-salience-2*))
   (logical
      (chain
...
(defrule partial-gbraid[16]-3
   (declare (salience ?*partial-gbraid[16]-salience-3*))
   (logical
      (chain
         (type partial-gwhip|partial-gbraid)
         (context ?cont)
