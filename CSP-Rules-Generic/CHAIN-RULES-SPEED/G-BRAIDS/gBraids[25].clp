...
(defrule activate-gbraid[25]
   (declare (salience ?*activate-gbraid[25]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gB25))
   (assert (technique ?cont gbraid[25]))
   (bind ?*technique* gB[25])
)



(defrule track-gbraid[25]
   (declare (salience ?*activate-gbraid[25]-salience*))
   ?level <- (technique ?cont gbraid[25])
=>
...
(defrule gbraid[25]
   (declare (salience ?*gbraid[25]-salience*))
   (chain
      (type partial-gbraid)
      (context ?cont)
      (length 24)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L25* ?*print-gbraid* ?*print-gbraid-25*) then
      (print-gbraid 25 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gbraid{24]-1
   (declare (salience ?*partial-gbraid[24]-salience-1*))
   (logical
      (chain
...
(defrule partial-gbraid[24]-2
   (declare (salience ?*partial-gbraid[24]-salience-2*))
   (logical
      (chain
...
(defrule partial-gbraid[24]-3
   (declare (salience ?*partial-gbraid[24]-salience-3*))
   (logical
      (chain
         (type partial-gwhip|partial-gbraid)
         (context ?cont)
