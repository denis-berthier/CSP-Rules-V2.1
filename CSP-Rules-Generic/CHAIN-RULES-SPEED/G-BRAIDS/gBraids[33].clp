...
(defrule activate-gbraid[33]
   (declare (salience ?*activate-gbraid[33]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont gbraid))
=>
   (if ?*print-levels* then (printout t Entering_level_gB33))
   (assert (technique ?cont gbraid[33]))
   (bind ?*technique* gB[33])
)



(defrule track-gbraid[33]
   (declare (salience ?*activate-gbraid[33]-salience*))
   ?level <- (technique ?cont gbraid[33])
=>
...
(defrule gbraid[33]
   (declare (salience ?*gbraid[33]-salience*))
   (chain
      (type partial-gbraid)
      (context ?cont)
      (length 32)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L33* ?*print-gbraid* ?*print-gbraid-33*) then
      (print-gbraid 33 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gbraid{32]-1
   (declare (salience ?*partial-gbraid[32]-salience-1*))
   (logical
      (chain
...
(defrule partial-gbraid[32]-2
   (declare (salience ?*partial-gbraid[32]-salience-2*))
   (logical
      (chain
...
(defrule partial-gbraid[32]-3
   (declare (salience ?*partial-gbraid[32]-salience-3*))
   (logical
      (chain
         (type partial-gwhip|partial-gbraid)
         (context ?cont)
