...
(defrule activate-gbraid[5]
   (declare (salience ?*activate-gbraid[5]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gB5))
   (assert (technique ?cont gbraid[5]))
   (bind ?*technique* gB[5])
)



(defrule track-gbraid[5]
   (declare (salience ?*activate-gbraid[5]-salience*))
   ?level <- (technique ?cont gbraid[5])
=>
...
=>
   ...
   (if (or ?*print-actions* ?*print-L5* ?*print-gbraid* ?*print-gbraid-5*) then
      (print-gbraid 5 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gbraid{4]-1
   (declare (salience ?*partial-gbraid[4]-salience-1*))
   (logical
      (chain
...
(defrule partial-gbraid[4]-2
   (declare (salience ?*partial-gbraid[4]-salience-2*))
   (logical
      (chain
...
(defrule partial-gbraid[4]-3
   (declare (salience ?*partial-gbraid[4]-salience-3*))
   (logical
      (chain
         (type partial-gwhip|partial-gbraid)
         (context ?cont)
