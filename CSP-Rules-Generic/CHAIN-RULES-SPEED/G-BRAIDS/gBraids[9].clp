...
(defrule activate-gbraid[9]
   (declare (salience ?*activate-gbraid[9]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont gbraid))
=>
   (if ?*print-levels* then (printout t Entering_level_gB9))
   (assert (technique ?cont gbraid[9]))
   (bind ?*technique* gB[9])
)



(defrule track-gbraid[9]
   (declare (salience ?*activate-gbraid[9]-salience*))
   ?level <- (technique ?cont gbraid[9])
=>
...
(defrule gbraid[9]
   (declare (salience ?*gbraid[9]-salience*))
   (chain
      (type partial-gbraid)
      (context ?cont)
      (length 8)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L9* ?*print-gbraid* ?*print-gbraid-9*) then
      (print-gbraid 9 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gbraid{8]-1
   (declare (salience ?*partial-gbraid[8]-salience-1*))
   (logical
      (chain
...
(defrule partial-gbraid[8]-2
   (declare (salience ?*partial-gbraid[8]-salience-2*))
   (logical
      (chain
...
(defrule partial-gbraid[8]-3
   (declare (salience ?*partial-gbraid[8]-salience-3*))
   (logical
      (chain
         (type partial-gwhip|partial-gbraid)
         (context ?cont)
