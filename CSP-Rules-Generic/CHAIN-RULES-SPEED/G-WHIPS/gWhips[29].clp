...
(defrule activate-gwhip[29]
   (declare (salience ?*activate-gwhip[29]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gW29))
   (assert (technique ?cont gwhip[29]))
   (assert (technique ?cont partial-gwhip[28]))
   (bind ?*technique* gW[29])
)



(defrule track-gwhip[29]
   (declare (salience ?*activate-gwhip[29]-salience*))
   ?level <- (technique ?cont gwhip[29])
=>
...
(defrule gwhip[29]
   (declare (salience ?*gwhip[29]-salience*))
   (chain
      (type partial-gwhip)
      (context ?cont)
      (length 28)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L29* ?*print-gwhip* ?*print-gwhip-29*) then
      (print-gwhip 29 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
