...
(defrule activate-gwhip[6]
   (declare (salience ?*activate-gwhip[6]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gW6))
   (assert (technique ?cont gwhip[6]))
   (bind ?*technique* gW[6])
)



(defrule track-gwhip[6]
   (declare (salience ?*activate-gwhip[6]-salience*))
   ?level <- (technique ?cont gwhip[6])
=>
...
(defrule gwhip[6]
   (declare (salience ?*gwhip[6]-salience*))
   (chain
      (type partial-gwhip)
      (context ?cont)
      (length 5)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L6* ?*print-gwhip* ?*print-gwhip-6*) then
      (print-gwhip 6 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gwhip[5]-1
   (declare (salience ?*partial-gwhip[5]-salience-1*))
   (logical
      (chain
         (type partial-gwhip)
         (context ?cont)
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont gwhip[6])
      ;;; because gwhips[5] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as gwhips[5] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule partial-gwhip[5]-2
   (declare (salience ?*partial-gwhip[5]-salience-2*))
   (logical
      (chain
         (type partial-whip)
         (context ?cont)
   ...
   ;;; as gwhips[5] based on ?zzz have been eliminated (because they have higher salience),
   ;;; and gwhips[4] extendable by a single candidate have already been generated (because they have higher salience),
...
(defrule partial-gwhip[5]-3
   (declare (salience ?*partial-gwhip[5]-salience-3*))
   (logical
      (chain
         (type partial-gwhip)
         (context ?cont)
