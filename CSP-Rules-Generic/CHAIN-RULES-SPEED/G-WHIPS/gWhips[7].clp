...
(defrule activate-gwhip[7]
   (declare (salience ?*activate-gwhip[7]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont gwhip))
=>
   (if ?*print-levels* then (printout t Entering_level_gW7))
   (assert (technique ?cont gwhip[7]))
   (bind ?*technique* gW[7])
)



(defrule track-gwhip[7]
   (declare (salience ?*activate-gwhip[7]-salience*))
   ?level <- (technique ?cont gwhip[7])
=>
...
(defrule gwhip[7]
   (declare (salience ?*gwhip[7]-salience*))
   (chain
      (type partial-gwhip)
      (context ?cont)
      (length 6)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L7* ?*print-gwhip* ?*print-gwhip-7*) then
      (print-gwhip 7 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gwhip[6]-1
   (declare (salience ?*partial-gwhip[6]-salience-1*))
   (logical
      (chain
         (type partial-gwhip)
         (context ?cont)
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont gwhip[7])
      ;;; because gwhips[6] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as gwhips[6] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule partial-gwhip[6]-2
   (declare (salience ?*partial-gwhip[6]-salience-2*))
   (logical
      (chain
         (type partial-whip)
         (context ?cont)
   ...
   ;;; as gwhips[6] based on ?zzz have been eliminated (because they have higher salience),
   ;;; and gwhips[5] extendable by a single candidate have already been generated (because they have higher salience),
...
(defrule partial-gwhip[6]-3
   (declare (salience ?*partial-gwhip[6]-salience-3*))
   (logical
      (chain
         (type partial-gwhip)
         (context ?cont)
