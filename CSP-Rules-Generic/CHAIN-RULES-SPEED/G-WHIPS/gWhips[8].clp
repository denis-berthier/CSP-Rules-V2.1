...
(defrule activate-gwhip[8]
   (declare (salience ?*activate-gwhip[8]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_gW8))
   (assert (technique ?cont gwhip[8]))
   (bind ?*technique* gW[8])
)



(defrule track-gwhip[8]
   (declare (salience ?*activate-gwhip[8]-salience*))
   ?level <- (technique ?cont gwhip[8])
=>
...
(defrule gwhip[8]
   (declare (salience ?*gwhip[8]-salience*))
   (chain
      (type partial-gwhip)
      (context ?cont)
      (length 7)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L8* ?*print-gwhip* ?*print-gwhip-8*) then
      (print-gwhip 8 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule partial-gwhip[7]-1
   (declare (salience ?*partial-gwhip[7]-salience-1*))
   (logical
      (chain
         (type partial-gwhip)
         (context ?cont)
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont gwhip[8])
      ;;; because gwhips[7] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as gwhips[7] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule partial-gwhip[7]-2
   (declare (salience ?*partial-gwhip[7]-salience-2*))
   (logical
      (chain
         (type partial-whip)
         (context ?cont)
   ...
   ;;; as gwhips[7] based on ?zzz have been eliminated (because they have higher salience),
   ;;; and gwhips[6] extendable by a single candidate have already been generated (because they have higher salience),
...
(defrule partial-gwhip[7]-3
   (declare (salience ?*partial-gwhip[7]-salience-3*))
   (logical
      (chain
         (type partial-gwhip)
         (context ?cont)
