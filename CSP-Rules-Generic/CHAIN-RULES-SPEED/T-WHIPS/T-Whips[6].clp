...
=>
   ...
   (if (not ?*blocked-t-Whips*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont t-whip[6] ?zzz ?zzz1 - $?csp-vars - $?llcs - $?rlcs ?last-rlc ?csp0 ?llc0))
   )
)
...
(defrule apply-t-whip[6]-to-more-targets
   ...
   (pseudo-blocked ?cont t-whip[6] ?zzz ?zzz1 - $?csp-vars - $?llcs - $?rlcs ?last-rlc ?csp0 ?llc0)
