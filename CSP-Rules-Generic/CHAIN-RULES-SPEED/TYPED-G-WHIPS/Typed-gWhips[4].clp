...
(defrule activate-typed-gwhip[4]
   (declare (salience ?*activate-typed-gwhip[4]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_TygW4))
   (assert (technique ?cont typed-gwhip[4]))
   (bind ?*technique* TygW[4])
)



(defrule track-typed-gwhip[4]
   (declare (salience ?*activate-typed-gwhip[4]-salience*))
   ?level <- (technique ?cont typed-gwhip[4])
=>
...
(defrule typed-gwhip[4]
   (declare (salience ?*typed-gwhip[4]-salience*))
   (typed-chain
      (csp-type ?csp-type)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L4* ?*print-typed-gwhip* ?*print-typed-gwhip-4*) then
      (print-typed-g-whip ?csp-type 4 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule typed-partial-gwhip[3]-1
   (declare (salience ?*typed-partial-gwhip[3]-salience-1*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
         (type typed-partial-gwhip)
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont typed-gwhip[4])
      ;;; because typed-gwhips[3] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as typed-gwhips[3] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule typed-partial-gwhip[3]-2
   (declare (salience ?*typed-partial-gwhip[3]-salience-2*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
   ...
   ;;; as typed-gwhips[3] based on ?zzz have been eliminated (because they have higher salience),
   ;;; and typed-gwhips[2] extendable by a single candidate have already been generated (because they have higher salience),
...
(defrule typed-partial-gwhip[3]-3
   (declare (salience ?*typed-partial-gwhip[3]-salience-3*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
         (type typed-partial-gwhip)
