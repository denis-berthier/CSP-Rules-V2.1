...
(defrule activate-typed-gwhip[5]
   (declare (salience ?*activate-typed-gwhip[5]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_TygW5))
   (assert (technique ?cont typed-gwhip[5]))
   (bind ?*technique* TygW[5])
)



(defrule track-typed-gwhip[5]
   (declare (salience ?*activate-typed-gwhip[5]-salience*))
   ?level <- (technique ?cont typed-gwhip[5])
=>
...
(defrule typed-gwhip[5]
   (declare (salience ?*typed-gwhip[5]-salience*))
   (typed-chain
      (csp-type ?csp-type)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L5* ?*print-typed-gwhip* ?*print-typed-gwhip-5*) then
      (print-typed-g-whip ?csp-type 5 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule typed-partial-gwhip[4]-1
   (declare (salience ?*typed-partial-gwhip[4]-salience-1*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
         (type typed-partial-gwhip)
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont typed-gwhip[5])
      ;;; because typed-gwhips[4] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as typed-gwhips[4] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule typed-partial-gwhip[4]-2
   (declare (salience ?*typed-partial-gwhip[4]-salience-2*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
   ...
   ;;; as typed-gwhips[4] based on ?zzz have been eliminated (because they have higher salience),
   ;;; and typed-gwhips[3] extendable by a single candidate have already been generated (because they have higher salience),
...
(defrule typed-partial-gwhip[4]-3
   (declare (salience ?*typed-partial-gwhip[4]-salience-3*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
         (type typed-partial-gwhip)
