...
(defrule activate-typed-gwhip[6]
   (declare (salience ?*activate-typed-gwhip[6]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_TygW6))
   (assert (technique ?cont typed-gwhip[6]))
   (bind ?*technique* TygW[6])
)



(defrule track-typed-gwhip[6]
   (declare (salience ?*activate-typed-gwhip[6]-salience*))
   ?level <- (technique ?cont typed-gwhip[6])
=>
...
(defrule typed-gwhip[6]
   (declare (salience ?*typed-gwhip[6]-salience*))
   (typed-chain
      (csp-type ?csp-type)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L6* ?*print-typed-gwhip* ?*print-typed-gwhip-6*) then
      (print-typed-g-whip ?csp-type 6 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule typed-partial-gwhip[5]-1
   (declare (salience ?*typed-partial-gwhip[5]-salience-1*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
         (type typed-partial-gwhip)
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont typed-gwhip[6])
      ;;; because typed-gwhips[5] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as typed-gwhips[5] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule typed-partial-gwhip[5]-2
   (declare (salience ?*typed-partial-gwhip[5]-salience-2*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
   ...
   ;;; as typed-gwhips[5] based on ?zzz have been eliminated (because they have higher salience),
   ;;; and typed-gwhips[4] extendable by a single candidate have already been generated (because they have higher salience),
...
(defrule typed-partial-gwhip[5]-3
   (declare (salience ?*typed-partial-gwhip[5]-salience-3*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
         (type typed-partial-gwhip)
