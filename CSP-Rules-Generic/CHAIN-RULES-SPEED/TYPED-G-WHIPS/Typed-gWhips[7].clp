...
(defrule activate-typed-gwhip[7]
   (declare (salience ?*activate-typed-gwhip[7]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_TygW7))
   (assert (technique ?cont typed-gwhip[7]))
   (bind ?*technique* TygW[7])
)



(defrule track-typed-gwhip[7]
   (declare (salience ?*activate-typed-gwhip[7]-salience*))
   ?level <- (technique ?cont typed-gwhip[7])
=>
...
(defrule typed-gwhip[7]
   (declare (salience ?*typed-gwhip[7]-salience*))
   (typed-chain
      (csp-type ?csp-type)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L7* ?*print-typed-gwhip* ?*print-typed-gwhip-7*) then
      (print-typed-g-whip ?csp-type 7 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule typed-partial-gwhip[6]-1
   (declare (salience ?*typed-partial-gwhip[6]-salience-1*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
         (type typed-partial-gwhip)
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont typed-gwhip[7])
      ;;; because typed-gwhips[6] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as typed-gwhips[6] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule typed-partial-gwhip[6]-2
   (declare (salience ?*typed-partial-gwhip[6]-salience-2*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
   ...
   ;;; as typed-gwhips[6] based on ?zzz have been eliminated (because they have higher salience),
   ;;; and typed-gwhips[5] extendable by a single candidate have already been generated (because they have higher salience),
...
(defrule typed-partial-gwhip[6]-3
   (declare (salience ?*typed-partial-gwhip[6]-salience-3*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
         (type typed-partial-gwhip)
