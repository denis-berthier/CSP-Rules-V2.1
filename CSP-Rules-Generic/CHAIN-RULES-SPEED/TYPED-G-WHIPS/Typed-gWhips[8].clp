...
(defrule activate-typed-gwhip[8]
   (declare (salience ?*activate-typed-gwhip[8]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_TygW8))
   (assert (technique ?cont typed-gwhip[8]))
   (bind ?*technique* TygW[8])
)



(defrule track-typed-gwhip[8]
   (declare (salience ?*activate-typed-gwhip[8]-salience*))
   ?level <- (technique ?cont typed-gwhip[8])
=>
...
(defrule typed-gwhip[8]
   (declare (salience ?*typed-gwhip[8]-salience*))
   (typed-chain
      (csp-type ?csp-type)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L8* ?*print-typed-gwhip* ?*print-typed-gwhip-8*) then
      (print-typed-g-whip ?csp-type 8 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule typed-partial-gwhip[7]-1
   (declare (salience ?*typed-partial-gwhip[7]-salience-1*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
         (type typed-partial-gwhip)
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont typed-gwhip[8])
      ;;; because typed-gwhips[7] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as typed-gwhips[7] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule typed-partial-gwhip[7]-2
   (declare (salience ?*typed-partial-gwhip[7]-salience-2*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
   ...
   ;;; as typed-gwhips[7] based on ?zzz have been eliminated (because they have higher salience),
   ;;; and typed-gwhips[6] extendable by a single candidate have already been generated (because they have higher salience),
...
(defrule typed-partial-gwhip[7]-3
   (declare (salience ?*typed-partial-gwhip[7]-salience-3*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
         (type typed-partial-gwhip)
