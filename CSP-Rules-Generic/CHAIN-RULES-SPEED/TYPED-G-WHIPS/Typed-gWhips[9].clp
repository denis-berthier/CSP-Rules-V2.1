...
(defrule activate-typed-gwhip[9]
   (declare (salience ?*activate-typed-gwhip[9]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_TygW9))
   (assert (technique ?cont typed-gwhip[9]))
   (bind ?*technique* TygW[9])
)



(defrule track-typed-gwhip[9]
   (declare (salience ?*activate-typed-gwhip[9]-salience*))
   ?level <- (technique ?cont typed-gwhip[9])
=>
...
(defrule typed-gwhip[9]
   (declare (salience ?*typed-gwhip[9]-salience*))
   (typed-chain
      (csp-type ?csp-type)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L9* ?*print-typed-gwhip* ?*print-typed-gwhip-9*) then
      (print-typed-g-whip ?csp-type 9 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
...
(defrule typed-partial-gwhip[8]-1
   (declare (salience ?*typed-partial-gwhip[8]-salience-1*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
         (type typed-partial-gwhip)
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont typed-gwhip[9])
      ;;; because typed-gwhips[8] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as typed-gwhips[8] based on ?zzz have been eliminated (because they have higher salience),
...
(defrule typed-partial-gwhip[8]-2
   (declare (salience ?*typed-partial-gwhip[8]-salience-2*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
   ...
   ;;; as typed-gwhips[8] based on ?zzz have been eliminated (because they have higher salience),
   ;;; and typed-gwhips[7] extendable by a single candidate have already been generated (because they have higher salience),
...
(defrule typed-partial-gwhip[8]-3
   (declare (salience ?*typed-partial-gwhip[8]-salience-3*))
   (logical
      (typed-chain
         (csp-type ?csp-type)
         (type typed-partial-gwhip)
