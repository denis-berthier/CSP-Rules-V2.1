...
=>
   ...
   (if (not ?*blocked-t-Whips*)
      then (printout t crlf)
         ...
         (assert (apply-rule-as-a-pseudo-block ?cont))
         (assert (pseudo-blocked ?cont typed-t-whip[23]  ?csp-type ?zzz ?zzz1 - $?csp-vars - $?llcs - $?rlcs - ?last-rlc ?csp0 ?llc0))
   )
)
...
(defrule apply-typed-t-whip[23]-to-more-targets
   ...
   (pseudo-blocked ?cont typed-t-whip[23]  ?csp-type ?zzz ?zzz1 - $?csp-vars - $?llcs - $?rlcs - ?last-rlc ?csp0 ?llc0)
