...
(defrule activate-typed-z-chain[19]
   (declare (salience ?*activate-typed-z-chain[19]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   ...
   (assert (technique ?cont typed-z-chain[19]))
   (assert (technique ?cont partial-typed-z-chain[18]))
   (bind ?*technique* TyZ[19])
)



(defrule track-typed-z-chain[19]
   (declare (salience ?*activate-typed-z-chain[19]-salience*))
   ?level <- (technique ?cont typed-z-chain[19])
=>
...
(defrule typed-partial-z-chain[18]
   (declare (salience ?*typed-partial-z-chain[18]-salience*))
   (logical
      (typed-chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont typed-partial-z-chain[18])
      ;;; because typed-z-chains[18] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as typed-z-chains[18] have been eliminated (because they have higher salience),
...
(defrule typed-z-chain[19]
   (declare (salience ?*typed-z-chain[19]-salience*))
   (typed-chain
      (type typed-partial-z-chain)
      (csp-type ?csp-type)
      (context ?cont)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L19* ?*print-typed-z-chain* ?*print-typed-z-chain-19*) then
      (print-typed-z-chain-without-crlf ?csp-type 19 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      ...
      (assert (pseudo-blocked ?cont typed-z-chain[19] ?csp-type ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-typed-z-chain[19]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont typed-z-chain[19] ?csp-type ?zzz ?llc1 $?z-cands)
...
(defrule typed-z-chain[19]-print-z-candidates
