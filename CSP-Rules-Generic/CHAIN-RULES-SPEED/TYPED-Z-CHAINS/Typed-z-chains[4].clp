...
=>
   (if ?*print-levels* then (printout t Entering_level_TyZ4))
   (assert (technique ?cont typed-z-chain[4]))
   (assert (technique ?cont typed-partial-z-chain[3]))
   (bind ?*technique* typed-z-chain[4])
)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L4* ?*print-typed-z-chain* ?*print-typed-z-chain-4*) then
      (print-typed-z-chain ?csp-type 4 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
