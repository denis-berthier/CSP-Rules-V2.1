...
=>
   (if ?*print-levels* then (printout t Entering_level_TyZ6))
   (assert (technique ?cont typed-z-chain[6]))
   (assert (technique ?cont typed-partial-z-chain[5]))
   (bind ?*technique* typed-z-chain[6])
)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L6* ?*print-typed-z-chain* ?*print-typed-z-chain-6*) then
      (print-typed-z-chain ?csp-type 6 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
