...
=>
   ...
   (assert (technique ?cont partial-typed-z-chain[6]))
   (bind ?*technique* TyZ[7])
)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L7* ?*print-typed-z-chain* ?*print-typed-z-chain-7*) then
      (print-typed-z-chain-without-crlf ?csp-type 7 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      ...
      (assert (pseudo-blocked ?cont typed-z-chain[7] ?csp-type ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-typed-z-chain[7]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont typed-z-chain[7] ?csp-type ?zzz ?llc1 $?z-cands)
...
(defrule typed-z-chain[7]-print-z-candidates
