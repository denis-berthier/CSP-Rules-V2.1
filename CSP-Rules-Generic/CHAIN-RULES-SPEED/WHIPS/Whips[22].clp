...
(defrule activate-whip[22]
   (declare (salience ?*activate-whip[22]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont whip))
=>
   (if ?*print-levels* then (printout t Entering_level_W22))
   (assert (technique ?cont partial-whip[21]))
   (assert (technique ?cont whip[22]))
   (bind ?*technique* W[22])
)



(defrule track-whip[22]
   (declare (salience ?*activate-whip[22]-salience*))
   ?level <- (technique ?cont whip[22])
=>
...
(defrule whip[22]
   (declare (salience ?*whip[22]-salience*))
   (chain
      (type partial-whip)
      (context ?cont)
      (length 21)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L22* ?*print-whip* ?*print-whip-22*) then
      (print-whip 22 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
