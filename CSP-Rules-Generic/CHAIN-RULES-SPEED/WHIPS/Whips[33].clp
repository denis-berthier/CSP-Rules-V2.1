...
(defrule activate-whip[33]
   (declare (salience ?*activate-whip[33]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont whip))
=>
   (if ?*print-levels* then (printout t Entering_level_W33))
   (assert (technique ?cont partial-whip[32]))
   (assert (technique ?cont whip[33]))
   (bind ?*technique* W[33])
)



(defrule track-whip[33]
   (declare (salience ?*activate-whip[33]-salience*))
   ?level <- (technique ?cont whip[33])
=>
...
(defrule whip[33]
   (declare (salience ?*whip[33]-salience*))
   (chain
      (type partial-whip)
      (context ?cont)
      (length 32)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L33* ?*print-whip* ?*print-whip-33*) then
      (print-whip 33 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
