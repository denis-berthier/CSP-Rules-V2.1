...
(defrule activate-whip[34]
   (declare (salience ?*activate-whip[34]-salience*))
   (logical (play) (context (name ?cont)))
   (not (deactivate ?cont whip))
=>
   (if ?*print-levels* then (printout t Entering_level_W34))
   (assert (technique ?cont partial-whip[33]))
   (assert (technique ?cont whip[34]))
   (bind ?*technique* W[34])
)



(defrule track-whip[34]
   (declare (salience ?*activate-whip[34]-salience*))
   ?level <- (technique ?cont whip[34])
=>
...
(defrule whip[34]
   (declare (salience ?*whip[34]-salience*))
   (chain
      (type partial-whip)
      (context ?cont)
      (length 33)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L34* ?*print-whip* ?*print-whip-34*) then
      (print-whip 34 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
)
