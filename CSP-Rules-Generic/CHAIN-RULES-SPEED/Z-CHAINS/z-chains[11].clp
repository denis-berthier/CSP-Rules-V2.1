...
=>
   ...
   (if (or ?*print-actions* ?*print-L11* ?*print-z-chain* ?*print-z-chain-11*) then
      (print-z-chain-without-crlf 11 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont z-chain[11] ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-z-chain[11]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont z-chain[11] ?zzz ?llc1 $?z-cands)
...
(defrule z-chain[11]-print-z-candidates
