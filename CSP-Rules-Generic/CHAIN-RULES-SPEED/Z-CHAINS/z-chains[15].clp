...
(defrule activate-z-chain[15]
   (declare (salience ?*activate-z-chain[15]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   ...
   (assert (technique ?cont z-chain[15]))
   (assert (technique ?cont partial-z-chain[14]))
   (bind ?*technique* Z[15])
)



(defrule track-z-chain[15]
   (declare (salience ?*activate-z-chain[15]-salience*))
   ?level <- (technique ?cont z-chain[15])
=>
...
(defrule partial-z-chain[14]
   (declare (salience ?*partial-z-chain[14]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont partial-z-chain[14])
      ;;; because z-chains[14] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as z-chains[14] have been eliminated (because they have higher salience),
...
(defrule z-chain[15]
   (declare (salience ?*z-chain[15]-salience*))
   (chain
      (type partial-z-chain)
      (context ?cont)
      (length 14)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L15* ?*print-z-chain* ?*print-z-chain-15*) then
      (print-z-chain-without-crlf 15 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont z-chain[15] ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-z-chain[15]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont z-chain[15] ?zzz ?llc1 $?z-cands)
...
(defrule z-chain[15]-print-z-candidates
