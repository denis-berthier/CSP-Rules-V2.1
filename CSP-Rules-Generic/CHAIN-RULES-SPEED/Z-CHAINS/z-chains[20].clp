...
(defrule activate-z-chain[20]
   (declare (salience ?*activate-z-chain[20]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_Z20))
   (assert (technique ?cont z-chain[20]))
   (assert (technique ?cont partial-z-chain[19]))
   (bind ?*technique* Z[20])
)



(defrule track-z-chain[20]
   (declare (salience ?*activate-z-chain[20]-salience*))
   ?level <- (technique ?cont z-chain[20])
=>
...
(defrule partial-z-chain[19]
   (declare (salience ?*partial-z-chain[19]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont partial-z-chain[19])
      ;;; because z-chains[19] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as z-chains[19] have been eliminated (because they have higher salience),
...
(defrule z-chain[20]
   (declare (salience ?*z-chain[20]-salience*))
   (chain
      (type partial-z-chain)
      (context ?cont)
      (length 19)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L20* ?*print-z-chain* ?*print-z-chain-20*) then
      (print-z-chain-without-crlf 20 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont z-chain[20] ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-z-chain[20]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont z-chain[20] ?zzz ?llc1 $?z-cands)
...
(defrule z-chain[20]-print-z-candidates
