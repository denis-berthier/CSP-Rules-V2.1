...
(defrule activate-z-chain[5]
   (declare (salience ?*activate-z-chain[5]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   ...
   (assert (technique ?cont z-chain[5]))
   (assert (technique ?cont partial-z-chain[4]))
   (bind ?*technique* Z[5])
)



(defrule track-z-chain[5]
   (declare (salience ?*activate-z-chain[5]-salience*))
   ?level <- (technique ?cont z-chain[5])
=>
...
(defrule partial-z-chain[4]
   (declare (salience ?*partial-z-chain[4]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont partial-z-chain[4])
      ;;; because z-chains[4] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as z-chains[4] have been eliminated (because they have higher salience),
...
(defrule z-chain[5]
   (declare (salience ?*z-chain[5]-salience*))
   (chain
      (type partial-z-chain)
      (context ?cont)
      (length 4)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L5* ?*print-z-chain* ?*print-z-chain-5*) then
      (print-z-chain-without-crlf 5 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont z-chain[5] ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-z-chain[5]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont z-chain[5] ?zzz ?llc1 $?z-cands)
...
(defrule z-chain[5]-print-z-candidates
