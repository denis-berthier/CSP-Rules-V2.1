...
(defrule activate-z-chain[6]
   (declare (salience ?*activate-z-chain[6]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   ...
   (assert (technique ?cont z-chain[6]))
   (assert (technique ?cont partial-z-chain[5]))
   (bind ?*technique* Z[6])
)



(defrule track-z-chain[6]
   (declare (salience ?*activate-z-chain[6]-salience*))
   ?level <- (technique ?cont z-chain[6])
=>
...
(defrule partial-z-chain[5]
   (declare (salience ?*partial-z-chain[5]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont partial-z-chain[5])
      ;;; because z-chains[5] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as z-chains[5] have been eliminated (because they have higher salience),
...
(defrule z-chain[6]
   (declare (salience ?*z-chain[6]-salience*))
   (chain
      (type partial-z-chain)
      (context ?cont)
      (length 5)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L6* ?*print-z-chain* ?*print-z-chain-6*) then
      (print-z-chain-without-crlf 6 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont z-chain[6] ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-z-chain[6]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont z-chain[6] ?zzz ?llc1 $?z-cands)
...
(defrule z-chain[6]-print-z-candidates
