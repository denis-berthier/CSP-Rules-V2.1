...
(defrule activate-z-chain[9]
   (declare (salience ?*activate-z-chain[9]-salience*))
   (logical
      (play)
      (context (name ?cont))
...
=>
   (if ?*print-levels* then (printout t Entering_level_Z9))
   (assert (technique ?cont z-chain[9]))
   (assert (technique ?cont partial-z-chain[8]))
   (bind ?*technique* Z[9])
)



(defrule track-z-chain[9]
   (declare (salience ?*activate-z-chain[9]-salience*))
   ?level <- (technique ?cont z-chain[9])
=>
...
(defrule partial-z-chain[8]
   (declare (salience ?*partial-z-chain[8]-salience*))
   (logical
      (chain
      ...
      ;;; ?new-rlc and ?new-csp
      (technique ?cont partial-z-chain[8])
      ;;; because z-chains[8] based on ?zzz (if any) have already been eliminated,
   ...
   ;;; as z-chains[8] have been eliminated (because they have higher salience),
...
(defrule z-chain[9]
   (declare (salience ?*z-chain[9]-salience*))
   (chain
      (type partial-z-chain)
      (context ?cont)
      (length 8)
...
=>
   ...
   (if (or ?*print-actions* ?*print-L9* ?*print-z-chain* ?*print-z-chain-9*) then
      (print-z-chain-without-crlf 9 ?zzz $?llcs $?rlcs $?csp-vars ?new-llc . ?new-csp)
   )
   ...
   (if (not ?*blocked-z-chains*)
      then (printout t crlf)
         ...
         (assert (pseudo-blocked ?cont z-chain[9] ?zzz (nth$ 1 ?llcs) $?z-cands))
   )
)
...
(defrule apply-z-chain[9]-to-more-targets
   (declare (salience ?*apply-a-blocked-rule-salience*))
   (apply-rule-as-a-pseudo-block ?cont)
   (pseudo-blocked ?cont z-chain[9] ?zzz ?llc1 $?z-cands)
...
(defrule z-chain[9]-print-z-candidates
