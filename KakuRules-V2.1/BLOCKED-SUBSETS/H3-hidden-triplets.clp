...
=>
    ...
    (assert (blocked ?cont hidden-triplets-in-horizontal-sector ?row ?nb1 ?nb2 ?nb3 ?col1 ?col2 ?col3))
	(if (or ?*print-actions* ?*print-L3* ?*print-hidden-triplets*) then
        (bind ?*blocked-rule-description*
            (str-cat
                "hidden-triplets-in-horiz-sector: "
				(row-name ?row)
...
(defrule apply-L3-hidden-triplets-in-a-row
    ...
    (blocked ?cont hidden-triplets-in-horizontal-sector ?row ?nb1 ?nb2 ?nb3 ?col1 ?col2 ?col3)
...
=>
    (assert (apply-rule-as-a-block ?cont))
    (assert (blocked ?cont hidden-triplets-in-vertical-sector ?col ?nb1 ?nb2 ?nb3 ?row1 ?row2 ?row3))
	(if (or ?*print-actions* ?*print-L3* ?*print-hidden-triplets*) then
        (bind ?*blocked-rule-description*
            (str-cat
			    "hidden-triplets-in-verti-sector: "
				(column-name ?col)
				?*starting-cell-symbol* (number-name ?nb1) ?*separation-sign-in-cell* (number-name ?nb2) 
				?*separation-sign-in-cell* (number-name ?nb3) ?*ending-cell-symbol*
...
(defrule apply-L3-hidden-triplets-in-a-column
    (declare (salience ?*apply-a-blocked-rule-salience-1*))
    (blocked ?cont hidden-triplets-in-vertical-sector ?col ?nb1 ?nb2 ?nb3 ?row1 ?row2 ?row3)
    ;;; identify the targets
