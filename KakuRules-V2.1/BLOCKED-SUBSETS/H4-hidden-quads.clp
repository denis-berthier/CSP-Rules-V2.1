...
=>
    ...
    (assert (blocked ?cont hidden-quads-in-horizontal-sector ?row ?nb1 ?nb2 ?nb3 ?nb4 ?col1 ?col2 ?col3 ?col4))
	(if (or ?*print-actions* ?*print-L4* ?*print-hidden-quads*) then
        (bind ?*blocked-rule-description*
        (str-cat
            "hidden-quads-in-horiz-sector: "
				(row-name ?row)
...
(defrule apply-L4-hidden-quads-in-a-row
    ...
    (blocked ?cont hidden-quads-in-horizontal-sector ?row ?nb1 ?nb2 ?nb3 ?nb4 ?col1 ?col2 ?col3 ?col4)
...
=>
    (assert (apply-rule-as-a-block ?cont))
    (assert (blocked ?cont hidden-quads-in-vertical-sector ?col ?nb1 ?nb2 ?nb3 ?nb4 ?row1 ?row2 ?row3 ?row4))
	(if (or ?*print-actions* ?*print-L4* ?*print-hidden-quads*) then
        (bind ?*blocked-rule-description*
            (str-cat
                "hidden-quads-in-verti-sector: "
				(column-name ?col)
				?*starting-cell-symbol* (number-name ?nb1) ?*separation-sign-in-cell* (number-name ?nb2) 
				?*separation-sign-in-cell* (number-name ?nb3) ?*separation-sign-in-cell* (number-name ?nb4) ?*ending-cell-symbol*
...
(defrule apply-L4-hidden-quads-in-a-column
    (declare (salience ?*apply-a-blocked-rule-salience-1*))
    (blocked ?cont hidden-quads-in-vertical-sector ?col ?nb1 ?nb2 ?nb3 ?nb4 ?row1 ?row2 ?row3 ?row4)
    ;;; identify the targets
