...
=>
    ...
    (assert (blocked ?cont jellyfish-in-horizontal-sectors ?nb ?row1 ?row2 ?row3 ?row4 ?col1 ?col2 ?col3 ?col4 ?vsec1 ?vsec2 ?vsec3 ?vsec4))
	(if (or ?*print-actions* ?*print-L4* ?*print-jellyfish*) then
        (bind ?*blocked-rule-description*
            (str-cat
			    "jellyfish-in-horiz-sectors: "
				(number-name ?nb) 
...
(defrule apply-L4-jellyfish-in-rows
    ...
    (blocked ?cont jellyfish-in-horizontal-sectors ?nb ?row1 ?row2 ?row3 ?row4 ?col1 ?col2 ?col3 ?col4 ?vsec1 ?vsec2 ?vsec3 ?vsec4)
...
=>
    (assert (apply-rule-as-a-block ?cont))
    (assert (blocked ?cont jellyfish-in-vertical-sectors ?nb ?col1 ?col2 ?col3 ?col4 ?row1 ?row2 ?row3 ?row4 ?hsec1 ?hsec2 ?hsec3 ?hsec4))
	(if (or ?*print-actions* ?*print-L4* ?*print-jellyfish*) then
        (bind ?*blocked-rule-description*
            (str-cat
			    "jellyfish-in-verti-sectors: "
				(number-name ?nb) 
				?*starting-cell-symbol* (column-name ?col1) ?*separation-sign-in-cell* (column-name ?col2) 
				?*separation-sign-in-cell* (column-name ?col3) ?*separation-sign-in-cell* (column-name ?col4) ?*ending-cell-symbol*
				?*starting-cell-symbol* (row-name ?row1) ?*separation-sign-in-cell* (row-name ?row2) 
...
(defrule apply-L4-jellyfish-in-columns
    (declare (salience ?*jellyfish-salience*))
    (blocked ?cont jellyfish-in-vertical-sectors ?nb ?col1 ?col2 ?col3 ?col4 ?row1 ?row2 ?row3 ?row4 ?hsec1 ?hsec2 ?hsec3 ?hsec4)
    ;;; identify the targets
