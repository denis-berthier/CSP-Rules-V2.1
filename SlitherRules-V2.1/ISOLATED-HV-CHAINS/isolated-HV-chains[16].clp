...
(defrule activate-isolated-HV-chain[16]
   (declare (salience ?*activate-xtd-loop[17]-salience*))
   (logical (context (name ?cont)) (technique ?cont BRT))
   (not (deactivate ?cont isolated-HV-chain))
=>
   (assert (technique ?cont isolated-HV-chain[16]))
   (bind ?*technique* isolated-HV-chain[16])
)
...
(defrule isolated-HV-chain-extension[16]
   (declare (salience ?*partial-loop[16]-salience*))
   (logical
      (isolated-HV-chain
         (context ?cont)
         (length 15)
      ...
      (technique ?cont isolated-HV-chain[16])
      ;;; extension
