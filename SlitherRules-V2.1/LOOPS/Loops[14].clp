...
(defrule activate-loop[14]
   (declare (salience ?*activate-loop[14]-salience*))
   (logical (context (name ?cont)) (technique ?cont BRT))
   (not (deactivate ?cont loop))
=>
   (assert (technique ?cont loop[14]))
   (assert (technique ?cont partial-loop[13]))
   (bind ?*technique* loop[14])
)
...
(defrule partial-loop[13]
   (declare (salience ?*partial-loop[13]-salience*))
   (logical
      (partial-loop
         (context ?cont)
         (length 11)
      ...
      (technique ?cont partial-loop[13])
      ;;; extension1
...
(defrule loop-elimination-or-solution[14]
   (declare (salience ?*loop[14]-salience*))
   (partial-loop
      (context ?cont)
      (length 13)
      (nb-cont ?nb-cont)
...
=>
   ...
   (if (< (+ ?nb-cont (nb-contacts ?typey ?rowy ?coly)) ?*sum-of-givens*)
      then ; value 1 would make a too short loop; it must be deleted
         ...
         (print-loop 14 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 0)
      else ; loop length is ok for solution; value 0 must be retracted to allow closing the loop
         (retract ?cand2)
         (modify ?cand (status c-value))
         (print-loop 14 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 1)
   )
