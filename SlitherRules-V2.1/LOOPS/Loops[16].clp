...
(defrule activate-loop[16]
   (declare (salience ?*activate-loop[16]-salience*))
   (logical (context (name ?cont)) (technique ?cont BRT))
   (not (deactivate ?cont loop))
=>
   (assert (technique ?cont loop[16]))
   (assert (technique ?cont partial-loop[15]))
   (bind ?*technique* loop[16])
)
...
(defrule partial-loop[15]
   (declare (salience ?*partial-loop[15]-salience*))
   (logical
      (partial-loop
         (context ?cont)
         (length 13)
      ...
      (technique ?cont partial-loop[15])
      ;;; extension1
...
(defrule loop-elimination-or-solution[16]
   (declare (salience ?*loop[16]-salience*))
   (partial-loop
      (context ?cont)
      (length 15)
      (nb-cont ?nb-cont)
...
=>
   ...
   (if (< (+ ?nb-cont (nb-contacts ?typey ?rowy ?coly)) ?*sum-of-givens*)
      then ; value 1 would make a too short loop; it must be deleted
         ...
         (print-loop 16 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 0)
      else ; loop length is ok for solution; value 0 must be retracted to allow closing the loop
         (retract ?cand2)
         (modify ?cand (status c-value))
         (print-loop 16 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 1)
   )
