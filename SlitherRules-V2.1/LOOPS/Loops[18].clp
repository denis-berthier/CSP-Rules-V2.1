...
(defrule activate-loop[18]
   (declare (salience ?*activate-loop[18]-salience*))
   (logical (context (name ?cont)) (technique ?cont BRT))
   (not (deactivate ?cont loop))
=>
   (assert (technique ?cont loop[18]))
   (assert (technique ?cont partial-loop[17]))
   (bind ?*technique* loop[18])
)
...
(defrule partial-loop[17]
   (declare (salience ?*partial-loop[17]-salience*))
   (logical
      (partial-loop
         (context ?cont)
         (length 15)
      ...
      (technique ?cont partial-loop[17])
      ;;; extension1
...
(defrule loop-elimination-or-solution[18]
   (declare (salience ?*loop[18]-salience*))
   (partial-loop
      (context ?cont)
      (length 17)
      (nb-cont ?nb-cont)
...
=>
   ...
   (if (< (+ ?nb-cont (nb-contacts ?typey ?rowy ?coly)) ?*sum-of-givens*)
      then ; value 1 would make a too short loop; it must be deleted
         ...
         (print-loop 18 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 0)
      else ; loop length is ok for solution; value 0 must be retracted to allow closing the loop
         (retract ?cand2)
         (modify ?cand (status c-value))
         (print-loop 18 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 1)
   )
