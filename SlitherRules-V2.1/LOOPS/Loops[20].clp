...
(defrule activate-loop[20]
   (declare (salience ?*activate-loop[20]-salience*))
   (logical (context (name ?cont)) (technique ?cont BRT))
   (not (deactivate ?cont loop))
=>
   (assert (technique ?cont loop[20]))
   (assert (technique ?cont partial-loop[19]))
   (bind ?*technique* loop[20])
)
...
(defrule partial-loop[19]
   (declare (salience ?*partial-loop[19]-salience*))
   (logical
      (partial-loop
         (context ?cont)
         (length 17)
      ...
      (technique ?cont partial-loop[19])
      ;;; extension1
...
(defrule loop-elimination-or-solution[20]
   (declare (salience ?*loop[20]-salience*))
   (partial-loop
      (context ?cont)
      (length 19)
      (nb-cont ?nb-cont)
...
=>
   ...
   (if (< (+ ?nb-cont (nb-contacts ?typey ?rowy ?coly)) ?*sum-of-givens*)
      then ; value 1 would make a too short loop; it must be deleted
         ...
         (print-loop 20 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 0)
      else ; loop length is ok for solution; value 0 must be retracted to allow closing the loop
         (retract ?cand2)
         (modify ?cand (status c-value))
         (print-loop 20 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 1)
   )
