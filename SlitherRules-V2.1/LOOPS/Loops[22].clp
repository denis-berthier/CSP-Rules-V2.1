...
(defrule activate-loop[22]
   (declare (salience ?*activate-loop[22]-salience*))
   (logical (context (name ?cont)) (technique ?cont BRT))
   (not (deactivate ?cont loop))
=>
   (assert (technique ?cont loop[22]))
   (assert (technique ?cont partial-loop[21]))
   (bind ?*technique* loop[22])
)
...
(defrule partial-loop[21]
   (declare (salience ?*partial-loop[21]-salience*))
   (logical
      (partial-loop
         (context ?cont)
         (length 19)
      ...
      (technique ?cont partial-loop[21])
      ;;; extension1
...
(defrule loop-elimination-or-solution[22]
   (declare (salience ?*loop[22]-salience*))
   (partial-loop
      (context ?cont)
      (length 21)
      (nb-cont ?nb-cont)
...
=>
   ...
   (if (< (+ ?nb-cont (nb-contacts ?typey ?rowy ?coly)) ?*sum-of-givens*)
      then ; value 1 would make a too short loop; it must be deleted
         ...
         (print-loop 22 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 0)
      else ; loop length is ok for solution; value 0 must be retracted to allow closing the loop
         (retract ?cand2)
         (modify ?cand (status c-value))
         (print-loop 22 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 1)
   )
