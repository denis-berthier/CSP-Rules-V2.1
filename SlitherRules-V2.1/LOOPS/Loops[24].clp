...
(defrule activate-loop[24]
   (declare (salience ?*activate-loop[24]-salience*))
   (logical (context (name ?cont)) (technique ?cont BRT))
   (not (deactivate ?cont loop))
=>
   (assert (technique ?cont loop[24]))
   (assert (technique ?cont partial-loop[23]))
   (bind ?*technique* loop[24])
)
...
(defrule partial-loop[23]
   (declare (salience ?*partial-loop[23]-salience*))
   (logical
      (partial-loop
         (context ?cont)
         (length 21)
      ...
      (technique ?cont partial-loop[23])
      ;;; extension1
...
(defrule loop-elimination-or-solution[24]
   (declare (salience ?*loop[24]-salience*))
   (partial-loop
      (context ?cont)
      (length 23)
      (nb-cont ?nb-cont)
...
=>
   ...
   (if (< (+ ?nb-cont (nb-contacts ?typey ?rowy ?coly)) ?*sum-of-givens*)
      then ; value 1 would make a too short loop; it must be deleted
         ...
         (print-loop 24 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 0)
      else ; loop length is ok for solution; value 0 must be retracted to allow closing the loop
         (retract ?cand2)
         (modify ?cand (status c-value))
         (print-loop 24 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 1)
   )
