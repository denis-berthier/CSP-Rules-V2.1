...
(defrule activate-loop[34]
   (declare (salience ?*activate-loop[34]-salience*))
   (logical (context (name ?cont)) (technique ?cont BRT))
   (not (deactivate ?cont loop))
=>
   (assert (technique ?cont loop[34]))
   (assert (technique ?cont partial-loop[33]))
   (bind ?*technique* loop[34])
)
...
(defrule partial-loop[33]
   (declare (salience ?*partial-loop[33]-salience*))
   (logical
      (partial-loop
         (context ?cont)
         (length 31)
      ...
      (technique ?cont partial-loop[33])
      ;;; extension1
...
(defrule loop-elimination-or-solution[34]
   (declare (salience ?*loop[34]-salience*))
   (partial-loop
      (context ?cont)
      (length 33)
      (nb-cont ?nb-cont)
...
=>
   ...
   (if (< (+ ?nb-cont (nb-contacts ?typey ?rowy ?coly)) ?*sum-of-givens*)
      then ; value 1 would make a too short loop; it must be deleted
         ...
         (print-loop 34 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 0)
      else ; loop length is ok for solution; value 0 must be retracted to allow closing the loop
         (retract ?cand2)
         (modify ?cand (status c-value))
         (print-loop 34 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 1)
   )
