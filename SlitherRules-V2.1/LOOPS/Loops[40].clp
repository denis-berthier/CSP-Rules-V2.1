...
(defrule activate-loop[40]
   (declare (salience ?*activate-loop[40]-salience*))
   (logical (context (name ?cont)) (technique ?cont BRT))
   (not (deactivate ?cont loop))
=>
   (assert (technique ?cont loop[40]))
   (assert (technique ?cont partial-loop[39]))
   (bind ?*technique* loop[40])
)
...
(defrule partial-loop[39]
   (declare (salience ?*partial-loop[39]-salience*))
   (logical
      (partial-loop
         (context ?cont)
         (length 37)
      ...
      (technique ?cont partial-loop[39])
      ;;; extension1
...
(defrule loop-elimination-or-solution[40]
   (declare (salience ?*loop[40]-salience*))
   (partial-loop
      (context ?cont)
      (length 39)
      (nb-cont ?nb-cont)
...
=>
   ...
   (if (< (+ ?nb-cont (nb-contacts ?typey ?rowy ?coly)) ?*sum-of-givens*)
      then ; value 1 would make a too short loop; it must be deleted
         ...
         (print-loop 40 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 0)
      else ; loop length is ok for solution; value 0 must be retracted to allow closing the loop
         (retract ?cand2)
         (modify ?cand (status c-value))
         (print-loop 40 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 1)
   )
