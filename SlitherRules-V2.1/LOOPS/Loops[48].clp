...
(defrule activate-loop[48]
   (declare (salience ?*activate-loop[48]-salience*))
   (logical (context (name ?cont)) (technique ?cont BRT))
   (not (deactivate ?cont loop))
=>
   (assert (technique ?cont loop[48]))
   (assert (technique ?cont partial-loop[47]))
   (bind ?*technique* loop[48])
)
...
(defrule partial-loop[47]
   (declare (salience ?*partial-loop[47]-salience*))
   (logical
      (partial-loop
         (context ?cont)
         (length 45)
      ...
      (technique ?cont partial-loop[47])
      ;;; extension1
...
(defrule loop-elimination-or-solution[48]
   (declare (salience ?*loop[48]-salience*))
   (partial-loop
      (context ?cont)
      (length 47)
      (nb-cont ?nb-cont)
...
=>
   ...
   (if (< (+ ?nb-cont (nb-contacts ?typey ?rowy ?coly)) ?*sum-of-givens*)
      then ; value 1 would make a too short loop; it must be deleted
         ...
         (print-loop 48 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 0)
      else ; loop length is ok for solution; value 0 must be retracted to allow closing the loop
         (retract ?cand2)
         (modify ?cand (status c-value))
         (print-loop 48 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 1)
   )
