...
(defrule activate-loop[52]
   (declare (salience ?*activate-loop[52]-salience*))
   (logical (context (name ?cont)) (technique ?cont BRT))
   (not (deactivate ?cont loop))
=>
   (assert (technique ?cont loop[52]))
   (assert (technique ?cont partial-loop[51]))
   (bind ?*technique* loop[52])
)
...
(defrule partial-loop[51]
   (declare (salience ?*partial-loop[51]-salience*))
   (logical
      (partial-loop
         (context ?cont)
         (length 49)
      ...
      (technique ?cont partial-loop[51])
      ;;; extension1
...
(defrule loop-elimination-or-solution[52]
   (declare (salience ?*loop[52]-salience*))
   (partial-loop
      (context ?cont)
      (length 51)
      (nb-cont ?nb-cont)
...
=>
   ...
   (if (< (+ ?nb-cont (nb-contacts ?typey ?rowy ?coly)) ?*sum-of-givens*)
      then ; value 1 would make a too short loop; it must be deleted
         ...
         (print-loop 52 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 0)
      else ; loop length is ok for solution; value 0 must be retracted to allow closing the loop
         (retract ?cand2)
         (modify ?cand (status c-value))
         (print-loop 52 ?row1 ?col1 ?type1 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?rowy ?coly ?typey 1)
   )
