...
=>
    ...
    (assert (blocked ?cont diagonal-3s ?row ?col ?diag))
	(if (or ?*print-actions* ?*print-diagonal-3s*) then
        (bind ?*blocked-rule-description* 
            (str-cat "w[1]-diagonal-3s-in-" ?*starting-cell-symbol* (row-name ?row) (column-name ?col) ?*diagonal-symbol* (row-name ?row2) (column-name ?col2)  ?*ending-cell-symbol*)
        )
    )
)
...
(defrule apply-diagonal-3s-T
    ...
    (blocked ?cont diagonal-3s ?row ?col ?diag)
...
;;; Similarly, if two 3s are in the same diagonal, but separated by any number of 2s (and only 2s)
;;; the outside edges of the 3s must be filled in, just as if they were diagonally adjacent.
...
=>
    (assert (apply-rule-as-a-block ?cont))
    (assert (blocked ?cont diagonal-3-2s-3 ?row ?col ?diag 1))
	(if (or ?*print-actions* ?*print-diagonal-3s*) then
        (bind ?*blocked-rule-description* 
            (str-cat "w[1]-diagonal-3-2-3-in-" ?*starting-cell-symbol* (row-name ?row) (column-name ?col) ?*diagonal-symbol* (row-name ?row3) (column-name ?col3)  ?*ending-cell-symbol*)
        )
    )
)


(defrule detect-diagonal-3-2-2-3
...
=>
    (assert (apply-rule-as-a-block ?cont))
    (assert (blocked ?cont diagonal-3-2s-3 ?row ?col ?diag 2))
	(if (or ?*print-actions* ?*print-diagonal-3s*) then
        (bind ?*blocked-rule-description* 
            (str-cat "w[1]-diagonal-3-2s-3-in-" ?*starting-cell-symbol* (row-name ?row) (column-name ?col) ?*diagonal-symbol* (row-name ?row4) (column-name ?col4)  ?*ending-cell-symbol*)
        )
    )
)


(defrule detect-diagonal-3-2-2-2-3
...
=>
    (assert (apply-rule-as-a-block ?cont))
    (assert (blocked ?cont diagonal-3-2s-3 ?row ?col ?diag 3))
	(if (or ?*print-actions* ?*print-diagonal-3s*) then
        (bind ?*blocked-rule-description* 
            (str-cat "w[1]-diagonal-3-2s-3-in-" ?*starting-cell-symbol* (row-name ?row) (column-name ?col) ?*diagonal-symbol* (row-name ?row5) (column-name ?col5)  ?*ending-cell-symbol*)
        )
    )
)


(defrule detect-diagonal-3-2-2-2-2-3
...
=>
    (assert (apply-rule-as-a-block ?cont))
    (assert (blocked ?cont diagonal-3-2s-3 ?row ?col ?diag 4))
	(if (or ?*print-actions* ?*print-diagonal-3s*) then
        (bind ?*blocked-rule-description* 
            (str-cat "w[1]-diagonal-3-2s-3-in-" ?*starting-cell-symbol* (row-name ?row) (column-name ?col) ?*diagonal-symbol* (row-name ?row6) (column-name ?col6)  ?*ending-cell-symbol*)
        )
    )
)


(defrule detect-diagonal-3-2-2-2-2-2-3
...
=>
    (assert (apply-rule-as-a-block ?cont))
    (assert (blocked ?cont diagonal-3-2s-3 ?row ?col ?diag 5))
    (if (or ?*print-actions* ?*print-diagonal-3s*) then
        (bind ?*blocked-rule-description*
            (str-cat "w[1]-diagonal-3-2s-3-in-" ?*starting-cell-symbol* (row-name ?row) (column-name ?col) ?*diagonal-symbol* (row-name ?row7) (column-name ?col7)  ?*ending-cell-symbol*)
        )
    )
)
...
(defrule detect-diagonal-3-2-2-2-2-2-2-3
    (declare (salience ?*detect-diagonal-3-2-2-2-2-2-2-3-salience*))
    (technique ?cont diagonal-3s)
        
    (diagonal-3-2s ?cont ?row ?col ?row7 ?col7 ?diag&se|sw 7)
    (candidate (context ?cont) (status c-value) (type N) (row ?row8) (column ?col8) (value 3))
    (test (is-direction ?diag ?row7 ?col7 ?row8 ?col8))
=>
    (assert (apply-rule-as-a-block ?cont))
    (assert (blocked ?cont diagonal-3-2s-3 ?row ?col ?diag 6))
    (if (or ?*print-actions* ?*print-diagonal-3s*) then
        (bind ?*blocked-rule-description*
            (str-cat "w[1]-diagonal-3-2s-3-in-" ?*starting-cell-symbol* (row-name ?row) (column-name ?col) ?*diagonal-symbol* (row-name ?row8) (column-name ?col8)  ?*ending-cell-symbol*)
        )
    )
)
...
(defrule apply-diagonal-3-2s-3-T
	(declare (salience ?*apply-a-blocked-rule-salience-1*))
    ;;; ?k is the number of 2s
    (blocked ?cont diagonal-3-2s-3 ?row ?col ?diag ?k)
...
(defrule apply-diagonal-3-2s-3-F
	(declare (salience ?*apply-a-blocked-rule-salience-2*))
    (blocked ?cont diagonal-3-2s-3 ?row ?col ?diag ?k)
