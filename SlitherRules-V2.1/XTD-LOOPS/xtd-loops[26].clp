...
(defrule activate-xtd-loop[26]
   (declare (salience ?*activate-xtd-loop[26]-salience*))
   (logical (context (name ?cont)) (technique ?cont BRT))
   (not (deactivate ?cont xtd-loop))
=>
   (assert (technique ?cont xtd-loop[26]))
   (assert (technique ?cont partial-loop[25]))
   (bind ?*technique* xtd-loop[26])
)
...
(defrule xtd-loop-elimination-or-solution[26]
   (declare (salience ?*xtd-loop[26]-salience*))
   (partial-loop
      (context ?cont)
      (length ?p&:(< ?p 25))
      (nb-cont ?nb-cont)
