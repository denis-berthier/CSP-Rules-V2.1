...
(defrule activate-xtd-loop[32]
   (declare (salience ?*activate-xtd-loop[32]-salience*))
   (logical (context (name ?cont)) (technique ?cont BRT))
   (not (deactivate ?cont xtd-loop))
=>
   (assert (technique ?cont xtd-loop[32]))
   (assert (technique ?cont partial-loop[31]))
   (bind ?*technique* xtd-loop[32])
)
...
(defrule xtd-loop-elimination-or-solution[32]
   (declare (salience ?*xtd-loop[32]-salience*))
   (partial-loop
      (context ?cont)
      (length ?p&:(< ?p 31))
      (nb-cont ?nb-cont)
...
=>
   ...
   (print-xtd-loop 32 ?row1 ?col1 ?type1 ?row2 ?col2 ?type2 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?row12 ?col12 ?type12 $?rows2 $?cols2 $?types2 ?rowp2 ?colp2 ?typep2 ?assert-values)
)
