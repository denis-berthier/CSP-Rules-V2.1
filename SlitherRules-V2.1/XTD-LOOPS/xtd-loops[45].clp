...
(defrule activate-xtd-loop[45]
   (declare (salience ?*activate-xtd-loop[45]-salience*))
   (logical (context (name ?cont)) (technique ?cont BRT))
   (not (deactivate ?cont xtd-loop))
=>
   (assert (technique ?cont xtd-loop[45]))
   (assert (technique ?cont partial-loop[44]))
   (bind ?*technique* xtd-loop[45])
)
...
(defrule xtd-loop-elimination-or-solution[45]
   (declare (salience ?*xtd-loop[45]-salience*))
   (partial-loop
      (context ?cont)
      (length ?p&:(< ?p 44))
      (nb-cont ?nb-cont)
...
=>
   ...
   (print-xtd-loop 45 ?row1 ?col1 ?type1 ?row2 ?col2 ?type2 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?row12 ?col12 ?type12 $?rows2 $?cols2 $?types2 ?rowp2 ?colp2 ?typep2 ?assert-values)
)
