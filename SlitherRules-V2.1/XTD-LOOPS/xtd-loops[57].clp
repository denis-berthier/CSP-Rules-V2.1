...
(defrule activate-xtd-loop[57]
   (declare (salience ?*activate-xtd-loop[57]-salience*))
   (logical (context (name ?cont)) (technique ?cont BRT))
   (not (deactivate ?cont xtd-loop))
=>
   (assert (technique ?cont xtd-loop[57]))
   (assert (technique ?cont partial-loop[56]))
   (bind ?*technique* xtd-loop[57])
)
...
(defrule xtd-loop-elimination-or-solution[57]
   (declare (salience ?*xtd-loop[57]-salience*))
   (partial-loop
      (context ?cont)
      (length ?p&:(< ?p 56))
      (nb-cont ?nb-cont)
...
=>
   ...
   (print-xtd-loop 57 ?row1 ?col1 ?type1 ?row2 ?col2 ?type2 $?middle-rows $?middle-cols $?middle-types ?rowp ?colp ?typep ?rowq ?colq ?typeq ?row12 ?col12 ?type12 $?rows2 $?cols2 $?types2 ?rowp2 ?colp2 ?typep2 ?assert-values)
)
