...
(deffunction stats-n-grids-after-first-p-from-text-file-keeping-solutions (?file-name ?p ?n ?levels-file ?times-file ?nb-facts-file ?solns-file)
	...
	(open ?times-file "times-file" "w")
	(open ?nb-facts-file "nb-facts-file" "w")
	(open ?solns-file ?*solutions-file* "w")
	(bind ?i 1)
	(while (< ?i (+ ?p 1))
...
(deffunction compare (?file1 ?file2 ?p ?nb)
	(open ?file1 "file1" "r")
	(open ?file2 "file2" "r")
	(bind ?i 1)
	(while (< ?i (+ ?nb 1))
		(bind ?L1 (readline "file1"))
		(bind ?L2 (readline "file2"))
		(if (neq (str-compare ?L1 ?L2) 0) then (printout t (+ ?p ?i) "   " ?L1 "   " ?L2 crlf))
		(bind ?i (+ ?i 1))
	)
	(close "file1")
	(close "file2")
