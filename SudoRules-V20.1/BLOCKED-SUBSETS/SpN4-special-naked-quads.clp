...
(defrule detect-L4-special-naked-quads-in-a-block
	(declare (salience ?*naked-quads-salience*))
	(technique ?cont naked-quads)
    ...
    =>
    (assert (apply-rule-as-a-block ?cont))
    (assert (blocked ?cont special-naked-quads-in-a-block ?bl ?sq1 ?sq2 ?sq3 ?sq4 ?nb1 ?nb2 ?nb3 ?nb4))
	(if (or ?*print-actions* ?*print-L4* ?*print-naked-quads* ?*print-exceptional-patterns*) then
        (bind ?*blocked-rule-description*
            (str-cat "special-naked-quads-in-a-block: "
				(block-name ?bl)
...
(defrule assert-L4-special-naked-quads-in-a-block
    (declare (salience ?*apply-a-blocked-rule-salience-1*))
    (blocked ?cont special-naked-quads-in-a-block ?bl ?sq1 ?sq2 ?sq3 ?sq4 ?nb1 ?nb2 ?nb3 ?nb4)
