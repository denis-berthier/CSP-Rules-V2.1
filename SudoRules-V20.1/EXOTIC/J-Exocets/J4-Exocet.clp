...
(defrule activate-J4-Exocet
	(declare (salience ?*activate-J4-Exocet-salience*))
	(logical (play) (context (name ?cont)))
    (not (deactivate ?cont J4-Exocet))
=>
	(if ?*print-levels* then (printout t "entering level J4E"))
	(assert (technique ?cont J4-Exocet))
	(bind ?*technique* J4-Exocet)
)



(defrule track-J4-Exocet
	(declare (salience ?*activate-J4-Exocet-salience*))
	(logical (play))
	?level <- (technique ?cont J4-Exocet)
=>
...
(defrule J4-Exocet-in-rows-detection
	(declare (salience ?*J4-Exocet-detection-salience*))
    ...
    (exists (column ?colx3&:(not (column-intersects ?colx3 ?blkB)))
        (exists (column ?coly3&:(< ?colx3 ?coly3)&:(not (column-intersects ?coly3 ?blkB)))
            (not (candidate (context ?cont) (number ?nb3) (row ?rowz&?rowSB|?rowSQ|?rowSR) (column ?colz&~?colx3&~?coly3&:(not (column-intersects ?colz ?blkB)))))
        )
    )
    ...
    (exists (column ?colx4&:(not (column-intersects ?colx4 ?blkB)))
        (exists (column ?coly4&:(< ?colx4 ?coly4)&:(not (column-intersects ?coly4 ?blkB)))
            (not (candidate (context ?cont) (number ?nb4) (row ?rowz&?rowSB|?rowSQ|?rowSR) (column ?colz&~?colx4&~?coly4&:(not (column-intersects ?colz ?blkB)))))
        )
    )
    ...
    (not (candidate (context ?cont) (number ?nb&?nb1|?nb2|?nb3|?nb4) (column ?colCQ) (row ?rowSQ)))
    (column-intersects ?colCR&~?colB ?blkB)
    (not (candidate (context ?cont) (number ?nb&?nb1|?nb2|?nb3|?nb4) (column ?colCR) (row ?rowSR)))
    
    ;;; conditions on digits in Base Cells
    (not (candidate (context ?cont) (column ?colB) (row ?row1) (number ?nb&~?nb1&~?nb2&~?nb3&~?nb4)))
    (not (candidate (context ?cont) (column ?colB) (row ?row2) (number ?nb&~?nb1&~?nb2&~?nb3&~?nb4)))
    ...
    (exists (candidate (context ?cont) (status cand) (number ?nbx1&?nb1|?nb2|?nb3|?nb4) (column ?colTQ) (row ?rowSQ)))
    (column-intersects ?colTR&~?colB&~?colCR ?blkB)
    (exists (candidate (context ?cont) (status cand) (number ?nbx1&?nb1|?nb2|?nb3|?nb4) (column ?colTR) (row ?rowSR)))
    
    (exists (candidate (context ?cont) (status cand) (number ?nbz1&?nb1|?nb2|?nb3|?nb4) (column ?colz1&?colTQ|?colTR) (row ?rowz1&?rowSQ|?rowSR))
        (test (target-cell ?rowz1 ?colz1 ?rowSQ ?colTQ ?rowSR ?colTR))
        (exists (candidate (context ?cont) (status cand) (number ?nbz2&:(<  ?nbz1 ?nbz2)&:(or (eq ?nbz2 ?nb1) (eq ?nbz2 ?nb2) (eq ?nbz2 ?nb3) (eq ?nbz2 ?nb4))) (column ?colz2&?colTQ|?colTR) (row ?rowz2&?rowSQ|?rowSR))
            (test (target-cell ?rowz2 ?colz2 ?rowSQ ?colTQ ?rowSR ?colTR))
        )
    )

	(not (J4-exocet-in-rows ?cont ?nb1 ?nb2 ?nb3 ?nb4 ?colB ?row1 ?row2 ?rowSB ?rowSQ ?rowSR ?colTQ ?colTR))

 =>
	(assert (J4-exocet-in-rows ?cont ?nb1 ?nb2 ?nb3 ?nb4 ?colB ?row1 ?row2 ?rowSB ?rowSQ ?rowSR ?colTQ ?colTR))
    (bind ?*has-J-exocet* TRUE)
	(if (or ?*print-actions* ?*print-JExocet*) then
            (printout t 
                "J4-Exocet-in-rows: " crlf
                "     base digits = " (number-name ?nb1) ", " (number-name ?nb2) ", " (number-name ?nb3) ", " (number-name ?nb4) crlf
...
(defrule J4-Exocet-in-rows-elimination
	(declare (salience ?*apply-a-blocked-rule-salience*))
	(technique ?cont J4-Exocet)
	(J4-exocet-in-rows ?cont ?nb1 ?nb2 ?nb3 ?nb4 ?colB ?row1 ?row2 ?rowSB ?rowSQ ?rowSR ?colTQ ?colTR)

    ;;; target candidate
    ?candz <- (candidate (context ?cont) (status cand) (number ?nbz&~?nb1&~?nb2&~?nb3&~?nb4) (column ?colz&?colTQ|?colTR) (row ?rowz&?rowSQ|?rowSR))
...
(defrule J4-Exocet-in-columns-detection
	(declare (salience ?*J4-Exocet-detection-salience*))
    ...
    (exists (row ?rowx3&:(not (row-intersects ?rowx3 ?blkB)))
        (exists (row ?rowy3&:(< ?rowx3 ?rowy3)&:(not (row-intersects ?rowy3 ?blkB)))
            (not (candidate (context ?cont) (number ?nb3) (column ?colz&?colSB|?colSQ|?colSR) (row ?rowz&~?rowx3&~?rowy3&:(not (row-intersects ?rowz ?blkB)))))
        )
    )
    ...
    (exists (row ?rowx4&:(not (row-intersects ?rowx4 ?blkB)))
        (exists (row ?rowy4&:(< ?rowx4 ?rowy4)&:(not (row-intersects ?rowy4 ?blkB)))
            (not (candidate (context ?cont) (number ?nb4) (column ?colz&?colSB|?colSQ|?colSR) (row ?rowz&~?rowx4&~?rowy4&:(not (row-intersects ?rowz ?blkB)))))
        )
    )

    ;;; conditions on digits in first and second Companion Cells, resp. (rowCQ, colSQ) and (rowCR, colSR)
    (row-intersects ?rowCQ&~?rowB ?blkB)
    (not (candidate (context ?cont) (number ?nb&?nb1|?nb2|?nb3|?nb4) (row ?rowCQ) (column ?colSQ)))
    (row-intersects ?rowCR&~?rowB ?blkB)
    (not (candidate (context ?cont) (number ?nb&?nb1|?nb2|?nb3|?nb4) (row ?rowCR) (column ?colSR)))

    ;;; conditions on digits in Base Cells
    (not (candidate (context ?cont) (row ?rowB) (column ?col1) (number ?nb&~?nb1&~?nb2&~?nb3&~?nb4)))
    (not (candidate (context ?cont) (row ?rowB) (column ?col2) (number ?nb&~?nb1&~?nb2&~?nb3&~?nb4)))
    ...
    (exists (candidate (context ?cont) (status cand) (number ?nbx1&?nb1|?nb2|?nb3|?nb4) (row ?rowTQ) (column ?colSQ)))
    (row-intersects ?rowTR&~?rowB&~?rowCR ?blkB)
    (exists (candidate (context ?cont) (status cand) (number ?nbx1&?nb1|?nb2|?nb3|?nb4) (row ?rowTR) (column ?colSR)))
    
    (exists (candidate (context ?cont) (status cand) (number ?nbz1&?nb1|?nb2|?nb3|?nb4) (row ?rowz1&?rowTQ|?rowTR) (column ?colz1&?colSQ|?colSR))
        (test (target-cell ?rowz1 ?colz1 ?rowTQ ?colSQ ?rowTR ?colSR))
        (exists (candidate (context ?cont) (status cand) (number ?nbz2&:(<  ?nbz1 ?nbz2)&:(or (eq ?nbz2 ?nb1) (eq ?nbz2 ?nb2) (eq ?nbz2 ?nb3) (eq ?nbz2 ?nb4))) (row ?rowz2&?rowTQ|?rowTR) (column ?colz2&?colSQ|?colSR))
            (test (target-cell ?rowz2 ?colz2 ?rowTQ ?colSQ ?rowTR ?colSR))
        )
    )

    (not (J4-exocet-in-columns ?cont ?nb1 ?nb2 ?nb3 ?nb4 ?rowB ?col1 ?col2 ?colSB ?colSQ ?colSR ?rowTQ ?rowTR))

 =>
	(assert (J4-exocet-in-columns ?cont ?nb1 ?nb2 ?nb3 ?nb4 ?rowB ?col1 ?col2 ?colSB ?colSQ ?colSR ?rowTQ ?rowTR))
    (bind ?*has-J-exocet* TRUE)
	(if (or ?*print-actions* ?*print-JExocet*) then
            (printout t 
                "J4-Exocet-in-columns: " crlf
                "     base digits = " (number-name ?nb1) ", " (number-name ?nb2) ", " (number-name ?nb3) ", " (number-name ?nb4) crlf
...
(defrule J4-Exocet-in-columns-elimination
	(declare (salience ?*apply-a-blocked-rule-salience*))
	(technique ?cont J4-Exocet)
	(J4-exocet-in-columns ?cont ?nb1 ?nb2 ?nb3 ?nb4 ?rowB ?col1 ?col2 ?colSB ?colSQ ?colSR ?rowTQ ?rowTR)

    ;;; target candidate
    ?candz <- (candidate (context ?cont) (status cand) (number ?nbz&~?nb1&~?nb2&~?nb3&~?nb4) (row ?rowz&?rowTQ|?rowTR) (column ?colz&?colSQ|?colSR))
