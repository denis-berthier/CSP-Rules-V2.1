...
(deffunction define-specific-pre-Singles-saliences ()
    (if (and ?*deadly-patterns-before-singles* (not ?*deadly-patterns-after-singles*))
        then (define-deadly-patterns-saliences)
        else TRUE
    )
)
(deffunction define-specific-post-Singles-pre-init-links-saliences ()
    (if (and ?*deadly-patterns-after-singles* (not ?*deadly-patterns-before-singles*))
        then (define-deadly-patterns-saliences)
        else TRUE
    )
)
...
(deffunction define-specific-saliences-at-L3 ()
    ...
    (bind ?*g-triplets-salience* (bind ?*next-rule-salience* (- ?*next-rule-salience* 1)))
     ...
     ;;; more complex use of deadly patterns, ; default behaviour:
     (if (and (not ?*deadly-patterns-before-singles*) (not ?*deadly-patterns-after-singles*))
         then (define-deadly-patterns-saliences))
