...
(defrule L3-naked-triplets-in-a-block
	(declare (salience ?*naked-triplets-salience*))
	(technique ?cont naked-triplets)
...
=>
    ...
    (if (eq ?cont 0) then (bind ?*nb-candidates* (- ?*nb-candidates* 1)))
    (if (or ?*print-actions* ?*print-L3* ?*print-naked-triplets*) then
            (printout t "naked-triplets-in-a-block: "
                (block-name ?bl)
    ...
    (if ?*blocked-Subsets* then
        (assert (apply-rule-as-a-pseudo-block ?cont))
        (assert (pseudo-blocked ?cont naked-triplets-in-a-block ?zzz ?bl ?sq1 ?sq2 ?sq3 ?nb1 ?nb2 ?nb3))
    )
)


(defrule apply-to-more-targets-L3-naked-triplets-in-a-block
    (declare (salience ?*apply-a-blocked-rule-salience*))
    (pseudo-blocked ?cont naked-triplets-in-a-block ?zzz ?bl ?sq1 ?sq2 ?sq3 ?nb1 ?nb2 ?nb3)
