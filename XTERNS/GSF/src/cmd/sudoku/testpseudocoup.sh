...
do	case $1 in
	...
	--help|--man)
		cat 1>&2 <<!!
Usage: \$SHELL testpseudocoup.sh [ --accept ] [ unit ... ]

pseudocoup regression test script.  Run this script to generate new
results in testpseudocoup.tmp and compare with expected results in
testpseudocoup.out.  The --accept option generates testpseudocoup.tmp
and moves it to testpseudocoup.out.
!!
		exit 2
		;;
...
} > testpseudocoup.tmp 2>&1 < /dev/null
case $ACCEPT in
0)	if	grep '$' testpseudocoup.tmp >/dev/null
	then	mv testpseudocoup.tmp testpseudocoup.junk
		sed 's/$//' < testpseudocoup.junk > testpseudocoup.tmp
		rm -f testpseudocoup.junk
	fi
	if	cmp -s testpseudocoup.tmp testpseudocoup.out
	then	echo pseudocoup tests PASSED
		rm -f testpseudocoup.tmp
	else	echo pseudocoup tests FAILED
		diff  testpseudocoup.tmp testpseudocoup.out
	fi
	;;

*)	mv testpseudocoup.tmp testpseudocoup.out
	;;
esac
