...
#                                                                      #
########################################################################
# : : generated from sudocoo.rt by mktest : : #
ACCEPT=0
while	:
do	case $1 in
	-a|--accept)
	...
	--help|--man)
		cat 1>&2 <<!!
Usage: \$SHELL testsudocoo.sh [ --accept ] [ unit ... ]

sudocoo regression test script.  Run this script to generate new
results in testsudocoo.tmp and compare with expected results in
testsudocoo.out.  The --accept option generates testsudocoo.tmp
and moves it to testsudocoo.out.
!!
		exit 2
		;;
...
020000600000100000700030050008040903300000008000020406630500000005009002010084000
!TEST-INPUT!
CODE=$?
case $CODE in
0) ;;
...
} > testsudocoo.tmp 2>&1 < /dev/null
case $ACCEPT in
0)	if	grep '$' testsudocoo.tmp >/dev/null
	then	mv testsudocoo.tmp testsudocoo.junk
		sed 's/$//' < testsudocoo.junk > testsudocoo.tmp
		rm -f testsudocoo.junk
	fi
	if	cmp -s testsudocoo.tmp testsudocoo.out
	then	echo sudocoo tests PASSED
		rm -f testsudocoo.tmp
	else	echo sudocoo tests FAILED
		diff  testsudocoo.tmp testsudocoo.out
	fi
	;;

*)	mv testsudocoo.tmp testsudocoo.out
	;;
esac
